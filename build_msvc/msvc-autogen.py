...
libs = [
    "libsugarchain_cli",
    "libsugarchain_common",
    "libsugarchain_crypto",
    "libsugarchain_node",
    "libsugarchain_util",
    "libsugarchain_wallet_tool",
    "libsugarchain_wallet",
    "libsugarchain_zmq",
    "bench_sugarchain",
    "libtest_util",
]
...
def parse_config_into_sugar_config():
    ...
    config_info = []
    with open(
        os.path.join(SOURCE_DIR, "../configure.ac"), encoding="utf8"
    ) as f:
        for line in f:
    ...
    with open(
        ...
        sugar_config.writelines(template)


def set_properties(vcxproj_filename, placeholder, content):
    with open(
        vcxproj_filename + ".in", "r", encoding="utf-8"
    ) as vcxproj_in_file:
        with open(vcxproj_filename, "w", encoding="utf-8") as vcxproj_file:
            vcxproj_file.write(
                vcxproj_in_file.read().replace(placeholder, content)
            )


def main():
    parser = argparse.ArgumentParser(
        description="Sugarchain-core msbuild configuration initialiser."
    )
    parser.add_argument(
        "-toolset",
        nargs="?",
        default=DEFAULT_PLATFORM_TOOLSET,
        help="Optionally sets the msbuild platform toolset, e.g. v143 for Visual Studio 2022."
        " default is %s." % DEFAULT_PLATFORM_TOOLSET,
    )
    args = parser.parse_args()
    set_properties(
        os.path.join(SOURCE_DIR, "../build_msvc/common.init.vcxproj"),
        "@TOOLSET@",
        args.toolset,
    )
    ...
    copyfile(
        os.path.join(SOURCE_DIR, "../build_msvc/sugarchain_config.h"),
        os.path.join(SOURCE_DIR, "config/sugarchain-config.h"),
    )
    copyfile(
        os.path.join(SOURCE_DIR, "../build_msvc/libsecp256k1_config.h"),
        os.path.join(SOURCE_DIR, "secp256k1/src/libsecp256k1-config.h"),
    )
