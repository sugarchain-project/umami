# Fuzzing Sugarchain Core using libFuzzer
...
## Overview of Sugarchain Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/sugarchain_report.pdf) includes an external overview of the status of Sugarchain Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.
...
It is possible to specify `sugarchaind` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`sugarchain-core/qa-assets`](https://github.com/sugarchain-core/qa-assets) repo.

To fuzz `process_message` using the [`sugarchain-core/qa-assets`](https://github.com/sugarchain-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/sugarchain-core/qa-assets
...
Fuzzing on a harness compiled with `--with-sanitizers=address,fuzzer,undefined` is good for finding bugs. However, the very slow execution even under libFuzzer will limit the ability to find new coverage. A good approach is to perform occasional long runs without the additional bug-detectors (configure `--with-sanitizers=fuzzer`) and then merge new inputs into a corpus as described in the qa-assets repo (https://github.com/sugarchain-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).  Patience is useful; even with improved throughput, libFuzzer may need days and 10s of millions of executions to reach deep/hard targets.
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`sugarchain-core/qa-assets`](https://github.com/sugarchain-core/qa-assets) repo.

Every single pull request submitted against the Sugarchain Core repo is automatically tested against all inputs in the [`sugarchain-core/qa-assets`](https://github.com/sugarchain-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Sugarchain Core more robust.
...
# Fuzzing Sugarchain Core using afl++
...
# Fuzzing Sugarchain Core using Honggfuzz
...
## Fuzzing the Sugarchain Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Sugarchain
Core without having to write any custom fuzzing harness. The `sugarchaind` server
...
$ git apply << "EOF"
diff --git a/src/sugarchaind.cpp b/src/sugarchaind.cpp
index 455a82e39..2faa3f80f 100644
--- a/src/sugarchaind.cpp
+++ b/src/sugarchaind.cpp
...
EOF
$ make -C src/ sugarchaind
$ mkdir -p inputs/
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=45340 -f inputs/ -- \
          src/sugarchaind -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:45340 -logthreadnames \
                       -debug
```

# Fuzzing Sugarchain Core using Eclipser (v1.x)
...
Sugarchain Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/sugarchain-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=sugarchain-core).
...
because Sugarchain is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_sugarchain-core/latest).
