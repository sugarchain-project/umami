# User-space, Statically Defined Tracing (USDT) for Sugarchain Core

Sugarchain Core includes statically defined tracepoints to allow for more
...
**Note**: Sugarchain Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Sugarchain Core
...
Multiple tools can list the available tracepoints in a `sugarchaind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Sugarchain Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/sugarchaind
stap net        outbound_message 0x0000000000107c05 /src/sugarchaind
stap validation block_connected  0x00000000002fb10c /src/sugarchaind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Sugarchain Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/sugarchaind | grep NT_STAPSDT -A 4 -B 2
