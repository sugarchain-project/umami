...
This guide describes how to build sugarchaind, command-line utilities, and GUI on NetBSD.
...
It is not necessary to build wallet functionality to run sugarchaind or the GUI.

###### Descriptor Wallet Support

`sqlite3` is required to enable support for [descriptor wallets](https://github.com/sugarchain/sugarchain/blob/master/doc/descriptors.md).
...
Sugarchain Core includes a GUI built with the cross-platform Qt Framework. To compile the GUI, we need to install `qt5`.
...
There are many ways to configure Sugarchain Core. Here is an example that
