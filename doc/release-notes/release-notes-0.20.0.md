...
#### Removal of BIP61 reject network messages from Sugarchain Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Sugarchain Core version 0.18.0.
...
- Testing or debugging of implementations of the Sugarchain P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Sugarchain Core. Sugarchain Core logs debug messages
...
- OpenSSL is no longer used by Sugarchain Core.  (#17265)

- BIP70 support has been fully removed from Sugarchain Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/sugarchain/sugarchain/issues/16599),
  [PR #16702](https://github.com/sugarchain/sugarchain/pull/16702), and the
  `sugarchaind help` for more information.  This option is experimental and
...
- All custom settings configured when Sugarchain Core starts are now
...
- The "Start Sugarchain Core on system login" option has been removed on macOS.
  (#17567)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Sugarchain Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/sugarchain-core/HWI) for
  signing.  Future versions of Sugarchain Core should support a GUI option
...
- The `sugarchain-cli` utility used with the `-getinfo` parameter now
...
- The `sugarchain-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `sugarchainstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_sugarchain-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_sugarchain` binary (hebasto)
...
- #18702 Fix ASLR for sugarchain-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate sugarchain schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest sugarchain.conf (sdaftuar)
...
- #18022 Fix appveyor `test_sugarchain` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (sugarchain-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone sugarchain-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Sugarchain Core files (hebasto)
...
- #17119 Fix broken sugarchain-cli examples (andrewtoth)
...
- #17648 Rename wallet-tool references to sugarchain-wallet (hel-o)
...
- #18341 Replace remaining literal SUGAR with `CURRENCY_UNIT` (domob1812)
...
- #16115 On sugarchaind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from sugarchain-cli -getinfo (malevolent)
