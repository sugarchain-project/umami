# CJDNS support in Sugarchain Core

It is possible to run Sugarchain Core over CJDNS, an encrypted IPv6 network that
...
done outside of Sugarchain Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Sugarchain network and individual nodes.
...
before it will work with your Sugarchain Core node. This is described in steps
...
## Run Sugarchain Core with CJDNS

Once you are connected to the CJDNS network, the following Sugarchain Core
...
When enabled, this option tells Sugarchain Core that it is running in an
...
IPv6 local network. This helps Sugarchain Core perform better address management:
...
CJDNS support was added to Sugarchain Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `sugarchain-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/sugarchain/sugarchain/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Sugarchain Core

There are several ways to see your CJDNS address in Sugarchain Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `sugarchain-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `sugarchain-cli getpeerinfo`).
