...
- Translations on Transifex:
    ...
    - Create [a new resource](https://www.transifex.com/sugarchain/sugarchain/content/) named after the major version with the slug `qt-translation-<RRR>x`, where `RRR` is the major branch number padded with zeros. Use `src/qt/locale/sugarchain_en.xlf` to create it.
    ...
    - Make an announcement that translators can start translating for the new version. You can use one of the [previous announcements](https://www.transifex.com/sugarchain/communication/) as a template.
    - Change the auto-update URL for the resource to `master`, e.g. `https://raw.githubusercontent.com/sugarchain/sugarchain/master/src/qt/locale/sugarchain_en.xlf`. (Do this only after the previous steps, to prevent an auto-update from interfering.)
...
- Merge the release notes from [the wiki](https://github.com/sugarchain-core/sugarchain-devwiki/wiki/) into the branch.
- Ensure the "Needs release note" label is removed from all relevant pull requests and issues.

#### Tagging a release (candidate)

To tag the version (or release candidate) in git, use the `make-tag.py` script from [sugarchain-maintainer-tools](https://github.com/sugarchain-core/sugarchain-maintainer-tools). From the root of the repository run:

    ../sugarchain-maintainer-tools/make-tag.py v(new version, e.g. 23.0)
...
Open a draft of the release notes for collaborative editing at https://github.com/sugarchain-core/sugarchain-devwiki/wiki.

For the period during which the notes are being edited on the wiki, the version on the branch should be wiped and replaced with a link to the wiki which should be used for all announcements until `-final`.

Generate the change log. As this is a huge amount of work to do manually, there is the `list-pulls` script to do a pre-sorting step based on github PR metadata. See the [documentation in the README.md](https://github.com/sugarchain-core/sugarchain-maintainer-tools/blob/master/README.md#list-pulls).
...
Checkout the Sugarchain Core version you'd like to build:
...
### macOS codesigner only: Create detached macOS signatures (assuming [signapple](https://github.com/achow101/signapple/) is installed and up to date with master branch)
    ...
    tar xf sugarchain-osx-unsigned.tar.gz
...
### Windows codesigner only: Create detached Windows signatures
    ...
    tar xf sugarchain-win-unsigned.tar.gz
...
However if this is done, once the release has been tagged in the sugarchain-detached-sigs repo, the `guix-codesign` step must be performed again in order for the guix attestation to be valid when compared against the attestations of non-codesigner builds.

### Windows and macOS codesigners only: Commit the detached codesign payloads

```sh
pushd ./sugarchain-detached-sigs
...
- Detached signatures will then be committed to the [sugarchain-detached-sigs](https://github.com/sugarchain-core/sugarchain-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
- Create a torrent of the `/var/www/bin/sugarchain-core-${VERSION}` directory such
  that at the top level there is only one file: the `sugarchain-core-${VERSION}`
  directory containing everything else. Name the torrent
  `sugarchain-${VERSION}.torrent` (note that there is no `-core-` in this name).
...
- Update other repositories and websites for new version
  ...
  - bitcoincore.org RPC documentation update
      ...
      - Install the new Sugarchain Core release
  ...
  - Update packaging repo
      ...
      - Push the flatpak to flathub, e.g. https://github.com/flathub/org.sugarchaincore.sugarchain-qt/pull/2

      - Push the snap, see https://github.com/sugarchain-core/packaging/blob/master/snap/build.md
