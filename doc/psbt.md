# PSBT Howto for Sugarchain Core

Since Sugarchain Core 0.17, an RPC interface exists for Partially Signed Sugarchain
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/sugarchain/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Sugarchain transactions that are not fully signed
...
#### Multisig with multiple Sugarchain Core instances
...
Sugarchain Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `sugarchain-cli` is used.
...
Later, when *V* SUGAR has been received on *Amulti*, and Bob and Carol want to
