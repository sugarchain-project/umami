...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/sugarchain.conf are parsed in qt/sugarchain.cpp's main()
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see sugarchaind -h for a list of options.", argv[i])));
            }
        }

        // -server defaults to true for sugarchaind but not for the GUI so do this here
