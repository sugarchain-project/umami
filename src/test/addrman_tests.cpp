...
BOOST_AUTO_TEST_CASE(addrman_simple)
{
    ...
    BOOST_CHECK_EQUAL(addr_ret1.ToStringAddrPort(), "250.1.1.1:34230");
...
BOOST_AUTO_TEST_CASE(addrman_ports)
{
    ...
    CService addr1_port = ResolveService("250.1.1.1", 34231);
    BOOST_CHECK(addrman->Add({CAddress(addr1_port, NODE_NONE)}, source));
    BOOST_CHECK_EQUAL(addrman->Size(), 2U);
    auto addr_ret2 = addrman->Select().first;
    BOOST_CHECK(addr_ret2.ToStringAddrPort() == "250.1.1.1:34230" || addr_ret2.ToStringAddrPort() == "250.1.1.1:34231");
    ...
    BOOST_CHECK_EQUAL(addr_ret3.ToStringAddrPort(), "250.1.1.1:34230");
}
...
BOOST_AUTO_TEST_CASE(addrman_select)
{
    ...
    BOOST_CHECK(addrman->Add({CAddress(addr2, NODE_NONE)}, ResolveService("250.3.1.1", 34230)));
    BOOST_CHECK(addrman->Add({CAddress(addr3, NODE_NONE)}, ResolveService("250.3.1.1", 34230)));
    BOOST_CHECK(addrman->Add({CAddress(addr4, NODE_NONE)}, ResolveService("250.4.1.1", 34230)));
    ...
    BOOST_CHECK(addrman->Add({CAddress(addr5, NODE_NONE)}, ResolveService("250.3.1.1", 34230)));
    BOOST_CHECK(addrman->Good(CAddress(addr5, NODE_NONE)));
    BOOST_CHECK(addrman->Add({CAddress(addr6, NODE_NONE)}, ResolveService("250.3.1.1", 34230)));
    BOOST_CHECK(addrman->Good(CAddress(addr6, NODE_NONE)));
    BOOST_CHECK(addrman->Add({CAddress(addr7, NODE_NONE)}, ResolveService("250.1.1.3", 34230)));
...
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    CAddress addr{CAddress(ResolveService("253.3.3.3", 34230), NODE_NONE)};
...
BOOST_AUTO_TEST_CASE(load_addrman)
{
    ...
    BOOST_CHECK(Lookup("250.7.1.1", addr1, 34230, false));
    ...
    // Add three addresses to new table.
    CService source;
    BOOST_CHECK(Lookup("252.5.1.1", source, 34230, false));
...
BOOST_AUTO_TEST_CASE(addrman_update_address)
{
    ...
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    CNetAddr source{ResolveIP("252.2.2.2")};
    CAddress addr{CAddress(ResolveService("250.1.1.1", 34230), NODE_NONE)};
    ...
    CAddress addr_diff_port{CAddress(ResolveService("250.1.1.1", 34231), NODE_NONE)};
...
BOOST_AUTO_TEST_CASE(addrman_size)
{
    ...
    const CAddress addr1{ResolveService("250.1.1.1", 34230), NODE_NONE};
    BOOST_CHECK(addrman->Add({addr1}, source));
    BOOST_CHECK(addrman->Good(addr1));
    const CAddress addr2{ResolveService("250.1.1.2", 34230), NODE_NONE};
