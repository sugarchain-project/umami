...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<SugarchainUnit>("SugarchainUnit");
#else
    qRegisterMetaType<SugarchainUnit>("SugarchainUnit");
#endif
}
...
void SugarchainApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // sugarchain: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &SugarchainGUI::handlePaymentRequest);
            connect(window, &SugarchainGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", SugarchainGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    /// 6-7. Parse sugarchain.conf, determine network, switch to network specific
    ...
    // sugarchain: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
