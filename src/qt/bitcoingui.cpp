...
const std::string SugarchainGUI::DEFAULT_UIPLATFORM =
...
SugarchainGUI::SugarchainGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &SugarchainGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &SugarchainGUI::showModalOverlay);
...
void SugarchainGUI::createActions()
{
    ...
    connect(overviewAction, &QAction::triggered, this, &SugarchainGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &SugarchainGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &SugarchainGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Sugarchain Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Sugarchain Transaction from clipboard"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Sugarchain command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &SugarchainGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &SugarchainGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &SugarchainGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &SugarchainGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &SugarchainGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &SugarchainGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &SugarchainGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &SugarchainGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &SugarchainGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &SugarchainGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &SugarchainGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &SugarchainGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &SugarchainGUI::showDebugWindow);
}

void SugarchainGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void SugarchainGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &SugarchainGUI::setCurrentWalletBySelectorIndex);
...
void SugarchainGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &SugarchainGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &SugarchainGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &SugarchainGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &SugarchainGUI::showProgress);
...
#ifdef ENABLE_WALLET
void SugarchainGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void SugarchainGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &SugarchainGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &SugarchainGUI::removeWallet);
...
WalletController* SugarchainGUI::getWalletController()
{
    return m_wallet_controller;
}

void SugarchainGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &SugarchainGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &SugarchainGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &SugarchainGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &SugarchainGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &SugarchainGUI::incomingTransaction);
    connect(this, &SugarchainGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void SugarchainGUI::removeWallet(WalletModel* walletModel)
{
...
void SugarchainGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void SugarchainGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void SugarchainGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SugarchainGUI::setWalletActionsEnabled(bool enabled)
{
...
void SugarchainGUI::createTrayIcon()
{
...
void SugarchainGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &SugarchainGUI::toggleHidden);
...
void SugarchainGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void SugarchainGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void SugarchainGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void SugarchainGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void SugarchainGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void SugarchainGUI::openClicked()
{
...
void SugarchainGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void SugarchainGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void SugarchainGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void SugarchainGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void SugarchainGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SugarchainGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void SugarchainGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void SugarchainGUI::updateNetworkState()
{
...
void SugarchainGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void SugarchainGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void SugarchainGUI::updateHeadersSyncProgressLabel()
{
...
void SugarchainGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void SugarchainGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &SugarchainGUI::quitRequested);
...
void SugarchainGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void SugarchainGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void SugarchainGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &SugarchainGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &SugarchainGUI::show);
                e->ignore();
            }
        }
    }
...
void SugarchainGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SugarchainGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SugarchainGUI::incomingTransaction(const QString& date, SugarchainUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(SugarchainUnits::formatWithUnit(unit, amount, true));
...
void SugarchainGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SugarchainGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SugarchainGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SugarchainGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SugarchainGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void SugarchainGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void SugarchainGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void SugarchainGUI::updateProxyIcon()
{
    std::string ip_port;
...
void SugarchainGUI::updateWindowTitle()
{
...
void SugarchainGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SugarchainGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SugarchainGUI::detectShutdown()
{
...
void SugarchainGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void SugarchainGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(SugarchainGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = SugarchainGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void SugarchainGUI::subscribeToCoreSignals()
{
...
void SugarchainGUI::unsubscribeFromCoreSignals()
{
...
bool SugarchainGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<SugarchainUnit> units = SugarchainUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const SugarchainUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, SugarchainUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const SugarchainUnit u : SugarchainUnits::availableUnits()) {
        menu->addAction(SugarchainUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(SugarchainUnit newUnits)
{
    setText(SugarchainUnits::longName(newUnits));
}
