...
#include <key_io.h>
#include <qt/sugarchainamountfield.h>
                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<SugarchainAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = SugarchainUnits::formatWithUnit(unit, expected_balance, false, SugarchainUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_sugarchain-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_sugarchain-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_sugarchain-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    SugarchainAmountField* amountInput = receiveCoinsDialog.findChild<SugarchainAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("sugarchain:"), 2);
