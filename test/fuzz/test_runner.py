...
def get_fuzz_env(*, target, source_dir):
    return {
        ...
        "ASAN_OPTIONS": "symbolize=0:detect_stack_use_after_return=1:check_initialization_order=1:strict_init_order=1",  # symbolizer disabled due to https://github.com/google/sanitizers/issues/1364#issuecomment-761072085
    }
...
def main():
    ...
    logging.basicConfig(
        format="%(message)s",
        level=int(args.loglevel)
        if args.loglevel.isdigit()
    ...
    logging.debug(
        "{} fuzz target(s) found: {}".format(
            len(test_list_all), " ".join(sorted(test_list_all))
        )
    )
    ...
    if args.exclude:
        for excluded_target in args.exclude.split(","):
            if excluded_target not in test_list_selection:
                logging.error(
                    'Target "{}" not found in current target list.'.format(
                        excluded_target
                    )
                )
                continue
    ...
    if not args.generate:
        ...
        for t in test_list_selection:
            ...
            if (
                ...
                or len(os.listdir(corpus_path)) == 0
            ):
        ...
        if test_list_missing_corpus:
            ...
            logging.info(
                "Please consider adding a fuzz corpus at https://github.com/sugarchain-core/qa-assets"
            )
...
def merge_inputs(
    *, fuzz_pool, corpus, test_list, src_dir, build_dir, merge_dir
    ...
    logging.info(
        "Merge the inputs from the passed dir into the corpus_dir. Passed dir {}".format(
            merge_dir
        )
    )
    jobs = []
