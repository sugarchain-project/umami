...
def check_fileopens():
    ...
    try:
        fileopens = check_output(
            ["git", "grep", r" open(", "--", "*.py"] + get_exclude_args(),
            text=True,
            encoding="utf8",
        ).splitlines()
    ...
    filtered_fileopens = [
        fileopen
        for fileopen in fileopens
        if not re.search(
            r"encoding=.(ascii|utf8|utf-8).|open\([^,]*, ['\"][^'\"]*b[^'\"]*['\"]",
            fileopen,
        )
    ]
...
def check_checked_outputs():
    ...
    try:
        checked_outputs = check_output(
            ["git", "grep", "check_output(", "--", "*.py"] + get_exclude_args(),
            text=True,
            encoding="utf8",
        ).splitlines()
    except CalledProcessError as e:
    ...
    filtered_checked_outputs = [
        checked_output
        for checked_output in checked_outputs
        if re.search(r"text=True", checked_output)
        and not re.search(r"encoding=.(ascii|utf8|utf-8).", checked_output)
    ]
...
def main():
    ...
    if nonexplicit_utf8_checked_outputs:
        if nonexplicit_utf8_fileopens:
            print("\n")
        print(
            "Python's check_output(...) seems to be used to get program outputs without explicitly specifying encoding='utf8':\n"
        )
