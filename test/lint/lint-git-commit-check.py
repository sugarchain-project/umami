...
def main():
    ...
    commit_hashes = check_output(
        ["git", "log", commit_range, "--format=%H"], text=True, encoding="utf8"
    ).splitlines()
    ...
    for hash in commit_hashes:
        ...
        if len(commit_info) >= 2:
            if commit_info[1]:
                print(
                    f"The subject line of commit hash {hash} is followed by a non-empty line. Subject lines should always be followed by a blank line."
                )
                exit_code = 1
