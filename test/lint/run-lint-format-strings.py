...
FALSE_POSITIVES = [
    ...
    (
        "src/netbase.cpp",
        "LogConnectFailure(bool manual_connection, const char* fmt, const Args&... args)",
    ),
    (
...
def parse_function_calls(function_name, source_code):
    ...
    """
    assert (
        type(function_name) is str
        and type(source_code) is str
        and function_name
    )
    lines = [
        re.sub("// .*", " ", line).strip()
...
def main():
    ...
    parser.add_argument(
        "--skip-arguments",
        type=int,
        help="number of arguments before the format string "
        "argument (e.g. 1 in the case of fprintf)",
        default=0,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "file", nargs="*", help="C++ source code file (e.g. foo.cpp)"
    )
    ...
    for filename in args.file:
        with open(filename, "r", encoding="utf-8") as f:
            for function_call_str in parse_function_calls(
                ...
                format_str = parse_string_content(
                    parts[1 + args.skip_arguments]
                )
