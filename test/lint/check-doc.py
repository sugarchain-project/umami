...
SET_DOC_OPTIONAL = set(
    ["-h", "-help", "-dbcrashratio", "-forcecompactdb", "-zapwallettxes"]
)
...
def lint_missing_argument_documentation():
    ...
    args_docd = set(re.findall(re.compile(REGEX_DOC), docd)).union(
        SET_DOC_OPTIONAL
    )
    ...
    assert 0 == len(
        args_need_doc
    ), "Please document the following arguments: {}".format(args_need_doc)


def lint_missing_hidden_wallet_args():
    wallet_args = (
        check_output(CMD_GREP_WALLET_ARGS, shell=True).decode("utf8").strip()
    )
    wallet_hidden_args = (
        check_output(CMD_GREP_WALLET_HIDDEN_ARGS, shell=True)
        .decode("utf8")
        .strip()
    )
    ...
    wallet_hidden_args = set(
        re.findall(re.compile(r'    "([^"=]+)'), wallet_hidden_args)
    )
    ...
    if hidden_missing:
        assert (
            0
        ), "Please add {} to the hidden args in DummyWalletInit::AddWalletOptions".format(
            hidden_missing
        )
