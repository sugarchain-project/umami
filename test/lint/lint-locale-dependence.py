...
#
# Be aware that sugarchaind and sugarchain-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in sugarchaind.
#
...
# C locale.
#
# In contrast, sugarchaind does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where sugarchaind is guaranteed to be running
# with the classic locale ("C") whereas the locale of sugarchain-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in sugarchaind but
# "1,230000" in sugarchain-qt.
#
...
def find_locale_dependent_function_uses():
    ...
    exclude_args = [
        ":(exclude)" + excl for excl in REGEXP_EXTERNAL_DEPENDENCIES_EXCLUSIONS
    ]
    git_grep_command = [
        "git",
        "grep",
        "-E",
        "[^a-zA-Z0-9_\\`'\"<>]("
        + regexp_locale_dependent_functions
        + ")(_r|_s)?[^a-zA-Z0-9_\\`'\"<>]",
        "--",
        "*.cpp",
        "*.h",
    ...
    try:
        git_grep_output = check_output(
            git_grep_command, text=True, encoding="utf8"
        ).splitlines()
