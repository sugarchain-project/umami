            ...
            if self.test_addr_contents:
                ...
                if not 8333 <= addr.port < 8343:
                    raise AssertionError(
                        "Invalid addr.port of {} (8333-8342 expected)".format(
                            addr.port
                        )
                    )
...
class AddrTest(SugarchainTestFramework):
    ...
    def setup_addr_msg(self, num, sequential_ips=True):
        ...
        for i in range(num):
            ...
            if sequential_ips:
                assert (
                    self.counter < 256**2
                )  # Don't allow the returned ip addresses to wrap.
    ...
    def relay_tests(self):
        ...
        assert_equal(
            total_ipv4_received, num_ipv4_addrs * ipv4_branching_factor
        )
        ...
        self.send_addr_msg(
            inbound_peer, msg3, [full_outbound_peer, block_relay_peer]
        )
    ...
    def inbound_blackhole_tests(self):
        ...
        assert_greater_than(
            self.sum_addr_messages(peerinfo[1]["bytesrecv_per_msg"]), 0
        )
        # And that peer received addresses
        assert_equal(
            receiver_peer.num_ipv4_received - initial_addrs_received, 2
        )

        # Confirm node has not received addr-related messages from blackhole peer
        assert_equal(
            self.sum_addr_messages(peerinfo[2]["bytesrecv_per_msg"]), 0
        )
    ...
    def send_addrs_and_test_rate_limiting(
        ...
        self.log.debug(
            f"addrs_processed = {addrs_processed}, addrs_rate_limited = {addrs_rate_limited}"
        )
    ...
    def destination_rotates_more_than_once_over_several_days_test(self):
        ...
        assert_greater_than(
            len(nodes_received_addr), ADDR_DESTINATIONS_THRESHOLD
        )
        self.nodes[0].disconnect_p2ps()
