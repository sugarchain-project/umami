...
class MempoolWtxidTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        raw_parent = node.signrawtransactionwithkey(
            hexstring=parent.serialize().hex(), privkeys=privkeys
        )["hex"]
        parent_txid = node.sendrawtransaction(
            hexstring=raw_parent, maxfeerate=0
        )
        ...
        assert_equal(
            node.testmempoolaccept([child_one.serialize().hex()]),
            [
                {
                    "txid": child_one_txid,
