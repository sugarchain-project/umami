...
class DustRelayFeeTest(SugarchainTestFramework):
    ...
    def test_dust_output(
        self,
        node: TestNode,
        dust_relay_fee: Decimal,
        ...
        self.log.info(
            f"-> Test {type_desc} output (size {len(output_script)}, limit {dust_threshold})"
        )

        # amount right on the dust threshold should pass
        tx = self.wallet.create_self_transfer()["tx"]
        tx.vout.append(
            CTxOut(nValue=dust_threshold, scriptPubKey=output_script)
        )
    ...
    def run_test(self):
        ...
        output_scripts = (
            ...
            (
                program_to_witness_script(2, b"\x66" * 2),
                "P2?? (future witness version 2)",
            ),
            (
                program_to_witness_script(16, b"\x77" * 40),
                "P2?? (future witness version 16)",
            ),
            # largest possible output script considered standard
            (
                keys_to_multisig_script([uncompressed_pubkey] * 3),
                "bare multisig (m-of-3)",
            ),
            (
                CScript([OP_RETURN, b"superimportanthash"]),
                "null data (OP_RETURN)",
            ),
        )

        # test default (no parameter), disabled (=0) and a bunch of arbitrary dust fee rates [sat/kvB]
        for dustfee_sat_kvb in (
            DUST_RELAY_TX_FEE,
            0,
            1,
            66,
            ...
            if dustfee_sat_kvb == DUST_RELAY_TX_FEE:
                self.log.info(
                    f"Test default dust limit setting ({dustfee_sat_kvb} sat/kvB)..."
                )
            else:
                dust_parameter = f"-dustrelayfee={dustfee_sugar_kvb:.8f}"
                self.log.info(
                    f"Test dust limit setting {dust_parameter} ({dustfee_sat_kvb} sat/kvB)..."
                )
                self.restart_node(0, extra_args=[dust_parameter])

            for output_script, description in output_scripts:
                self.test_dust_output(
                    self.nodes[0], dustfee_sugar_kvb, output_script, description
                )
