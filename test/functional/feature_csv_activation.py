...
class BIP68_112_113Test(SugarchainTestFramework):
    ...
    def create_self_transfer_from_utxo(self, input_tx):
        utxo = self.miniwallet.get_utxo(
            txid=input_tx.rehash(), mark_as_spent=False
        )
    ...
    def create_bip112emptystack(self, input, txversion):
        ...
        self.miniwallet.sign_tx(tx)
        tx.vin[0].scriptSig = CScript(
            [OP_CHECKSEQUENCEVERIFY] + list(CScript(tx.vin[0].scriptSig))
        )
        tx.rehash()
        return tx
    ...
    def create_bip68txs(self, bip68inputs, txversion, locktime_delta=0):
        ...
        for i, (sdf, srhb, stf, srlb) in enumerate(
            product(*[[True, False]] * 4)
        ):
    ...
    def create_bip112txs(
        ...
        for i, (sdf, srhb, stf, srlb) in enumerate(
            ...
            if varyOP_CSV:
                ...
                tx.vin[0].scriptSig = CScript(
                    [BASE_RELATIVE_LOCKTIME, OP_CHECKSEQUENCEVERIFY, OP_DROP]
                    + list(CScript(tx.vin[0].scriptSig))
                )
            tx.rehash()
            txs.append({"tx": tx, "sdf": sdf, "stf": stf})
    ...
    def send_blocks(self, blocks, success=True, reject_reason=None):
        ...
        self.helper_peer.send_blocks_and_test(
            blocks, self.nodes[0], success=success, reject_reason=reject_reason
        )

    def run_test(self):
        self.helper_peer = self.nodes[0].add_p2p_connection(P2PDataStore())
        self.miniwallet = MiniWallet(
            self.nodes[0], mode=MiniWalletMode.RAW_P2PK
        )
        ...
        test_blocks = self.generate_blocks(
            CSV_ACTIVATION_HEIGHT - 5 - COINBASE_BLOCK_COUNT
        )
        ...
        inputblockhash = self.generate(self.nodes[0], 1)[
            0
        ]  # 1 block generated for inputs to be in chain at height 431
        self.nodes[0].setmocktime(0)
        self.tip = int(inputblockhash, 16)
        self.tipheight += 1
        self.last_block_time += 600
        assert_equal(
            len(self.nodes[0].getblock(inputblockhash, True)["tx"]),
            TESTING_TX_COUNT + 1,
        )
        ...
        self.log.info(
            f"Height = {self.tipheight}, CSV not yet active (will activate for block {CSV_ACTIVATION_HEIGHT}, not {CSV_ACTIVATION_HEIGHT - 1})"
        )
        assert not softfork_active(self.nodes[0], "csv")
        ...
        bip112tx_emptystack_v1 = self.create_bip112emptystack(
            bip112emptystackinput, 1
        )
        bip112tx_emptystack_v2 = self.create_bip112emptystack(
            bip112emptystackinput, 2
        )
        ...
        bip113tx_v1.nLockTime = (
            self.last_block_time - 600 * 5
        ...
        for bip113tx in [bip113tx_v1, bip113tx_v2]:
            self.send_blocks(
                [self.create_test_block([bip113tx])],
                success=False,
        ...
        self.send_blocks(
            [self.create_test_block([bip112tx_special_v1])],
            success=False,
            reject_reason="non-mandatory-script-verify-flag (Negative locktime)",
        )
        self.send_blocks(
            [self.create_test_block([bip112tx_emptystack_v1])],
            success=False,
        ...
        fail_txs += [
            tx["tx"] for tx in bip112txs_vary_OP_CSV_v1 if not tx["sdf"]
        ]
        fail_txs += [
        ...
        # -1 OP_CSV tx and (empty stack) OP_CSV tx should fail
        self.send_blocks(
            [self.create_test_block([bip112tx_special_v2])],
            success=False,
            reject_reason="non-mandatory-script-verify-flag (Negative locktime)",
        )
        self.send_blocks(
            [self.create_test_block([bip112tx_emptystack_v2])],
            success=False,
            reject_reason="non-mandatory-script-verify-flag (Operation not valid with the current stack size)",
        )
