...
class MerkleBlockTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        txid_unspent = (
            txid1  # Input was change from txid2, so txid1 should be unspent
        )
        ...
        assert_equal(
            self.nodes[0].verifytxoutproof(
                self.nodes[0].gettxoutproof([txid_unspent])
            ),
            [txid_unspent],
        )
        ...
        assert_equal(
            self.nodes[0].verifytxoutproof(
                self.nodes[1].gettxoutproof([txid_spent])
            ),
            [txid_spent],
        )
