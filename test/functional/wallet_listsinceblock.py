...
class ListSinceBlockTest(SugarchainTestFramework):
    ...
    def test_targetconfirmations(self):
        ...
        assert_equal(
            ...
            self.nodes[0].listsinceblock(blockhash, blockheight + 1)[
                "lastblock"
            ],
        )
        assert_equal(
            self.nodes[0].getblockhash(0),
            self.nodes[0].listsinceblock(blockhash, blockheight + 1000)[
                "lastblock"
            ],
        )
        assert_raises_rpc_error(
    ...
    def test_double_spend(self):
        ...
        1. User 1 receives SUGAR in tx1 from utxo1 in block aa1.
        2. User 2 receives SUGAR in tx2 from utxo1 (same) in block bb1
    ...
    def test_double_send(self):
        ...
        assert_equal(
            tx1["blockheight"],
            self.nodes[0].getblockheader(tx1["blockhash"])["height"],
        )
    ...
    def double_spends_filtered(self):
        ...
        tx_input = dict(
            ...
            **next(u for u in spending_node.listunspent())
        )
    ...
    def test_send_to_self(self):
        ...
        coins = self.nodes[2].listsinceblock(blockhash=block_hash)[
            "transactions"
        ]
        ...
        assert all(
            self.nodes[2].getaddressinfo(c["address"])["ischange"]
            for c in coins
        )
    ...
    def test_op_return(self):
        ...
        op_ret_tx = [
            tx
            for tx in self.nodes[2].listsinceblock(blockhash=block_hash)[
                "transactions"
            ]
            if tx["txid"] == tx_id
        ][0]
