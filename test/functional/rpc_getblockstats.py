...
class GetblockstatsTest(SugarchainTestFramework):
    start_height = 101
    max_stat_pos = 2
    ...
    def get_stats(self):
        return [
            self.nodes[0].getblockstats(hash_or_height=self.start_height + i)
    ...
    def generate_test_data(self, filename):
        ...
        self.nodes[0].sendtoaddress(
            address=address, amount=10, subtractfeefromamount=False
        )
        self.nodes[0].settxfee(amount=0.003)
        self.nodes[0].sendtoaddress(
            address=address, amount=1, subtractfeefromamount=True
        )
    ...
    def run_test(self):
        ...
        assert_equal(
            stats[self.max_stat_pos]["height"],
        ...
        assert_raises_rpc_error(
            -8,
            "Target block height %d after current tip %d" % (tip + 1, tip),
        ...
        for inv_stat in inv_stats:
            assert_raises_rpc_error(
                -8,
                f"Invalid selected statistic '{inv_sel_stat}'",
                self.nodes[0].getblockstats,
                hash_or_height=1,
                stats=inv_stat,
            )
        ...
        assert_raises_rpc_error(
            -8,
            f"Invalid selected statistic 'aaa{inv_sel_stat}'",
            self.nodes[0].getblockstats,
            hash_or_height=1,
            stats=["minfee", f"aaa{inv_sel_stat}"],
        )
