...
class MempoolPackagesTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        if self.is_specified_wallet_compiled():
            self.nodes[0].createwallet(
                "watch_wallet", disable_private_keys=True
            )
        ...
        for i, t in enumerate(chain):
            ...
            if self.is_specified_wallet_compiled():
                ...
                this_unspent = next(
                    utxo_info
                    for utxo_info in wallet_unspent
                    if utxo_info["txid"] == t["txid"]
                )
        ...
        next_hop = self.wallet.create_self_transfer(
            utxo_to_spend=chain[-1]["new_utxo"]
        )["hex"]
        assert_raises_rpc_error(
            -26,
            "too-long-mempool-chain",
            lambda: self.nodes[0].sendrawtransaction(next_hop),
        )
        ...
        for x in reversed(chain):
            ...
            for tx_in in witnesstx["vin"]:
                spending_result = self.nodes[0].gettxspendingprevout(
                    [{"txid": tx_in["txid"], "vout": tx_in["vout"]}]
                )
                assert_equal(
                    spending_result,
                    [
            ...
            for descendant, dinfo in (
                ...
                assert_equal(
                    dinfo["depends"], [chain[chain.index(descendant) - 1]]
                )
                if dinfo["descendantcount"] > 1:
                    assert_equal(
                        dinfo["spentby"], [chain[chain.index(descendant) + 1]]
                    )
                else:
            ...
            for ancestor, ainfo in (
                self.nodes[0].getmempoolancestors(x, True).items()
            ):
                assert_equal(
                    ainfo["spentby"], [chain[chain.index(ancestor) + 1]]
                )
                if ainfo["ancestorcount"] > 1:
                    assert_equal(
                        ainfo["depends"], [chain[chain.index(ancestor) - 1]]
                    )
                else:
        ...
        for x in chain:
            ...
            ancestor_fees += entry["fees"]["base"]
            assert_equal(
                entry["fees"]["ancestor"], ancestor_fees + Decimal("0.00001")
            )
        ...
        for x in reversed(chain):
            ...
            if x == chain[-1]:
                assert_equal(
                    entry["fees"]["modified"],
                    entry["fees"]["base"] + Decimal("0.00002"),
                )
            assert_equal(
                entry["fees"]["descendant"],
        ...
        tx_with_children = self.wallet.send_self_transfer_multi(
            from_node=self.nodes[0], num_outputs=10
        )
        ...
        for _ in range(DEFAULT_DESCENDANT_LIMIT - 1):
            ...
            new_tx = self.wallet.send_self_transfer_multi(
                from_node=self.nodes[0], num_outputs=10, utxos_to_spend=[utxo]
            )
            txid = new_tx["txid"]
            chain.append(txid)
        ...
        self.wait_until(
            lambda: len(self.nodes[1].getrawmempool())
            == CUSTOM_ANCESTOR_LIMIT + 1 + CUSTOM_DESCENDANT_LIMIT,
            timeout=10,
        )
        mempool0 = self.nodes[0].getrawmempool(False)
        mempool1 = self.nodes[1].getrawmempool(False)
        ...
        tx0 = self.wallet.send_self_transfer_multi(
            from_node=self.nodes[0], num_outputs=2
        )

        # Create tx1
        tx1 = self.wallet.send_self_transfer(
            from_node=self.nodes[0], utxo_to_spend=tx0["new_utxos"][0]
        )

        # Create tx2-7
        tx7 = self.wallet.send_self_transfer_chain(
            from_node=self.nodes[0],
            utxo_to_spend=tx0["new_utxos"][1],
            chain_length=6,
        )[-1]
        ...
        self.wallet.send_self_transfer_multi(
            from_node=self.nodes[0],
            utxos_to_spend=[tx1["new_utxo"], tx7["new_utxo"]],
            fee_per_output=40000,
        )
        self.sync_mempools()
