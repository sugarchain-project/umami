...
class ToolWalletTest(SugarchainTestFramework):
    ...
    def sugarchain_wallet_process(self, *args):
        binary = (
        ...
        default_args = [
            "-datadir={}".format(self.nodes[0].datadir),
            "-chain=%s" % self.chain,
        ]
    ...
    def assert_raises_tool_error(self, error, *args):
        p = self.sugarchain_wallet_process(*args)
    ...
    def get_expected_info_output(
        self, name="", transactions=0, keypool=2, address=0, imported_privs=0
    ):
        ...
        if self.options.descriptors:
            ...
            """
                % (
                    wallet_name,
                    keypool * output_types,
                    transactions,
                    imported_privs * 3 + address,
                )
            )
        else:
            output_types = (
                3  # p2pkh, p2sh, segwit. Legacy wallets do not support bech32m.
            )
            return textwrap.dedent(
                """\
                Wallet info
            ...
            """
                % (
                    ...
                    keypool,
                    transactions,
                    (address + imported_privs) * output_types,
                )
            )
    ...
    def assert_is_bdb(self, filename):
        with open(filename, "rb") as f:
            ...
            file_magic = f.read(4)
            assert (
                file_magic == b"\x00\x05\x31\x62"
                or file_magic == b"\x62\x31\x05\x00"
            )
    ...
    def do_tool_createfromdump(self, wallet_name, dumpfile, file_format=None):
        ...
        rt_dumppath = os.path.join(
            self.nodes[0].datadir, "rt-{}.dump".format(wallet_name)
        )
        ...
        assert os.path.isdir(
            os.path.join(self.nodes[0].datadir, "regtest/wallets", wallet_name)
        )

        self.assert_tool_output(
            "The dumpfile may contain private keys. To ensure the safety of your Sugarchain, do not share the dumpfile.\n",
            "-wallet={}".format(wallet_name),
            "-dumpfile={}".format(rt_dumppath),
            "dump",
        )

        rt_dump_data = self.read_dump(rt_dumppath)
        wallet_dat = os.path.join(
            self.nodes[0].datadir, "regtest/wallets/", wallet_name, "wallet.dat"
        )
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        self.assert_raises_tool_error(
            "Error parsing command line arguments: Invalid command 'foo'", "foo"
        )
        # `sugarchain-wallet help` raises an error. Use `sugarchain-wallet -help`.
        self.assert_raises_tool_error(
            "Error parsing command line arguments: Invalid command 'help'",
            "help",
        )
        self.assert_raises_tool_error(
            "Error: Additional arguments provided (create). Methods do not take arguments. Please refer to `-help`.",
            "info",
            "create",
        )
        self.assert_raises_tool_error(
        ...
        self.assert_raises_tool_error(
            "No method provided. Run `sugarchain-wallet -help` for valid methods."
        )
        self.assert_raises_tool_error(
            "Wallet name must be provided when creating a new wallet.", "create"
        )
        locked_dir = os.path.join(
            self.options.tmpdir, "node0", "regtest", "wallets"
        )
        error = 'Error initializing wallet database environment "{}"!'.format(
            locked_dir
        )
        if self.options.descriptors:
        ...
        self.assert_raises_tool_error(
            "Failed to load database path '{}'. Path does not exist.".format(
                path
            ),
    ...
    def test_tool_wallet_info(self):
        ...
        self.log.debug(
            "Wallet file timestamp before calling info: {}".format(
                timestamp_before
            )
        )
        out = self.get_expected_info_output(imported_privs=1)
        self.assert_tool_output(
            out, "-wallet=" + self.default_wallet_name, "info"
        )
        timestamp_after = self.wallet_timestamp()
        self.log.debug(
            "Wallet file timestamp after calling info: {}".format(
                timestamp_after
            )
        )
        ...
        assert self.wallet_permissions() in [
            "600",
            "666",
    ...
    def test_tool_wallet_info_after_transaction(self):
        ...
        self.log.info(
            "Calling wallet tool info after generating a transaction, testing output"
        )
        shasum_before = self.wallet_shasum()
        timestamp_before = self.wallet_timestamp()
        self.log.debug(
    ...
    def test_tool_wallet_create_on_existing_wallet(self):
        self.log.info(
            "Calling wallet tool create on an existing wallet, testing output"
        )
        shasum_before = self.wallet_shasum()
        timestamp_before = self.wallet_timestamp()
        self.log.debug(
            "Wallet file timestamp before calling create: {}".format(
                timestamp_before
            )
        )
        out = "Topping up keypool...\n" + self.get_expected_info_output(
            name="foo", keypool=2000
        )
        self.assert_tool_output(out, "-wallet=foo", "create")
        shasum_after = self.wallet_shasum()
        timestamp_after = self.wallet_timestamp()
        self.log.debug(
            "Wallet file timestamp after calling create: {}".format(
                timestamp_after
            )
        )
        self.log_wallet_timestamp_comparison(timestamp_before, timestamp_after)
    ...
    def test_getwalletinfo_on_different_wallet(self):
        ...
        timestamp_before = self.wallet_timestamp()
        self.log.debug(
            "Wallet file timestamp before calling getwalletinfo: {}".format(
                timestamp_before
            )
        )
        ...
        timestamp_after = self.wallet_timestamp()
        self.log.debug(
            "Wallet file timestamp after calling getwalletinfo: {}".format(
                timestamp_after
            )
        )
    ...
    def test_salvage(self):
        # TODO: Check salvage actually salvages and doesn't break things. https://github.com/sugarchain/sugarchain/issues/7463
    ...
    def test_dump_createfromdump(self):
        ...
        file_format = (
            self.nodes[0].get_wallet_rpc("todump").getwalletinfo()["format"]
        )
        ...
        self.assert_raises_tool_error(
            "File {} already exists. If you are sure this is what you want, move it out of the way first.".format(
                wallet_dump
            ),
            "-wallet=todump2",
            "-dumpfile={}".format(wallet_dump),
        ...
        self.assert_raises_tool_error(
            'Unknown wallet file format "notaformat" provided. Please provide one of "bdb" or "sqlite".',
            "-wallet=todump",
            "-format=notaformat",
            "-dumpfile={}".format(wallet_dump),
            "createfromdump",
        ...
        self.assert_raises_tool_error(
            "Dump file {} does not exist.".format(non_exist_dump),
            "-wallet=todump",
            "-dumpfile={}".format(non_exist_dump),
            "createfromdump",
        )
        wallet_path = os.path.join(
            self.nodes[0].datadir, "regtest", "wallets", "todump2"
        )
        self.assert_raises_tool_error(
            "Failed to create database path '{}'. Database already exists.".format(
                wallet_path
            ),
            "-wallet=todump2",
            "-dumpfile={}".format(wallet_dump),
        ...
        bad_ver_wallet_dump = os.path.join(
            self.nodes[0].datadir, "wallet-bad_ver1.dump"
        )
        dump_data["BITCOIN_CORE_WALLET_DUMP"] = "0"
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error(
            "Error: Dumpfile version is not supported. This version of sugarchain-wallet only supports version 1 dumpfiles. Got dumpfile with version 0",
            "-wallet=badload",
            "-dumpfile={}".format(bad_ver_wallet_dump),
            "createfromdump",
        )
        assert not os.path.isdir(
            os.path.join(self.nodes[0].datadir, "regtest/wallets", "badload")
        )
        bad_ver_wallet_dump = os.path.join(
            self.nodes[0].datadir, "wallet-bad_ver2.dump"
        )
        dump_data["BITCOIN_CORE_WALLET_DUMP"] = "2"
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error(
            "Error: Dumpfile version is not supported. This version of sugarchain-wallet only supports version 1 dumpfiles. Got dumpfile with version 2",
            "-wallet=badload",
            "-dumpfile={}".format(bad_ver_wallet_dump),
            "createfromdump",
        )
        ...
        bad_magic_wallet_dump = os.path.join(
            self.nodes[0].datadir, "wallet-bad_magic.dump"
        )
        ...
        self.assert_raises_tool_error(
            ...
            "-wallet=badload",
            "-dumpfile={}".format(bad_magic_wallet_dump),
            "createfromdump",
        )
        assert not os.path.isdir(
            os.path.join(self.nodes[0].datadir, "regtest/wallets", "badload")
        ...
        bad_sum_wallet_dump = os.path.join(
            self.nodes[0].datadir, "wallet-bad_sum1.dump"
        )
        ...
        self.assert_raises_tool_error(
            "Error: Dumpfile checksum does not match. Computed {}, expected {}".format(
                checksum, "1" * 64
            ),
            "-wallet=bad",
            "-dumpfile={}".format(bad_sum_wallet_dump),
            "createfromdump",
        )
        assert not os.path.isdir(
            os.path.join(self.nodes[0].datadir, "regtest/wallets", "badload")
        ...
        bad_sum_wallet_dump = os.path.join(
            self.nodes[0].datadir, "wallet-bad_sum2.dump"
        )
        ...
        bad_sum_wallet_dump = os.path.join(
            self.nodes[0].datadir, "wallet-bad_sum3.dump"
        )
