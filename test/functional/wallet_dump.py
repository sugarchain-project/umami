...
class WalletDumpTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        dump_best_block_1 = (
            "# * Best block at time of backup was {} ({}),".format(
        ...
        assert_equal(
            dump_best_block_1,
            next(c for c in found_comments if c.startswith("# * Best block")),
        )
        assert_equal(
            dump_best_block_2,
            next(c for c in found_comments if c.startswith("#   mined on")),
        )
        assert_equal(
            found_legacy_addr, test_addr_count
        )  # all keys must be in the dump
        assert_equal(
            found_p2sh_segwit_addr, test_addr_count
        )  # all keys must be in the dump
        assert_equal(
            found_bech32_addr, test_addr_count
        )  # all keys must be in the dump
        ...
        (
            ...
            wallet_enc_dump, addrs, [multisig_addr], hd_master_addr_unenc
        )
        assert (
            "# End of dump" in found_comments
        )  # Check that file is not corrupt
        ...
        with self.nodes[0].assert_debug_log(
            ["Flushing wallet.dat"], timeout=20
        ):
