...
class SignRawTransactionWithKeyTest(SugarchainTestFramework):
    ...
    def send_to_address(self, addr, amount):
        input = {
            "txid": self.nodes[0].getblock(self.block_hash[self.blk_idx])["tx"][
                0
            ],
            "vout": 0,
        }
        ...
        txid = self.nodes[0].sendrawtransaction(
            self.nodes[0].signrawtransactionwithkey(
                rawtx, [self.nodes[0].get_deterministic_priv_key().key]
            )["hex"],
            0,
        )
        return txid
    ...
    def witness_script_test(self):
        ...
        unspent_output = self.nodes[1].scantxoutset(
            "start", [p2sh_p2wsh_address["descriptor"]]
        )["unspents"][0]
