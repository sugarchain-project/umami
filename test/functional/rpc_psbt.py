...
class PSBTTest(SugarchainTestFramework):
    ...
    def set_test_params(self):
        ...
        self.extra_args = [
            [
                ...
                "-addresstype=bech32",
                "-changetype=bech32",
            ],  # TODO: Remove address type restrictions once taproot has psbt extensions
    ...
    def test_utxo_conversion(self):
        self.log.info(
            "Check that non-witness UTXOs are removed for segwit v1+ inputs"
        )
        ...
        online_node.createwallet(
            wallet_name="wonline", disable_private_keys=True
        )
        ...
        self.generate(
            mining_node,
            nblocks=1,
            sync_fun=lambda: self.sync_all([online_node, mining_node]),
        )

        # Construct an unsigned PSBT on the online node
        utxos = wonline.listunspent(addresses=[offline_addr])
        raw = wonline.createrawtransaction(
            [{"txid": utxos[0]["txid"], "vout": utxos[0]["vout"]}],
            [{online_addr: 0.9999}],
        )
        psbt = wonline.walletprocesspsbt(online_node.converttopsbt(raw))["psbt"]
        assert (
            not "not_witness_utxo" in mining_node.decodepsbt(psbt)["inputs"][0]
        )
        ...
        assert (
            "non_witness_utxo"
            in mining_node.decodepsbt(psbt_new.to_base64())["inputs"][0]
        )
        ...
        assert (
            not "non_witness_utxo"
            in mining_node.decodepsbt(signed_psbt)["inputs"][0]
        )

        # Make sure we can mine the resulting transaction
        txid = mining_node.sendrawtransaction(
            mining_node.finalizepsbt(signed_psbt)["hex"]
        )
        self.generate(
            mining_node,
            nblocks=1,
    ...
    def test_input_confs_control(self):
        ...
        psbtx1 = wallet.walletcreatefundedpsbt(
            [], {target_address: 0.1}, 0, {"fee_rate": 1, "maxconf": 0}
        )["psbt"]
        ...
        self.log.info(
            "Fail to craft a new PSBT that sends more funds with add_inputs = False"
        )
        assert_raises_rpc_error(
            -4,
        ...
        self.log.info(
            "Fail to broadcast a new PSBT with maxconf 0 due to BIP125 rules to verify it actually chose unconfirmed outputs"
        )
        psbt_invalid = wallet.walletcreatefundedpsbt(
            [{"txid": utxo1["txid"], "vout": utxo1["vout"]}],
            {target_address: 1},
            0,
            {"add_inputs": True, "maxconf": 0, "fee_rate": 10},
        )["psbt"]
        ...
        self.log.info(
            "Craft a replacement adding inputs with highest confs possible"
        )
        psbtx2 = wallet.walletcreatefundedpsbt(
            [{"txid": utxo1["txid"], "vout": utxo1["vout"]}],
            {target_address: 1},
            0,
            {"add_inputs": True, "minconf": 2, "fee_rate": 10},
        )["psbt"]
    ...
    def assert_change_type(self, psbtx, expected_type):
        ...
        assert_equal(
            decoded_psbt["tx"]["vout"][changepos]["scriptPubKey"]["type"],
            expected_type,
        )

    def run_test(self):
        # Create and fund a raw tx for sending 10 SUGAR
        ...
        assert_raises_rpc_error(
            ...
            self.nodes[0].walletprocesspsbt,
            psbtx,
        )
        ...
        signed_tx = self.nodes[0].walletprocesspsbt(psbt=psbtx, finalize=False)[
            "psbt"
        ]
        finalized_tx = self.nodes[0].walletprocesspsbt(
            psbt=psbtx, finalize=True
        )["psbt"]
        ...
        pubkey0 = self.nodes[0].getaddressinfo(self.nodes[0].getnewaddress())[
            "pubkey"
        ]
        pubkey1 = self.nodes[1].getaddressinfo(self.nodes[1].getnewaddress())[
            "pubkey"
        ]
        pubkey2 = self.nodes[2].getaddressinfo(self.nodes[2].getnewaddress())[
            "pubkey"
        ]
        ...
        p2sh = wmulti.addmultisigaddress(
            2, [pubkey0, pubkey1, pubkey2], "", "legacy"
        )["address"]
        p2wsh = wmulti.addmultisigaddress(
            2, [pubkey0, pubkey1, pubkey2], "", "bech32"
        )["address"]
        p2sh_p2wsh = wmulti.addmultisigaddress(
            2, [pubkey0, pubkey1, pubkey2], "", "p2sh-segwit"
        )["address"]
        ...
        inputs = [
            {"txid": txid, "vout": p2wpkh_pos},
            {"txid": txid, "vout": p2sh_p2wpkh_pos},
        ...
        self.log.info(
            "Test walletcreatefundedpsbt fee rate of 10000 sat/vB and 0.1 SUGAR/kvB produces a total fee at or slightly below -maxtxfee (~0.05290000)"
        )
        res1 = self.nodes[1].walletcreatefundedpsbt(
            inputs, outputs, 0, {"fee_rate": 10000, "add_inputs": True}
        )
        assert_approx(res1["fee"], 0.055, 0.005)
        res2 = self.nodes[1].walletcreatefundedpsbt(
            inputs, outputs, 0, {"feeRate": "0.1", "add_inputs": True}
        )
        ...
        res3 = self.nodes[1].walletcreatefundedpsbt(
            inputs, outputs, 0, {"fee_rate": "0.999", "add_inputs": True}
        )
        assert_approx(res3["fee"], 0.00000381, 0.0000001)
        res4 = self.nodes[1].walletcreatefundedpsbt(
            inputs, outputs, 0, {"feeRate": 0.00000999, "add_inputs": True}
        )
        ...
        for param, zero_value in product(
            ...
            assert_equal(
                ...
                self.nodes[1].walletcreatefundedpsbt(
                    inputs, outputs, 0, {param: zero_value, "add_inputs": True}
                )["fee"],
            )
        ...
        for bool_add, outputs_array in {
            True: outputs,
            False: [{self.nodes[1].getnewaddress(): 1}],
        }.items():
            ...
            assert_raises_rpc_error(
                ...
                outputs_array,
                0,
                {"fee_rate": 1000000, "add_inputs": bool_add},
            )
            assert_raises_rpc_error(
                -4,
                msg,
        ...
        psbtx = wmulti.walletcreatefundedpsbt(
            inputs=[
                {"txid": txid, "vout": p2wsh_pos},
                {"txid": txid, "vout": p2sh_pos},
                {"txid": txid, "vout": p2sh_p2wsh_pos},
            ],
            outputs={self.nodes[1].getnewaddress(): 29.99},
            options={"changeAddress": self.nodes[1].getrawchangeaddress()},
        )["psbt"]
        ...
        psbt2 = self.nodes[2].walletprocesspsbt(psbt_orig, False, "ALL", False)[
            "psbt"
        ]
        ...
        psbtx_info = self.nodes[0].walletcreatefundedpsbt(
            ...
            block_height + 2,
            {"replaceable": False, "add_inputs": True},
            False,
        )
        decoded_psbt = self.nodes[0].decodepsbt(psbtx_info["psbt"])
        for tx_in, psbt_in in zip(
            decoded_psbt["tx"]["vin"], decoded_psbt["inputs"]
        ):
        ...
        psbtx_info = self.nodes[1].walletcreatefundedpsbt(
            [{"txid": unspent1["txid"], "vout": unspent1["vout"]}],
        ...
        assert_raises_rpc_error(
            -8,
            "both change address and address type options",
            self.nodes[0].walletcreatefundedpsbt,
            [],
            [small_output],
            0,
        ...
        for invalid in invalid_with_msgs:
            psbt, msg = invalid
            assert_raises_rpc_error(
                -22, f"TX decode failed {msg}", self.nodes[0].decodepsbt, psbt
            )
        ...
        for i, signer in enumerate(signers):
            ...
            signed_tx = wrpc.walletprocesspsbt(signer["psbt"], True, "ALL")[
                "psbt"
            ]
        ...
        decoded = self.nodes[1].decodepsbt(updated)
        test_psbt_input_keys(
            decoded["inputs"][0], ["witness_utxo", "bip32_derivs"]
        )
        test_psbt_input_keys(decoded["inputs"][1], [])
        test_psbt_input_keys(
            decoded["inputs"][2],
        ...
        psbt2_decoded = self.nodes[0].decodepsbt(psbt2)
        assert (
            "final_scriptwitness" in psbt2_decoded["inputs"][0]
            and "final_scriptSig" in psbt2_decoded["inputs"][0]
        )
        ...
        assert (
            ...
            and "final_scriptwitness" not in joined_decoded["inputs"][3]
        ...
        updated = self.nodes[1].walletprocesspsbt(psbt, False, "ALL", True)[
            "psbt"
        ]
        ...
        assert (
            ...
            and analyzed["inputs"][0]["missing"]["signatures"][0]
            == addrinfo["embedded"]["witness_program"]
        )

        # Check fee and size things
        assert (
            analyzed["fee"] == Decimal("0.001")
            and analyzed["estimated_vsize"] == 134
            and analyzed["estimated_feerate"] == Decimal("0.00746268")
        )
        ...
        assert_equal(analysis["next"], "creator")
        assert_equal(
            analysis["error"], "PSBT is not valid. Input 0 has invalid value"
        )
        ...
        assert_equal(analysis["next"], "creator")
        assert_equal(
            analysis["error"], "PSBT is not valid. Output amount invalid"
        )
            ...
            0,
            {
                ...
                "solving_data": {
                    ...
                    "scripts": [
                        ...
                        addr_info["embedded"]["embedded"]["scriptPubKey"],
                    ],
                },
            },
        )
        ...
        for i, txin in enumerate(dec["tx"]["vin"]):
            if (
                txin["txid"] == ext_utxo["txid"]
                and txin["vout"] == ext_utxo["vout"]
            ):
                input_idx = i
                break
        ...
        len_scriptsig = (
            len(psbt_in["final_scriptSig"]["hex"]) // 2
            if "final_scriptSig" in psbt_in
            else 0
        )
        ...
        len_scriptwitness = (
            (
                sum(
                    [
                        (len(x) // 2) + len(ser_compact_size(len(x) // 2))
                        for x in psbt_in["final_scriptwitness"]
                    ]
                )
                + len(psbt_in["final_scriptwitness"])
                + 1
            )
            if "final_scriptwitness" in psbt_in
            else 0
        )
        input_weight = (
            (40 + len_scriptsig) * WITNESS_SCALE_FACTOR
        ) + len_scriptwitness
        ...
        self.log.info(
            "Test signing inputs that the wallet has keys for but is not watching the scripts"
        )
        self.nodes[1].createwallet(
            wallet_name="scriptwatchonly", disable_private_keys=True
        )
        ...
        desc = descsum_create(
            "wsh(pkh({}))".format(eckey.get_pubkey().get_bytes().hex())
        )
        if self.options.descriptors:
            res = watchonly.importdescriptors(
                [{"desc": desc, "timestamp": "now"}]
            )
        else:
        ...
        self.nodes[0].sendrawtransaction(
            self.nodes[0].finalizepsbt(psbt)["hex"]
        )
        ...
        if self.options.descriptors:
            ...
            desc = descsum_create(
                "tr({},pk({}))".format(
                    H_POINT, eckey.get_pubkey().get_bytes().hex()
                )
            )
            res = watchonly.importdescriptors(
                [{"desc": desc, "timestamp": "now"}]
            ...
            self.nodes[0].sendtoaddress(addr, 10)
            self.generate(self.nodes[0], 1)
            self.nodes[0].importdescriptors(
                [
                    {
            ...
            assert_greater_than(
                len(parsed_psbt.o[vout].map[PSBT_OUT_TAP_TREE]), 0
            )
            assert (
                "taproot_tree"
                in self.nodes[0].decodepsbt(psbt)["outputs"][vout]
            )
            parsed_psbt.make_blank()
            comb_psbt = self.nodes[0].combinepsbt(
                [psbt, parsed_psbt.to_base64()]
            )
            assert_equal(comb_psbt, psbt)

            self.log.info(
                "Test that walletprocesspsbt both updates and signs a non-updated psbt containing Taproot inputs"
            )
            ...
            psbt = self.nodes[0].createpsbt(
                [{"txid": txid, "vout": vout}],
                [{self.nodes[0].getnewaddress(): 0.9999}],
            )
            ...
            assert (
                "taproot_tree"
                not in self.nodes[0].decodepsbt(psbt)["outputs"][0]
            )
            parsed_psbt.make_blank()
            comb_psbt = self.nodes[0].combinepsbt(
                [psbt, parsed_psbt.to_base64()]
        ...
        tx.vin = [
            ...
            CTxIn(
                outpoint=COutPoint(hash=int("bb" * 32, 16), n=0), scriptSig=b""
            ),
            CTxIn(
                outpoint=COutPoint(hash=int("cc" * 32, 16), n=0), scriptSig=b""
            ),
            CTxIn(
                outpoint=COutPoint(hash=int("dd" * 32, 16), n=0), scriptSig=b""
            ),
        ]
        ...
        psbt.i = [
            PSBTMap(
                {
                    bytes([PSBT_IN_RIPEMD160])
                    + hash_ripemd160: preimage_ripemd160
                }
            ),
            PSBTMap({bytes([PSBT_IN_SHA256]) + hash_sha256: preimage_sha256}),
            PSBTMap(
                {bytes([PSBT_IN_HASH160]) + hash_hash160: preimage_hash160}
            ),
            PSBTMap(
                {bytes([PSBT_IN_HASH256]) + hash_hash256: preimage_hash256}
            ),
        ]
        psbt.o = [PSBTMap()]
        ...
        for res_input, preimage_key, hash, preimage in zip(
            res_inputs, preimage_keys, expected_hashes, expected_preimages
        ):
        ...
        self.log.info(
            "Test that PSBT inputs are being checked via script execution"
        )
        ...
        psbt.g = PSBTMap({PSBT_GLOBAL_UNSIGNED_TX: tx.serialize()})
        psbt.i = [
            PSBTMap({bytes([PSBT_IN_WITNESS_UTXO]): acs_prevout.serialize()})
        ]
        psbt.o = [PSBTMap()]
        assert_equal(
            self.nodes[0].finalizepsbt(psbt.to_base64()),
            {
        ...
        self.log.info(
            "Test we don't crash when making a 0-value funded transaction at 0 fee without forcing an input selection"
        )
        assert_raises_rpc_error(
            -4,
