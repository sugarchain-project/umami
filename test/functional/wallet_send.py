...
class WalletSendTest(SugarchainTestFramework):
    ...
    def test_send(
        ...
        amount=None,
        data=None,
        arg_conf_target=None,
        arg_estimate_mode=None,
        arg_fee_rate=None,
        conf_target=None,
        estimate_mode=None,
        fee_rate=None,
        ...
        if add_to_wallet is not None:
            ...
            if psbt:
                ...
                add_to_wallet = from_wallet.getwalletinfo()[
        ...
        if expect_error is None:
            res = from_wallet.send(
                outputs=outputs,
                conf_target=arg_conf_target,
                estimate_mode=arg_estimate_mode,
                fee_rate=arg_fee_rate,
                options=options,
            )
        else:
            try:
        ...
        if include_unsafe:
            from_balance += from_wallet.getbalances()["mine"][
                "untrusted_pending"
            ]
        ...
        if add_to_wallet and not include_watching:
            ...
            assert tx
            assert_equal(
                tx["bip125-replaceable"], "yes" if replaceable else "no"
            )
            ...
            if amount:
                if subtract_fee_from_outputs:
                    ...
                    assert_greater_than(
                        from_balance_before - from_balance, amount
                    )
            else:
                assert next(
                    (
            ...
            if add_to_wallet:
                if not subtract_fee_from_outputs:
                    assert_equal(
                        ...
                        + Decimal(amount if amount else 0),
                    )
            else:
                assert_equal(
                    to_wallet.getbalances()["mine"]["untrusted_pending"],
    ...
    def run_test(self):
        ...
        if not self.options.descriptors:
            ...
            self.nodes[1].createwallet(
                wallet_name="w4", disable_private_keys=False
            )
        ...
        self.test_send(
            from_wallet=w0, to_wallet=w1, amount=1, add_to_wallet=True
        )

        self.log.info("Don't broadcast...")
        res = self.test_send(
            from_wallet=w0, to_wallet=w1, amount=1, add_to_wallet=False
        )
        assert res["hex"]
        ...
        assert_equal(
            self.nodes[1].decodepsbt(res1["psbt"])["fee"],
        ...
        for mode in ["", "foo", Decimal("3.141592")]:
            ...
            assert_raises_rpc_error(
                -8, msg, w0.send, {w1.getnewaddress(): 1}, 0.1, mode
            )
        ...
        assert res["complete"]
        assert_equal(
            self.nodes[0].decodepsbt(res["psbt"])["tx"]["vout"][0][
                "scriptPubKey"
            ]["address"],
            change_address,
        )
        ...
        assert res["complete"]
        change_address = self.nodes[0].decodepsbt(res["psbt"])["tx"]["vout"][0][
            "scriptPubKey"
        ]["address"]
        ...
        res = self.test_send(
            from_wallet=w0, to_wallet=w1, amount=1, locktime=height + 1
        )
        assert res["complete"]
        assert res["txid"]
        txid = res["txid"]
        ...
        res = self.test_send(
            from_wallet=w5, to_wallet=w0, amount=1, include_unsafe=True
        )
        assert res["complete"]
        ...
        self.test_send(
            ...
            amount=1,
            minconf=-4,
            expect_error=(-8, "Negative minconf"),
        )
        res = self.test_send(
            from_wallet=minconfw, to_wallet=w0, amount=1, minconf=3
        )
        assert res["complete"]
