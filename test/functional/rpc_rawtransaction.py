...
class RawTransactionsTest(SugarchainTestFramework):
    ...
    def getrawtransaction_tests(self):
        ...
        for n in [0, 2]:
            self.log.info(
                f"Test getrawtransaction {'with' if n == 0 else 'without'} -txindex"
            )
            ...
            if n == 0:
                ...
                assert_equal(
                    self.nodes[n].getrawtransaction(txId, 0), tx["hex"]
                )

                # 3. valid parameters - supply txid and False for non-verbose
                assert_equal(
                    self.nodes[n].getrawtransaction(txId, False), tx["hex"]
                )

                # 4. valid parameters - supply txid and 1 for verbose.
                # We only check the "hex" field of the output so we don't need to update this test every time the output format changes.
                assert_equal(
                    self.nodes[n].getrawtransaction(txId, 1)["hex"], tx["hex"]
                )
                assert_equal(
                    self.nodes[n].getrawtransaction(txId, 2)["hex"], tx["hex"]
                )

                # 5. valid parameters - supply txid and True for non-verbose
                assert_equal(
                    self.nodes[n].getrawtransaction(txId, True)["hex"],
                    tx["hex"],
                )
            else:
                ...
                for verbose in [None, 0, False, 1, True]:
                    assert_raises_rpc_error(
                        -5,
                        err_msg,
                        self.nodes[n].getrawtransaction,
                        txId,
                        verbose,
                    )
        ...
        for n in [0, 2]:
            self.log.info(
                f"Test getrawtransaction {'with' if n == 0 else 'without'} -txindex, with blockhash"
            )
            # We should be able to get the raw transaction by providing the correct block
            gottx = self.nodes[n].getrawtransaction(
            ...
            if n == 0:
                self.log.info(
                    "Test getrawtransaction with -txindex, without blockhash: 'in_active_chain' should be absent"
                )
                for v in [1, 2]:
                    gottx = self.nodes[n].getrawtransaction(
                        txid=tx, verbosity=v
                ...
                self.log.info(
                    "Test getrawtransaction without -txindex, without blockhash: expect the call to raise"
                )
                assert_raises_rpc_error(
                    -5,
                    err_msg,
            ...
            assert_raises_rpc_error(
                -8,
                f"parameter 3 must be hexadecimal string (not '{foo}')",
                self.nodes[n].getrawtransaction,
                txid=tx,
                blockhash=foo,
            )
    ...
    def getrawtransaction_verbosity_tests(self):
        ...
        for n, v in product([0, 2], [1, 2]):
            self.log.info(
                f"Test getrawtransaction_verbosity {v} {'with' if n == 0 else 'without'} -txindex, with blockhash"
            )
            gottx = self.nodes[n].getrawtransaction(
                txid=tx, verbosity=v, blockhash=block1
            )
            missing_fields = set(fields).difference(gottx.keys())
            if missing_fields:
                raise AssertionError(
                    f"fields {', '.join(missing_fields)} are not in transaction"
                )
            ...
            if v == 2:
                ...
                missing_fields = set(script_pub_key_fields).difference(
                    script_pub_key.keys()
                )
                if missing_fields:
                    raise AssertionError(
        ...
        gottx = self.nodes[1].getrawtransaction(
            txid=coin_base, verbosity=2, blockhash=block1
        )
        assert "fee" not in gottx
    ...
    def createrawtransaction_tests(self):
        ...
        assert_raises_rpc_error(
            -8,
            f"txid must be hexadecimal string (not '{txid}')",
            self.nodes[0].createrawtransaction,
            [{"txid": txid}],
            {},
        )
        ...
        assert_raises_rpc_error(
            -8,
            "Invalid parameter, duplicated address: %s" % address,
            self.nodes[0].createrawtransaction,
            [],
    ...
    def sendrawtransaction_tests(self):
        ...
        self.log.info(
            "Test sendrawtransaction exceeding, falling short of, and equaling maxburnamount"
        )
        ...
        tx.vout = [
            CTxOut(
                int(Decimal(tx_val) * COIN),
                CScript([OP_RETURN] + [OP_FALSE] * 30),
            )
        ]
        tx_hex = tx.serialize().hex()
        assert_raises_rpc_error(
            -25, max_burn_exceeded, self.nodes[2].sendrawtransaction, tx_hex
        )
        ...
        tx_val = 0.001
        tx.vout = [
            CTxOut(int(Decimal(tx_val) * COIN), CScript([OP_FALSE] * 10001))
        ]
        tx_hex = tx.serialize().hex()
        assert_raises_rpc_error(
            -25, max_burn_exceeded, self.nodes[2].sendrawtransaction, tx_hex
        ...
        tx_val = 0.01
        tx.vout = [
            CTxOut(int(Decimal(tx_val) * COIN), CScript([OP_INVALIDOPCODE]))
        ]
        tx_hex = tx.serialize().hex()
        assert_raises_rpc_error(
            -25, max_burn_exceeded, self.nodes[2].sendrawtransaction, tx_hex
        ...
        tx_hex = tx.serialize().hex()
        self.nodes[2].sendrawtransaction(
            hexstring=tx_hex, maxfeerate="0", maxburnamount="0.0011"
        )
        ...
        tx_hex = tx.serialize().hex()
        self.nodes[2].sendrawtransaction(
            hexstring=tx_hex, maxfeerate="0", maxburnamount="0.001"
        )
    ...
    def sendrawtransaction_testmempoolaccept_tests(self):
        ...
        # Fee rate is 0.00100000 SUGAR/kvB
        ...
        # Fee rate is 0.20000000 SUGAR/kvB
        ...
        # and sendrawtransaction should throw
        assert_raises_rpc_error(
            -25, fee_exceeds_max, self.nodes[2].sendrawtransaction, tx["hex"]
        )
        # and the following calls should both succeed
        testres = self.nodes[2].testmempoolaccept(
            rawtxs=[tx["hex"]], maxfeerate="0.20000000"
        )[0]
        assert_equal(testres["allowed"], True)
        self.nodes[2].sendrawtransaction(
            hexstring=tx["hex"], maxfeerate="0.20000000"
        )

        self.log.info(
            "Test sendrawtransaction/testmempoolaccept with tx already in the chain"
        )
    ...
    def decoderawtransaction_tests(self):
        ...
        assert_raises_rpc_error(
            ...
            encrawtx,
            False,
        )  # force decode as non-witness transaction
        ...
        assert_raises_rpc_error(
            ...
            encrawtx,
            False,
        )  # fails to decode as non-witness transaction
        assert_equal(
            decrawtx, decrawtx_wit
        )  # the witness interpretation should be chosen
    ...
    def raw_multisig_transaction_legacy_tests(self):
        ...
        vout = next(
            o for o in rawTx["vout"] if o["value"] == Decimal("2.20000000")
        )
        ...
        assert_equal(
            rawTxPartialSigned["complete"], False
        ...
        assert_equal(
            ...
            bal + Decimal("50.00000000") + Decimal("2.19000000"),
        )  # block reward + tx
        ...
        vout = next(
            o for o in rawTx2["vout"] if o["value"] == Decimal("2.20000000")
        )
        ...
        assert_equal(
            rawTxPartialSigned1["complete"], False
        )  # node1 only has one key, can't comp. sign the tx
        ...
        assert_equal(
            rawTxPartialSigned2["complete"], False
