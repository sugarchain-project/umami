...
class RawTransactionsTest(SugarchainTestFramework):
    ...
    def set_test_params(self):
        ...
        self.rpc_timeout = (
            90  # to prevent timeouts in `test_transaction_too_large`
        )
    ...
    def unlock_utxos(self, wallet):
        ...
        if self.watchonly_txid is not None and self.watchonly_vout is not None:
            to_keep.append(
                {"txid": self.watchonly_txid, "vout": self.watchonly_vout}
            )
    ...
    def test_change_position(self):
        ...
        self.nodes[3].createwallet(
            wallet_name="wwatch", disable_private_keys=True
        )
    ...
    def test_invalid_change_address(self):
        ...
        assert_raises_rpc_error(
            -5,
            "Change address must be a valid sugarchain address",
            self.nodes[2].fundrawtransaction,
            rawtx,
    ...
    def test_valid_change_address(self):
        ...
        assert_raises_rpc_error(
            ...
            self.nodes[2].fundrawtransaction,
            rawtx,
            {"changeAddress": change, "changePosition": 2},
        )
    ...
    def test_change_type(self):
        ...
        rawtx = self.nodes[2].fundrawtransaction(
            rawtx, {"change_type": "bech32"}
        )
    ...
    def test_spend_2of2(self):
        ...
        self.nodes[2].createwallet(
            wallet_name="wmulti", disable_private_keys=True
        )
        ...
        funded_psbt = wmulti.walletcreatefundedpsbt(
            inputs=inputs,
            outputs=outputs,
            options={"changeAddress": w2.getrawchangeaddress()},
        )["psbt"]
    ...
    def test_locked_wallet(self):
        ...
        if self.options.descriptors:
            ...
            self.nodes[1].importdescriptors(
                [
                    {
                        "desc": descsum_create(
                            "wpkh(tprv8ZgxMBicQKsPdYeeZbPSKd2KYLmeVKtcFA7kqCxDvDR13MQ6us8HopUR2wLcS2ZKPhLyKsqpDL2FtL73LMHcgoCL7DXsciA8eX8nbjCR2eG/0h/*h)"
                        ),
                        "timestamp": "now",
                        "active": True,
                    },
                    {
                        "desc": descsum_create(
                            "wpkh(tprv8ZgxMBicQKsPdYeeZbPSKd2KYLmeVKtcFA7kqCxDvDR13MQ6us8HopUR2wLcS2ZKPhLyKsqpDL2FtL73LMHcgoCL7DXsciA8eX8nbjCR2eG/1h/*h)"
                        ),
                        "timestamp": "now",
                        "active": True,
                        "internal": True,
        ...
        value = sum(inp["amount"] for inp in inputs) - Decimal(
            "0.00000500"
        )  # Pay a 500 sat fee
    ...
    def test_many_inputs_fee(self):
        ...
        feeDelta = Decimal(fundedTx["fee"]) - Decimal(signedFee)
        assert (
            feeDelta >= 0 and feeDelta <= self.fee_tolerance * 19
        )  # ~19 inputs
    ...
    def test_watchonly(self):
        ...
        desc_import = [
            {
                "desc": descsum_create(
                    "wpkh(tpubD6NzVbkrYhZ4YNXVQbNhMK1WqguFsUXceaVJKbmno2aZ3B6QfbMeraaYvnBSGpV3vxLyTTK9DYT1yoEck4XUScMzXoQ2U2oSmE2JyMedq3H/1/*)"
                ),
                "timestamp": "now",
                "internal": True,
                "active": True,
    ...
    def test_all_watched_funds(self):
        ...
        assert_equal(
            result["fee"] + res_dec["vout"][0]["value"], self.watchonly_amount
        )
    ...
    def test_option_feerate(self):
        self.log.info(
            "Test fundrawtxn with explicit fee rates (fee_rate sat/vB and feeRate SUGAR/kvB)"
        )
        node = self.nodes[3]
        ...
        sugar_kvb_to_sat_vb = 100000  # (1e5)
        result1 = node.fundrawtransaction(
            rawtx,
            {"fee_rate": str(2 * sugar_kvb_to_sat_vb * self.min_relay_tx_fee)},
        )
        ...
        result3 = node.fundrawtransaction(
            rawtx,
            {"fee_rate": 10 * sugar_kvb_to_sat_vb * self.min_relay_tx_fee},
        )
        result4 = node.fundrawtransaction(
            rawtx, {"feeRate": str(10 * self.min_relay_tx_fee)}
        )

        result_fee_rate = result["fee"] * 1000 / count_bytes(result["hex"])
        assert_fee_amount(
            result1["fee"], count_bytes(result1["hex"]), 2 * result_fee_rate
        )
        assert_fee_amount(
        ...
        assert_fee_amount(
            result4["fee"], count_bytes(result4["hex"]), 10 * result_fee_rate
        )
        ...
        for param, zero_value in product(
            ...
            assert_equal(
                self.nodes[3].fundrawtransaction(rawtx, {param: zero_value})[
                    "fee"
                ],
                0,
            )
        ...
        assert_approx(
            node.fundrawtransaction(rawtx)["fee"],
            vexp=0.00000141,
            vspan=0.00000001,
        )
        ...
        for k, v in {"number": 42, "object": {"foo": "bar"}}.items():
            assert_raises_rpc_error(
                -3,
                f"JSON value of type {k} for field estimate_mode is not of expected type string",
                node.fundrawtransaction,
                rawtx,
                {"estimate_mode": v, "conf_target": 0.1, "add_inputs": True},
            )
        for mode in ["", "foo", Decimal("3.141592")]:
            assert_raises_rpc_error(
                ...
                node.fundrawtransaction,
                rawtx,
                {"estimate_mode": mode, "conf_target": 0.1, "add_inputs": True},
            )
        ...
        for mode in ["unset", "economical", "conservative"]:
            ...
            for k, v in {"string": "", "object": {"foo": "bar"}}.items():
                assert_raises_rpc_error(
                    -3,
                    f"JSON value of type {k} for field conf_target is not of expected type number",
                    node.fundrawtransaction,
                    rawtx,
                    {
                        "estimate_mode": mode,
        ...
        for param, value in {("fee_rate", 100000), ("feeRate", 1.000)}:
            ...
            for invalid_value in [
                ...
                assert_raises_rpc_error(
                    ...
                    node.fundrawtransaction,
                    rawtx,
                    {param: invalid_value, "add_inputs": True},
                )
        ...
        for invalid_value in [
            ...
            assert_raises_rpc_error(
                ...
                node.fundrawtransaction,
                rawtx,
                {"fee_rate": invalid_value, "add_inputs": True},
            )
        ...
        node.fundrawtransaction(
            rawtx, {"feeRate": 0.00000999, "add_inputs": True}
        )
        ...
        assert_raises_rpc_error(
            ...
            node.fundrawtransaction,
            rawtx,
            {"fee_rate": 0.1, "feeRate": 0.1, "add_inputs": True},
        )
        ...
        assert_raises_rpc_error(
            ...
            node.fundrawtransaction,
            rawtx,
            {"estimate_mode": "economical", "feeRate": 0.1, "add_inputs": True},
        )

        for param in ["feeRate", "fee_rate"]:
            self.log.info(
                "- raises RPC error if both {} and conf_target are passed".format(
                    param
                )
            )
            assert_raises_rpc_error(
                ...
                "target in blocks for automatic fee estimation, or an explicit fee rate.".format(
                    param
                ),
                node.fundrawtransaction,
                rawtx,
                {param: 1, "conf_target": 1, "add_inputs": True},
            )
        ...
        assert_raises_rpc_error(
            ...
            node.fundrawtransaction,
            rawtx,
            {"fee_rate": 1, "estimate_mode": "economical", "add_inputs": True},
        )
    ...
    def test_option_subtract_fee_from_outputs(self):
        ...
        # Test subtract fee from outputs with feeRate (SUGAR/kvB)
        result = [
        ...
        result = [
            ...
            self.nodes[3].fundrawtransaction(
                rawtx,
                {"fee_rate": 2 * sugar_kvb_to_sat_vb * self.min_relay_tx_fee},
            ),
            self.nodes[3].fundrawtransaction(
                rawtx,
                {
    ...
    def test_subtract_fee_with_presets(self):
        ...
        rawtx = self.nodes[0].createrawtransaction(
            [{"txid": txid, "vout": vout}], [{self.nodes[0].getnewaddress(): 5}]
        )
    ...
    def test_transaction_too_large(self):
        self.log.info(
            "Test fundrawtx where BnB solution would result in a too large transaction, but Knapsack would not"
        )
        self.nodes[0].createwallet("large")
        wallet = self.nodes[0].get_wallet_rpc(self.default_wallet_name)
        recipient = self.nodes[0].get_wallet_rpc("large")
        outputs = {}
        rawtx = recipient.createrawtransaction(
            [], {wallet.getnewaddress(): 147.99899260}
        )

        # Make 1500 0.1 SUGAR outputs. The amount that we target for funding is in
    ...
    def test_external_inputs(self):
        ...
        raw_tx = wallet.createrawtransaction(
            [ext_utxo], {self.nodes[0].getnewaddress(): ext_utxo["amount"] / 2}
        )
        assert_raises_rpc_error(
            -4,
            "Not solvable pre-selected input COutPoint(%s, %s)"
            % (ext_utxo["txid"][0:10], ext_utxo["vout"]),
        ...
        assert_raises_rpc_error(
            -5,
            "'not a pubkey' is not hex",
            wallet.fundrawtransaction,
            raw_tx,
            {"solving_data": {"pubkeys": ["not a pubkey"]}},
        )
        assert_raises_rpc_error(
            -5,
            "'01234567890a0b0c0d0e0f' is not a valid public key",
            wallet.fundrawtransaction,
            raw_tx,
            {"solving_data": {"pubkeys": ["01234567890a0b0c0d0e0f"]}},
        )
        assert_raises_rpc_error(
            -5,
            "'not a script' is not hex",
            wallet.fundrawtransaction,
            raw_tx,
            {"solving_data": {"scripts": ["not a script"]}},
        )
        assert_raises_rpc_error(
            -8,
            "Unable to parse descriptor 'not a descriptor'",
            wallet.fundrawtransaction,
            raw_tx,
            {"solving_data": {"descriptors": ["not a descriptor"]}},
        )
        assert_raises_rpc_error(
            -8,
            ...
            wallet.fundrawtransaction,
            raw_tx,
            {"input_weights": [{"txid": ext_utxo["txid"]}]},
        )
        assert_raises_rpc_error(
            -8,
            ...
            wallet.fundrawtransaction,
            raw_tx,
            {"input_weights": [{"txid": ext_utxo["txid"], "vout": -1}]},
        )
        assert_raises_rpc_error(
            -8,
            "Invalid parameter, missing weight key",
            wallet.fundrawtransaction,
            raw_tx,
            {
                "input_weights": [
                    {"txid": ext_utxo["txid"], "vout": ext_utxo["vout"]}
                ]
            },
        )
        assert_raises_rpc_error(
            -8,
            "Invalid parameter, weight cannot be less than 165",
            wallet.fundrawtransaction,
            raw_tx,
            {
                "input_weights": [
        ...
        assert_raises_rpc_error(
            -8,
            "Invalid parameter, weight cannot be greater than",
            wallet.fundrawtransaction,
            raw_tx,
            {
                "input_weights": [
        ...
        funded_tx = wallet.fundrawtransaction(
            raw_tx,
            {
                "solving_data": {
        ...
        funded_tx = wallet.fundrawtransaction(
            raw_tx, {"solving_data": {"descriptors": [desc]}}
        )
        signed_tx1 = wallet.signrawtransactionwithwallet(funded_tx["hex"])
        assert not signed_tx1["complete"]
        signed_tx2 = self.nodes[0].signrawtransactionwithwallet(
            signed_tx1["hex"]
        )
        assert signed_tx2["complete"]

        unsigned_weight = self.nodes[0].decoderawtransaction(signed_tx1["hex"])[
            "weight"
        ]
        signed_weight = self.nodes[0].decoderawtransaction(signed_tx2["hex"])[
            "weight"
        ]
        ...
        signed_tx = self.nodes[0].signrawtransactionwithwallet(signed_tx["hex"])
        assert_equal(
            self.nodes[0].testmempoolaccept([signed_tx["hex"]])[0]["allowed"],
            True,
        )
        assert_equal(signed_tx["complete"], True)
        # Reducing the weight should have a lower fee
        funded_tx2 = wallet.fundrawtransaction(
            raw_tx,
            {
                "input_weights": [
                    {
        ...
        funded_tx3 = wallet.fundrawtransaction(
            raw_tx,
            {
        ...
        funded_tx4 = wallet.fundrawtransaction(
            raw_tx,
            {
                "input_weights": [
                    {
        ...
        tx4_weight = (
            wallet.decoderawtransaction(funded_tx4["hex"])["weight"]
            + input_add_weight
        )
    ...
    def test_add_inputs_default_value(self):
        ...
        # Create and fund the wallet with 5 SUGAR
        ...
        # Fund wallet with 2 outputs, 5 SUGAR each.
        addr2 = wallet.getnewaddress(address_type="bech32")
        source_tx = self.nodes[0].send(
            outputs=[{addr1: 5}, {addr2: 5}], options={"change_position": 0}
        )
        self.generate(self.nodes[0], 1)
        ...
        assert "psbt" in wallet.walletcreatefundedpsbt(
        ...
        options["add_inputs"] = True
        assert "psbt" in wallet.walletcreatefundedpsbt(
            outputs=[{addr1: 8}], inputs=inputs, options=options
        )
        ...
        psbt_tx = wallet.walletcreatefundedpsbt(
            outputs=[{addr1: 8}], inputs=inputs, options=options
        )
        # Check that only the preset inputs were added to the tx
        decoded_psbt_inputs = self.nodes[0].decodepsbt(psbt_tx["psbt"])["tx"][
            "vin"
        ]
        assert_equal(len(decoded_psbt_inputs), 2)
        for input in decoded_psbt_inputs:
        ...
        options = {"add_inputs": True}
        assert "psbt" in wallet.walletcreatefundedpsbt(
            inputs=[], outputs=outputs, options=options
        )
    ...
    def test_preset_inputs_selection(self):
        self.log.info(
            "Test wallet preset inputs are not double-counted or reused in coin selection"
        )

        # Create and fund the wallet with 4 UTXO of 5 SUGAR each (20 SUGAR total)
        ...
        # Attempt to send 29 SUGAR from a wallet that only has 20 SUGAR. The wallet should exclude
        # the preset inputs from the pool of available coins, realize that there is not enough
        # money to fund the 29 SUGAR payment, and fail with "Insufficient funds".
        #
        # Even with SFFO, the wallet can only afford to send 20 SUGAR.
        # If the wallet does not properly exclude preset inputs from the pool of available coins
        # prior to coin selection, it may create a transaction that does not fund the full payment
        # amount or, through SFFO, incorrectly reduce the recipient's amount by the difference
        # between the original target and the wrongly counted inputs (in this case 9 SUGAR)
        # so that the recipient's amount is no longer equal to the user's selected target of 29 SUGAR.
        ...
        assert_raises_rpc_error(
            ...
            "Insufficient funds",
            wallet.send,
            outputs=[{wallet.getnewaddress(address_type="bech32"): 29}],
            options=options,
        )
    ...
    def test_weight_calculation(self):
        ...
        self.nodes[0].sendtoaddress(
            wallet.getnewaddress(address_type="bech32"), 5
        )
        self.generate(self.nodes[0], 1)

        rawtx = wallet.createrawtransaction(
            [{"txid": txid, "vout": vout}],
            [{self.nodes[0].getnewaddress(address_type="bech32"): 8}],
        )
        fundedtx = wallet.fundrawtransaction(
            rawtx, {"fee_rate": 10, "change_type": "bech32"}
        )
        ...
        rawtx = wallet.createrawtransaction(
            [{"txid": txid, "vout": ext_vout}],
            [{self.nodes[0].getnewaddress(): 13}],
        )
        ...
        fundedtx = wallet.fundrawtransaction(
            rawtx,
            {
                "fee_rate": 10,
                "change_type": "bech32",
                "solving_data": {"descriptors": [ext_desc]},
            },
        )
    ...
    def test_include_unsafe(self):
        ...
        rawtx = wallet.createrawtransaction(
            [], [{self.nodes[2].getnewaddress(): 7.5}]
        )
        assert_raises_rpc_error(
        ...
        assert all(
            (txin["txid"], txin["vout"]) in inputs for txin in tx_dec["vin"]
        )
    ...
    def test_22670(self):
        ...
        # that this bug is not detected because the transaction fee must be at least 0.01 SUGAR (the minimum change value).
        # Otherwise the targeted minimum change value will be enough to cover the transaction fees that were not
        # being accounted for. So the minimum relay fee is set to 0.1 SUGAR/kvB in this test.
        self.log.info("Test issue 22670 ApproximateBestSubset bug")
        # Make sure the default wallet will not be loaded when restarted with a high minrelaytxfee
        self.nodes[0].unloadwallet(self.default_wallet_name, False)
        feerate = Decimal("0.1")
        self.restart_node(
            0, [f"-minrelaytxfee={feerate}", "-discardfee=0"]
        )  # Set high minrelayfee, set discardfee to 0 for easier calculation
        ...
        change_tx = tester.fundrawtransaction(
            tester.createrawtransaction([], [{funds.getnewaddress(): 1.5}])
        )
        tx = tester.createrawtransaction([], [{funds.getnewaddress(): 2}])
        no_change_tx = tester.fundrawtransaction(
            tx, {"subtractFeeFromOutputs": [0]}
        )
        ...
        def do_fund_send(target):
            create_tx = tester.createrawtransaction(
                [], [{funds.getnewaddress(): target}]
            )
        ...
        upper_bound = (
            Decimal("2.0") - cost_of_change - overhead_fees - Decimal("0.01")
        )
    ...
    def test_feerate_rounding(self):
        self.log.info(
            "Test that rounding of GetFee does not result in an assertion"
        )
        ...
        # fail with insufficient funds rather than sugarchaind asserting.
        rawtx = w.createrawtransaction(
            inputs=[],
            outputs=[
                {
                    self.nodes[0].getnewaddress(address_type="bech32"): 1
                    - 0.00000202
                }
            ],
        )
    ...
    def test_input_confs_control(self):
        ...
        for _ in range(2):
            self.nodes[2].sendmany(
                "", {wallet.getnewaddress(): 1, wallet.getnewaddress(): 1}
            )
            self.generate(self.nodes[2], 1)
        ...
        raw_tx1 = wallet.createrawtransaction(
            [], {target_address: 0.1}, 0, True
        )
        funded_tx1 = wallet.fundrawtransaction(
            raw_tx1, {"fee_rate": 1, "maxconf": 0}
        )["hex"]
        ...
        # Create a replacement tx to 'final_tx1' that has 1 SUGAR target instead of 0.1.
        raw_tx2 = wallet.createrawtransaction(
        ...
        assert_raises_rpc_error(
            ...
            wallet.fundrawtransaction,
            raw_tx2,
            {"add_inputs": True, "minconf": 3, "fee_rate": 10},
        )

        self.log.info(
            "Fail to broadcast a new TX with maxconf 0 due to BIP125 rules to verify it actually chose unconfirmed outputs"
        )
        # Now fund 'raw_tx2' to fulfill the total target (1 SUGAR) by using all the wallet unconfirmed outputs.
        # As it was created with the first unconfirmed output, 'raw_tx2' only has 0.1 SUGAR covered (need to fund 0.9 SUGAR more).
        ...
        funded_invalid = wallet.fundrawtransaction(
            raw_tx2, {"add_inputs": True, "maxconf": 0, "fee_rate": 10}
        )["hex"]
        final_invalid = wallet.signrawtransactionwithwallet(funded_invalid)[
            "hex"
        ]
        assert_raises_rpc_error(
            -26,
        ...
        self.log.info(
            "Craft a replacement adding inputs with highest depth possible"
        )
        funded_tx2 = wallet.fundrawtransaction(
            raw_tx2, {"add_inputs": True, "minconf": 2, "fee_rate": 10}
        )["hex"]
        ...
        for vin in tx2_inputs:
            if vin["txid"] != unconfirmed_txid:
                assert_greater_than_or_equal(
                    self.nodes[0].gettxout(vin["txid"], vin["vout"])[
                        "confirmations"
                    ],
                    2,
                )
