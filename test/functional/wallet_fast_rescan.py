...
NUM_DESCRIPTORS = (
    9  # number of descriptors (8 default ranged ones + 1 fixed non-ranged one)
)
NUM_BLOCKS = 6  # number of blocks to mine


class WalletFastRescanTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        print(
            w.importdescriptors(
                [
                    {
                        "desc": descsum_create(f"wpkh({fixed_key.privkey})"),
                        "timestamp": "now",
                    }
                ]
            )
            ...
            for desc_info in w.listdescriptors()["descriptors"]:
                if "range" in desc_info:
                    ...
                    addr = w.deriveaddresses(
                        desc_info["desc"], [end_range, end_range]
                    )[0]
        ...
        with node.assert_debug_log(["fast variant using block filters"]):
            ...
            w.importdescriptors(
                [
                    {"desc": descriptor["desc"], "timestamp": 0}
