...
class ScantxoutsetTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(
            sum(
                u["coinbase"]
                for u in self.nodes[0].scantxoutset(
                    "start", [self.wallet.get_descriptor()]
                )["unspents"]
            ),
            49,
        )
        ...
        # Note that all amounts in the UTXO set are powers of 2 multiplied by 0.001 SUGAR, so each amounts uniquely identifies a subset.
        assert_equal(
            self.nodes[0].scantxoutset(
                "start",
                [
