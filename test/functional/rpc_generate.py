...
class RPCGenerateTest(SugarchainTestFramework):
    ...
    def test_generatetoaddress(self):
        self.generatetoaddress(
            self.nodes[0], 1, "mneYUmWYsuk7kySiURxCi3AGxrAqZxLgPZ"
        )
        assert_raises_rpc_error(
            -5,
            "Invalid address",
    ...
    def test_generateblock(self):
        ...
        generated_block = self.generateblock(
            node, output=address, transactions=[], submit=False
        )
        ...
        assert_equal(len(block["tx"]), 1)
        assert_equal(
            block["tx"][0]["vout"][0]["scriptPubKey"]["address"], combo_address
        )
    ...
    def test_generate(self):
        ...
        self.log.info(
            "Test rpc generate is a hidden command not discoverable in general help"
        )
