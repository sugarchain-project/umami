...
By default, sugarchaind will dump mempool on shutdown and
...
class MempoolPersistTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        if self.is_sqlite_compiled():
            ...
            assert_equal(
                [{"success": True}],
                wallet_watch.importdescriptors(
                    [
                        {
                            "desc": self.mini_wallet.get_descriptor(),
                            "timestamp": 0,
                        }
                    ]
                ),
        ...
        for _ in range(5):
            last_txid = self.mini_wallet.send_self_transfer(
                from_node=self.nodes[2]
            )["txid"]
        if self.is_sqlite_compiled():
            self.nodes[
                2
            ].syncwithvalidationinterfacequeue()  # Flush mempool to wallet
        ...
        assert_equal(
            entry_prioritised_before_restart["fees"]["base"]
            + Decimal("0.00009999"),
        ...
        self.log.debug(
            "Force -persistmempool=0 node1 to savemempool to disk via RPC"
        )
        ...
        self.log.debug(
            "Stop nodes, make node1 use mempool.dat from node0. Verify it has 7 transactions"
        )
        ...
        self.log.debug(
            "Prevent sugarchaind from writing mempool.dat to disk. Verify that `savemempool` fails"
        )
