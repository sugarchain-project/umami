...
class WalletMultisigDescriptorPSBTTest(SugarchainTestFramework):
    ...
    def _check_psbt(psbt, to, value, multisig):
        ...
        for vout in tx["vout"]:
            ...
            assert_equal(
                multisig.getaddressinfo(address)["ischange"], address != to
            )
            if address == to:
                amount += vout["value"]
    ...
    def run_test(self):
        ...
        for _ in range(
            ...
            all(
                address == receive_addresses[0] for address in receive_addresses
            )
            change_addresses = [
        ...
        self.generatetoaddress(
            self.nodes[0], 101, coordinator_wallet.getnewaddress()
        )
        ...
        assert_equal(
            participants["signers"][self.N - 1].getbalance(), value * 2
        )
