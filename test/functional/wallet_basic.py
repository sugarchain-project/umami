...
class WalletTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        # Send 21 SUGAR from 0 to 2 using sendtoaddress call.
        ...
        assert_equal(txout["value"], 50)
        txout = self.nodes[0].gettxout(
            confirmed_txid, confirmed_index
        ...
        assert_equal(
            set([txout1["value"], txout2["value"]]), set([10, balance])
        )
        walletinfo = self.nodes[0].getwalletinfo()
        ...
        # node0 should end up with 100 sugar in block rewards plus fees, but
        ...
        # Send 10 SUGAR with subtract fee from amount
        ...
        # Sendmany 10 SUGAR with subtract fee from amount
        ...
        fee_rate_sat_vb = 2
        fee_rate_sugar_kvb = fee_rate_sat_vb * 1e3 / 1e8
        explicit_fee_rate_sugar_kvb = Decimal(fee_rate_sugar_kvb) / 1000

        # Test passing fee_rate as a string
        txid = self.nodes[2].sendmany(
            amounts={address: 10}, fee_rate=str(fee_rate_sat_vb)
        )
        self.generate(
            self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3])
        )
        ...
        txid = self.nodes[2].sendmany(
            amounts={address: amount}, fee_rate=fee_rate_sat_vb
        )
        self.generate(
            self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3])
        )
        ...
        for target, mode in product([-1, 0], ["sugar/kb", "sat/b"]):
            assert_raises_rpc_error(
                -8,
        ...
        usp = self.nodes[1].listunspent(
            query_options={"minimumAmount": "49.998"}
        )[0]
        ...
        if not self.options.descriptors:
            ...
            # This will raise an exception for attempting to get the private key of an invalid Sugarchain address
            assert_raises_rpc_error(
                -5,
                "Invalid Sugarchain address",
                self.nodes[0].dumpprivkey,
                "invalid",
            )

            # This will raise an exception for attempting to set a label for an invalid Sugarchain address
            assert_raises_rpc_error(
                -5,
                "Invalid Sugarchain address",
            ...
            assert_raises_rpc_error(
                -5,
                "Invalid Sugarchain address or script",
                self.nodes[0].importaddress,
                "invalid",
            )
            ...
            assert_fee_amount(fee, tx_size, Decimal(fee_rate_sugar_kvb))
            ...
            assert self.nodes[1].getaddressinfo(address_to_import)[
                "iswatchonly"
            ]
        ...
        assert extra_txid in [
            tx["txid"] for tx in self.nodes[0].listtransactions()
        ]
        ...
        assert_equal(
            address_info["address"], "mneYUmWYsuk7kySiURxCi3AGxrAqZxLgPZ"
        )
        assert_equal(
        ...
        output_addresses = [
            vout["scriptPubKey"]["address"] for vout in tx["vout"]
        ]
        ...
        assert_equal(
            tx[verbose_field], self.nodes[0].decoderawtransaction(tx["hex"])
        )

        self.log.info("Test send* RPCs with verbose=True")
        address = self.nodes[0].getnewaddress("test")
        txid_feeReason_one = self.nodes[2].sendtoaddress(
            address=address, amount=5, verbose=True
        )
        assert_equal(txid_feeReason_one["fee_reason"], "Fallback fee")
        txid_feeReason_two = self.nodes[2].sendmany(
            dummy="", amounts={address: 5}, verbose=True
        )
        assert_equal(txid_feeReason_two["fee_reason"], "Fallback fee")
        self.log.info("Test send* RPCs with verbose=False")
        txid_feeReason_three = self.nodes[2].sendtoaddress(
            address=address, amount=5, verbose=False
        )
        assert_equal(
            self.nodes[2].gettransaction(txid_feeReason_three)["txid"],
            txid_feeReason_three,
        )
        txid_feeReason_four = self.nodes[2].sendmany(
            dummy="", amounts={address: 5}, verbose=False
        )
        assert_equal(
            self.nodes[2].gettransaction(txid_feeReason_four)["txid"],
            txid_feeReason_four,
        )
        ...
        if self.options.descriptors:
            ...
            multi_a = descsum_create(
                "wsh(multi(1,tpubD6NzVbkrYhZ4YBNjUo96Jxd1u4XKWgnoc7LsA1jz3Yc2NiDbhtfBhaBtemB73n9V5vtJHwU6FVXwggTbeoJWQ1rzdz8ysDuQkpnaHyvnvzR/*,tpubD6NzVbkrYhZ4YHdDGMAYGaWxMSC1B6tPRTHuU5t3BcfcS3nrF523iFm5waFd1pP3ZvJt4Jr8XmCmsTBNx5suhcSgtzpGjGMASR3tau1hJz4/*))"
            )
            multi_b = descsum_create(
                "wsh(multi(1,tpubD6NzVbkrYhZ4YHdDGMAYGaWxMSC1B6tPRTHuU5t3BcfcS3nrF523iFm5waFd1pP3ZvJt4Jr8XmCmsTBNx5suhcSgtzpGjGMASR3tau1hJz4/*,tpubD6NzVbkrYhZ4Y2RLiuEzNQkntjmsLpPYDm3LTRBYynUQtDtpzeUKAcb9sYthSFL3YR74cdFgF5mW8yKxv2W2CWuZDFR2dUpE5PF9kbrVXNZ/*))"
            )
            ...
            self.nodes[0].createwallet(
                wallet_name="wo", descriptors=True, disable_private_keys=True
            )
