...
class SettingsTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        node.assert_start_raises_init_error(
            expected_msg="Unable to parse settings file",
        ...
        with settings.open("w") as fp:
            fp.write('"string"')
        node.assert_start_raises_init_error(
            expected_msg='Found non-object value "string" in settings file',
            match=ErrorMatch.PARTIAL_REGEX,
        )
        ...
        node.assert_start_raises_init_error(
            expected_msg="Found duplicate key key in settings file",
            match=ErrorMatch.PARTIAL_REGEX,
        )

        # Test invalid settings file is ignored with command line -nosettings
        with node.assert_debug_log(
            expected_msgs=["Command-line arg: settings=false"]
        ):
        ...
        with node.assert_debug_log(
            expected_msgs=["Config file arg: [regtest] settings=false"]
        ):
            self.start_node(0)
            self.stop_node(0)
        ...
        with altsettings.open("w") as fp:
            fp.write('{"key": "value"}')
        with node.assert_debug_log(
            expected_msgs=['Setting file arg: key = "value"']
        ):
