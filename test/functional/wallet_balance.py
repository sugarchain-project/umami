...
from test_framework.address import (
    ADDRESS_BCRT1_UNSPENDABLE as ADDRESS_WATCHONLY,
)
...
class WalletTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(
            len(
                self.nodes[0].listunspent(
                    query_options={"include_immature_coinbase": True}
                )
            ),
            1,
        )
        assert_equal(
            len(
                self.nodes[0].listunspent(
                    query_options={"include_immature_coinbase": False}
                )
            ),
            0,
        )
        ...
        assert_equal(
            ...
            1 if self.options.descriptors else 2,
        )
        assert_equal(
            len(
                self.nodes[0].listunspent(
            ...
            1 if self.options.descriptors else COINBASE_MATURITY + 2,
        )
        ...
        if not self.options.descriptors:
            ...
            assert_equal(
                self.nodes[0].getbalances()["watchonly"]["immature"], 5000
            )
            assert "watchonly" not in self.nodes[1].getbalances()
        ...
        if not self.options.descriptors:
            ...
            assert_equal(
                self.nodes[0].getbalance(minconf=0, include_watchonly=True), 50
            )
        ...
        # Send 40 SUGAR from 0 to 1 and 60 SUGAR from 1 to 0.
        ...
        # SUGAR, which is more than would be available if transaction 1 were
        # replaced.
        ...
        def test_balances(*, fee_node_1=0):
            ...
            assert_equal(
                self.nodes[0].getbalance(), Decimal("9.99")
            )  # change from node 0's send
            assert_equal(
                self.nodes[1].getbalance(), Decimal("0")
            )  # node 1's send had an unsafe input
            # Same with minconf=0
            ...
            assert_equal(
                self.nodes[0].getunconfirmedbalance(), Decimal("60")
        ...
        balance_node0 = Decimal(
            "69.99"
        )  # node 1's send plus change from node 0's send
        balance_node1 = Decimal("29.98")  # change from node 0's send
        assert_equal(
            self.nodes[0].getbalances()["mine"]["trusted"], balance_node0
        )
        assert_equal(
            self.nodes[1].getbalances()["mine"]["trusted"], balance_node1
        )
        ...
        block_reorg = self.generatetoaddress(
            self.nodes[1], 1, ADDRESS_WATCHONLY
        )[0]
        ...
        assert_equal(
            self.nodes[0].getbalance(minconf=0), 0
        )  # wallet txs not in the mempool are untrusted
        self.generatetoaddress(
            self.nodes[0], 1, ADDRESS_WATCHONLY, sync_fun=self.no_op
        )
        ...
        self.generatetoaddress(self.nodes[1], 1, ADDRESS_WATCHONLY)
        assert_equal(
            self.nodes[0].getbalance(minconf=0), total_amount + 1
        )  # The reorg recovered our fee of 1 coin

        if not self.options.descriptors:
            self.log.info(
                "Check if mempool is taken into account after import*"
            )
            ...
            assert_equal(
                self.nodes[0].getbalances()["mine"]["untrusted_pending"], 0
            )
            assert_equal(
                self.nodes[0].getbalances()["watchonly"]["untrusted_pending"],
                Decimal("0.1"),
            )
            ...
            assert_equal(
                self.nodes[0].getbalances()["watchonly"]["untrusted_pending"], 0
            )
