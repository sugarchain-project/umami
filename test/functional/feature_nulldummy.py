...
class NULLDUMMYTest(SugarchainTestFramework):
    ...
    def create_transaction(
        ...
        signedtx = self.nodes[0].signrawtransactionwithkey(
            rawtx, [privkey], input
        )
    ...
    def run_test(self):
        ...
        ms_unlock_details = {
            "scriptPubKey": address_to_scriptpubkey(self.ms_address).hex(),
        ...
        self.generate(
            self.nodes[0], COINBASE_MATURITY
        )  # block height = COINBASE_MATURITY + 2
        self.lastblockhash = self.nodes[0].getbestblockhash()
        self.lastblockheight = COINBASE_MATURITY + 2
        self.lastblocktime = int(time.time()) + self.lastblockheight

        self.log.info(
            f"Test 1: NULLDUMMY compliant base transactions should be accepted to mempool and mined before activation [{COINBASE_MATURITY + 3}]"
        )
        test1txs = [
            self.create_transaction(
                txid=coinbase_txid[0],
        ...
        test1txs.append(
            self.create_transaction(
                txid=txid1,
                input_details=ms_unlock_details,
                addr=self.ms_address,
                amount=48,
        ...
        test1txs.append(
            self.create_transaction(
                txid=coinbase_txid[1],
                addr=self.wit_ms_address,
                amount=49,
                privkey=self.nodes[0].get_deterministic_priv_key().key,
            )
        )
        ...
        assert_raises_rpc_error(
            ...
            test2tx.serialize_with_witness().hex(),
            0,
        )

        self.log.info(
            f"Test 3: Non-NULLDUMMY base transactions should be accepted in a block before activation [{COINBASE_MATURITY + 4}]"
        )
        ...
        test4tx = self.create_transaction(
            txid=test2tx.hash,
            input_details=ms_unlock_details,
        ...
        assert_raises_rpc_error(
            ...
            NULLDUMMY_ERROR,
            self.nodes[0].sendrawtransaction,
            test4tx.serialize_with_witness().hex(),
            0,
        )
        ...
        test5tx = self.create_transaction(
            txid=txid3,
            input_details={
                "scriptPubKey": test1txs[2].vout[0].scriptPubKey.hex(),
                "amount": 49,
                "witnessScript": wms["redeemScript"],
            },
        ...
        assert_raises_rpc_error(
            ...
            NULLDUMMY_ERROR,
            self.nodes[0].sendrawtransaction,
            test5tx.serialize_with_witness().hex(),
            0,
        )
        self.block_submit(
            self.nodes[0], [test5tx], with_witness=True, accept=False
        )

        self.log.info(
            f"Test 6: NULLDUMMY compliant base/witness transactions should be accepted to mempool and in block after activation [{COINBASE_MATURITY + 5}]"
        )
        for i in test6txs:
            self.nodes[0].sendrawtransaction(
                i.serialize_with_witness().hex(), 0
            )
        self.block_submit(
            self.nodes[0], test6txs, with_witness=True, accept=True
        )
    ...
    def block_submit(self, node, txs, *, with_witness=False, accept):
        ...
        block = create_block(
            tmpl=tmpl, ntime=self.lastblocktime + 1, txlist=txs
        )
        if with_witness:
            add_witness_commitment(block)
        block.solve()
        assert_equal(
            None if accept else NULLDUMMY_ERROR,
