...
class WalletSignerTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        self.restart_node(
            1, [f"-signer={self.mock_invalid_signer_path()}", "-keypool=10"]
        )
        self.test_invalid_signer()
        self.restart_node(
            1, [f"-signer={self.mock_multi_signers_path()}", "-keypool=10"]
        )
    ...
    def test_valid_signer(self):
        ...
        assert_raises_rpc_error(
            -8,
            "Wallet flag is immutable: external_signer",
            not_hww.setwalletflag,
            "external_signer",
            True,
        )
        ...
        assert_equal(
            address4,
            "bcrt1phw4cgpt6cd30kz9k4wkpwm872cdvhss29jga2xpmftelhqll62ms4e9sqj",
        )
        ...
        self.set_mock_result(self.nodes[1], "2")
        assert_raises_rpc_error(
            -1, "RunCommandParseJSON error", hww.walletdisplayaddress, address1
        )
        self.clear_mock_result(self.nodes[1])
        ...
        mock_psbt = mock_wallet.walletcreatefundedpsbt(
            [], {dest: 0.5}, 0, {"replaceable": True}, True
        )["psbt"]
        mock_psbt_signed = mock_wallet.walletprocesspsbt(
            psbt=mock_psbt, sign=True, sighashtype="ALL", bip32derivs=True
        )
        ...
        with open(
            os.path.join(self.nodes[1].cwd, "mock_psbt"), "w", encoding="utf8"
        ) as f:
        ...
        res = hww.sendall(
            recipients=[{dest: 0.5}, hww.getrawchangeaddress()],
        ...
        mock_psbt_bumped_signed = mock_wallet.walletprocesspsbt(
            psbt=mock_psbt_bumped,
            sign=True,
            sighashtype="ALL",
            bip32derivs=True,
        )
    ...
    def test_multiple_signers(self):
        ...
        assert_raises_rpc_error(
            -1,
            "GetExternalSigner: More than one external signer found",
            self.nodes[1].createwallet,
            wallet_name="multi_hww",
            disable_private_keys=True,
            descriptors=True,
            external_signer=True,
        )
