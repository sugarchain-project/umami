...
class NetTest(SugarchainTestFramework):
    ...
    def test_getpeerinfo(self):
        ...
        for node, peer, field in product(
            range(self.num_nodes), range(2), ["last_block", "last_transaction"]
        ):
        ...
        assert (
            "(ipv4, ipv6, onion, i2p, cjdns, not_publicly_routable)"
            in self.nodes[0].help("getpeerinfo")
        )

        self.log.info(
            "Check getpeerinfo output before a version message was sent"
        )
        no_version_peer_id = 2
        no_version_peer_conntime = int(time.time())
        self.nodes[0].setmocktime(no_version_peer_conntime)
        with self.nodes[0].assert_debug_log(
            [f"Added connection peer={no_version_peer_id}"]
        ):
    ...
    def test_getnetworkinfo(self):
        ...
        for n in self.nodes:
            self.wait_until(
                lambda: n.getnetworkinfo()["connections"] == 0, timeout=3
            )
    ...
    def test_service_flags(self):
        ...
        self.nodes[0].add_p2p_connection(
            P2PInterface(), services=(1 << 4) | (1 << 63)
        )
        assert_equal(
            ["UNKNOWN[2^4]", "UNKNOWN[2^63]"],
    ...
    def test_getnodeaddresses(self):
        ...
        assert_equal(
            len(self.nodes[0].getnodeaddresses()), 1
        )  # default count is 1
    ...
    def test_addpeeraddress(self):
        ...
        assert_equal(
            node.addpeeraddress(address="", port=8333), {"success": False}
        )
