#!/usr/bin/env python3
# Copyright (c) 2022-2022 The Sugarchain Core developers
...
class I2PSessions(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info(
            "Ensure we create a persistent session when -i2pacceptincoming=1"
        )
        node0 = self.nodes[0]
        with node0.assert_debug_log(
            expected_msgs=["Creating persistent SAM session"]
        ):
            node0.addnode(node=addr, command="onetry")

        self.log.info(
            "Ensure we create a transient session when -i2pacceptincoming=0"
        )
        node1 = self.nodes[1]
        with node1.assert_debug_log(
            expected_msgs=["Creating transient SAM session"]
        ):
