...
class P2PIBDTxRelayTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info(
            "Check that nodes don't send getdatas for transactions while still in IBD"
        )
        ...
        self.log.info(
            "Check that nodes don't process unsolicited transactions while still in IBD"
        )
        ...
        assert self.nodes[1].decoderawtransaction(
            rawhex
        )  # returns a dict, should not throw
        tx = from_hex(CTransaction(), rawhex)
        peer_txer = self.nodes[0].add_p2p_connection(P2PInterface())
        with self.nodes[0].assert_debug_log(
            expected_msgs=["received: tx"], unexpected_msgs=["was not accepted"]
        ):
        ...
        self.log.info(
            "Check that nodes process the same transaction, even when unsolicited, when no longer in IBD"
        )
        peer_txer = self.nodes[0].add_p2p_connection(P2PInterface())
