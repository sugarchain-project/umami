...
class AddressTypeTest(SugarchainTestFramework):
    ...
    def test_desc(self, node, address, multisig, typ, utxo):
        ...
        assert (
            info["desc"]
            == self.nodes[0].getdescriptorinfo(info["desc"])["descriptor"]
        )
        assert_equal(
            info["desc"][-8:],
    ...
    def test_change_output_type(self, node_sender, destinations, expected_type):
        txid = self.nodes[node_sender].sendmany(
            dummy="", amounts=dict.fromkeys(destinations, 0.001)
        )
        tx = self.nodes[node_sender].gettransaction(txid=txid, verbose=True)[
            "decoded"
        ]
    ...
    def run_test(self):
        ...
        if not self.options.descriptors:
            ...
            self.test_address(
                2,
                self.nodes[2].addmultisigaddress(
                    2, [compressed_1, compressed_2]
                )["address"],
                True,
                "p2sh-segwit",
            ...
            self.test_address(
                3,
                self.nodes[3].addmultisigaddress(
                    2, [compressed_1, compressed_2]
                )["address"],
                True,
                "bech32",
        ...
        for explicit_type, multisig, from_node in itertools.product(
            ...
            to_send = (
                old_balances[from_node] / (COINBASE_MATURITY + 1)
        ...
        self.test_address(
            3, self.nodes[3].getrawchangeaddress(), multisig=False, typ="bech32"
        )
        ...
        assert_raises_rpc_error(
            -5, "Unknown address type ''", self.nodes[3].getnewaddress, None, ""
        )
        assert_raises_rpc_error(
            -5, "Unknown address type ''", self.nodes[3].getrawchangeaddress, ""
        )
        assert_raises_rpc_error(
            -5,
        ...
        if self.options.descriptors:
            ...
            self.test_address(
                4,
                self.nodes[4].getnewaddress("", "bech32m"),
                multisig=False,
                typ="bech32m",
            )
            self.test_address(
                4,
                self.nodes[4].getrawchangeaddress("bech32m"),
                multisig=False,
                typ="bech32m",
            )
        else:
            ...
            assert_raises_rpc_error(
                -8,
                "Legacy wallets cannot provide bech32m addresses",
                self.nodes[0].getrawchangeaddress,
                "bech32m",
            )
