...
class GetBlockFilterTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        chain0_hashes = [
            self.nodes[0].getblockhash(block_height)
            for block_height in range(4)
        ]
        ...
        for filter_type in FILTER_TYPES:
            assert_raises_rpc_error(
                -1,
                "Index is not enabled for filtertype {}".format(filter_type),
                self.nodes[0].getblockfilter,
                genesis_hash,
                filter_type,
            )
