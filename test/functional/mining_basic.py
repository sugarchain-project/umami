...
class MiningTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(
            mining_info["difficulty"], Decimal("4.656542373906925E-10")
        )
        assert_equal(
            mining_info["networkhashps"], Decimal("0.003333333333333334")
        )
        ...
        assert_submitblock(
            bad_block, "bad-txns-duplicate", "bad-txns-duplicate"
        )
        ...
        assert_submitblock(
            bad_block, "prev-blk-not-found", "prev-blk-not-found"
        )
        ...
        # Must be active now:
        assert (
            chain_tip(block.hash, status="active", branchlen=0)
        ...
        assert_equal(
            node.submitblock(hexdata=block.serialize().hex()), "duplicate"
        )  # valid
