#!/usr/bin/env python3
# Copyright (c) 2017-2019 The Sugarchain Core developers
...
import os
from test_framework.util import get_datadir_path, assert_equal, str_to_b64str
import http.client
import urllib.parse
...
def rpccall(node, user, method):
    ...
    headers = {
        "Authorization": "Basic "
        + str_to_b64str("{}:{}".format(user[0], user[3]))
    }
...
class RPCWhitelistTest(SugarchainTestFramework):
    ...
    def setup_chain(self):
        ...
        with open(
            os.path.join(
                get_datadir_path(self.options.tmpdir, 0), "sugarchain.conf"
            ),
            "a",
            encoding="utf8",
        ) as f:
            ...
            for strangedude in self.strange_users:
                f.write(
                    "rpcauth=" + strangedude[0] + ":" + strangedude[1] + "\n"
                )
                f.write(
                    "rpcwhitelist=" + strangedude[0] + strangedude[2] + "\n"
                )
    ...
    def run_test(self):
        for user in self.users:
            ...
            for permission in permissions:
                self.log.info(
                    "["
                    + user[0]
                    + "]: Testing a permitted permission ("
                    + permission
                    + ")"
                )
                assert_equal(
                    200, rpccall(self.nodes[0], user, permission).status
                )
            for permission in self.never_allowed:
                self.log.info(
                    "["
                    + user[0]
                    + "]: Testing a non permitted permission ("
                    + permission
                    + ")"
                )
                assert_equal(
                    403, rpccall(self.nodes[0], user, permission).status
                )
        ...
        for permission in self.never_allowed:
            ...
            assert_equal(
                403,
                rpccall(
                    self.nodes[0], self.strange_users[0], permission
                ).status,
            )
        for permission in self.never_allowed:
            ...
            assert_equal(
                403,
                rpccall(
                    self.nodes[0], self.strange_users[1], permission
                ).status,
            )
        ...
        assert_equal(
            200,
            rpccall(
                self.nodes[0], self.strange_users[2], "getblockcount"
            ).status,
        )
        ...
        assert_equal(
            403,
            rpccall(
                self.nodes[0], self.strange_users[3], "getbestblockhash"
            ).status,
        )
        ...
        assert_equal(
            200,
            rpccall(
                self.nodes[0], self.strange_users[4], "getblockcount"
            ).status,
        )
