...
class SegWitTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        for i in range(3):
            ...
            p2sh_ms_addr = self.nodes[i].createmultisig(
                1, [self.pubkey[-1]], "p2sh-segwit"
            )["address"]
            bip173_ms_addr = self.nodes[i].createmultisig(
                1, [self.pubkey[-1]], "bech32"
            )["address"]
            ...
            assert_equal(
                self.nodes[i].deriveaddresses(p2sh_ms_desc)[0], p2sh_ms_addr
            )
            assert_equal(
                self.nodes[i].deriveaddresses(bip173_ms_desc)[0], bip173_ms_addr
            )

            sh_wpkh_desc = descsum_create(f"sh(wpkh({key.privkey}))")
            wpkh_desc = descsum_create(f"wpkh({key.privkey})")
            assert_equal(
                self.nodes[i].deriveaddresses(sh_wpkh_desc)[0],
                key.p2sh_p2wpkh_addr,
            )
            assert_equal(
                self.nodes[i].deriveaddresses(wpkh_desc)[0], key.p2wpkh_addr
            )
        ...
        blockhash = self.generate(self.nodes[2], 1)[
            0
        ]  # block 165 (first block with new rules)
        ...
        assert self.nodes[2].getblock(blockhash, False) != self.nodes[
            0
        ].getblock(blockhash, False)
        assert self.nodes[1].getblock(blockhash, False) == self.nodes[
            2
        ].getblock(blockhash, False)
        ...
        for tx_id in segwit_tx_list:
            ...
            ) != self.nodes[0].getrawtransaction(tx_id, False, blockhash)
            assert self.nodes[1].getrawtransaction(
                tx_id, False, blockhash
            ) == self.nodes[2].getrawtransaction(tx_id, False, blockhash)
            assert (
                self.nodes[0].getrawtransaction(tx_id, False, blockhash)
                != self.nodes[2].gettransaction(tx_id)["hex"]
            )
            assert (
                self.nodes[1].getrawtransaction(tx_id, False, blockhash)
                == self.nodes[2].gettransaction(tx_id)["hex"]
            )
            assert (
                self.nodes[0].getrawtransaction(tx_id, False, blockhash)
                == tx.serialize_without_witness().hex()
            )

        # Coinbase contains the witness commitment nonce, check that RPC shows us
        coinbase_txid = self.nodes[2].getblock(blockhash)["tx"][0]
        coinbase_tx = self.nodes[2].gettransaction(
            txid=coinbase_txid, verbose=True
        )
        ...
        assert_equal(
            int(self.nodes[0].getmempoolentry(txid1)["wtxid"], 16),
            tx1.calc_sha256(True),
        )

        # Check that weight and vsize are properly reported in mempool entry (txid1)
        assert_equal(
            self.nodes[0].getmempoolentry(txid1)["vsize"], tx1.get_vsize()
        )
        assert_equal(
            self.nodes[0].getmempoolentry(txid1)["weight"], tx1.get_weight()
        )
        ...
        tx.vout.append(
            CTxOut(
                int(49.99 * COIN), CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE])
            )
        )
        ...
        assert_equal(
            int(self.nodes[0].getmempoolentry(txid2)["wtxid"], 16),
            tx.calc_sha256(True),
        )

        # Check that weight and vsize are properly reported in mempool entry (txid2)
        assert_equal(
            self.nodes[0].getmempoolentry(txid2)["vsize"], tx.get_vsize()
        )
        assert_equal(
            self.nodes[0].getmempoolentry(txid2)["weight"], tx.get_weight()
        )
        ...
        tx.vout.append(
            CTxOut(
                int(49.95 * COIN), CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE])
            )
        )  # Huge fee
        tx.calc_sha256()
        ...
        assert_equal(
            int(self.nodes[0].getmempoolentry(txid3)["wtxid"], 16),
            tx.calc_sha256(True),
        )

        # Check that weight and vsize are properly reported in mempool entry (txid3)
        assert_equal(
            self.nodes[0].getmempoolentry(txid3)["vsize"], tx.get_vsize()
        )
        assert_equal(
            self.nodes[0].getmempoolentry(txid3)["weight"], tx.get_weight()
        )
        ...
        if not self.options.descriptors:
            ...
            uncompressed_spendable_address.append(
                self.nodes[0].addmultisigaddress(
                    2,
                    [
                        uncompressed_spendable_address[0],
                        compressed_spendable_address[0],
            ...
            for i in (
                uncompressed_spendable_address + uncompressed_solvable_address
            ):
                v = self.nodes[0].getaddressinfo(i)
                if v["isscript"]:
                    [
            ...
            assert_equal(
                v1_decoded["vout"][0]["scriptPubKey"]["address"], v1_addr
            )
            assert_equal(
                v1_decoded["vout"][0]["scriptPubKey"]["hex"], "51020305"
            )
            ...
            for use_p2wsh in [False, True]:
                ...
                assert_equal(
                    self.nodes[1].gettransaction(txid, True)["txid"], txid
                )
                assert_equal(
        ...
        self.log.info(
            "Test negative and unknown rpcserialversion throw an init error"
        )
        self.stop_node(0)
        self.nodes[0].assert_start_raises_init_error(
            ["-rpcserialversion=-1"],
            "Error: rpcserialversion must be non-negative.",
        )
        self.nodes[0].assert_start_raises_init_error(
            ["-rpcserialversion=100"],
            "Error: Unknown rpcserialversion requested.",
        )
    ...
    def mine_and_test_listunspent(self, script_list, ismine):
        ...
        tx = CTransaction()
        tx.vin.append(
            CTxIn(COutPoint(int("0x" + utxo["txid"], 0), utxo["vout"]))
        )
        for i in script_list:
            tx.vout.append(CTxOut(10000000, i))
        tx.rehash()
        signresults = self.nodes[0].signrawtransactionwithwallet(
            tx.serialize_without_witness().hex()
        )["hex"]
        txid = self.nodes[0].sendrawtransaction(
            hexstring=signresults, maxfeerate=0
        )
