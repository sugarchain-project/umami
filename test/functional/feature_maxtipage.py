...
class MaxTipAgeTest(SugarchainTestFramework):
    ...
    def test_maxtipage(self, maxtipage, set_parameter=True, test_deltas=True):
        ...
        self.restart_node(
            1, [f"-maxtipage={maxtipage}"] if set_parameter else None
        )
        ...
        if test_deltas:
            ...
            for delta in [5, 4, 3, 2, 1]:
                ...
                assert_equal(
                    node_ibd.getblockchaininfo()["initialblockdownload"], True
                )
        ...
        self.generate(node_miner, 1)
        assert_equal(
            node_ibd.getblockchaininfo()["initialblockdownload"], False
        )
    ...
    def run_test(self):
        ...
        for hours in [20, 10, 5, 2, 1]:
            ...
            self.log.info(
                f"Test IBD with maximum tip age of {hours} hours (-maxtipage={maxtipage})."
            )
            self.test_maxtipage(maxtipage)

        max_long_val = 9223372036854775807
        self.log.info(
            f"Test IBD with highest allowable maximum tip age ({max_long_val})."
        )
