...
class WalletLabelsTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        # each with 1 address with a balance of 50 Sugarchains
        ...
        assert_equal(
            node.listlabels(purpose="receive"),
        ...
        assert_equal(
            node.listlabels(),
            sorted(
                ["coinbase"] + [label.name for label in labels] + ["node2_addr"]
            ),
        )
        ...
        if self.options.descriptors:
            ...
            node.createwallet(
                wallet_name="watch_only", disable_private_keys=True
            )
            ...
            for l in BECH32_VALID:
                ...
                assert_equal(
                    wallet_watch_only.getaddressesbylabel(label=l),
                    {ad: {"purpose": "receive"}},
                )
            ...
            for l in BECH32_INVALID:
                ...
                assert_raises_rpc_error(
                    ...
                    if self.options.descriptors
                    else "Invalid Sugarchain address or script",
...
class Label:
    ...
    def verify(self, node):
        ...
        assert_equal(
            ...
            {
                address: {"purpose": self.purpose[address]}
                for address in self.addresses
            },
        )
