...
class ListDescriptorsTest(SugarchainTestFramework):
    ...
    def run_test(self):
        node = self.nodes[0]
        assert_raises_rpc_error(
            -18, "No wallet is loaded.", node.listdescriptors
        )
        ...
        assert_equal(
            4, len([d for d in result["descriptors"] if d["internal"]])
        )
        ...
        self.log.info(
            "Test that descriptor strings are returned in lexicographically sorted order."
        )
        descriptor_strings = [
            descriptor["desc"] for descriptor in result["descriptors"]
        ]
        ...
        assert_raises_rpc_error(
            ...
            watch_only_wallet.listdescriptors,
            True,
        )
        ...
        wallet.importdescriptors(
            [
                {
                    "desc": descsum_create(
                        "combo(" + node.get_deterministic_priv_key().key + ")"
                    ),
                    "timestamp": TIME_GENESIS_BLOCK,
                }
            ]
