...
"""Test sugarchaind with different proxy configuration.

Test plan:
- Start sugarchaind's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on sugarchaind side:
...
class ProxyTest(SugarchainTestFramework):
    ...
    def node_test(self, node, *, proxies, auth, test_onion, test_cjdns):
        ...
        # Note: sugarchaind's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
    ...
    def run_test(self):
        ...
        for net in NETWORKS:
            ...
            assert_equal(
                n0[net]["proxy_randomize_credentials"], expected_randomize
            )
        ...
        for net in ["ipv4", "ipv6"]:
            assert_equal(
                n1[net]["proxy"], f"{self.conf1.addr[0]}:{self.conf1.addr[1]}"
            )
            assert_equal(n1[net]["proxy_randomize_credentials"], False)
        assert_equal(
            n1["onion"]["proxy"], f"{self.conf2.addr[0]}:{self.conf2.addr[1]}"
        )
        ...
        for net in NETWORKS:
            ...
            assert_equal(
                n2[net]["proxy_randomize_credentials"], expected_randomize
            )
        ...
        if self.have_ipv6:
            ...
            for net in NETWORKS:
                expected_proxy = (
                    "" if net == NET_I2P or net == NET_ONION else proxy
                )
        ...
        for net in NETWORKS:
            ...
            assert_equal(
                n4[net]["proxy_randomize_credentials"], expected_randomize
            )
