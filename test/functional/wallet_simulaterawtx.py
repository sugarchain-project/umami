...
class SimulateTxTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(
            w0.simulaterawtransaction([tx1, tx2])["balance_change"], 0.0
        )
        assert_equal(
            w2.simulaterawtransaction([tx1, tx2])["balance_change"], 5.0
        )

        # w1 should see +15 balance for both transactions
        assert_equal(
            w1.simulaterawtransaction([tx1, tx2])["balance_change"], 15.0
        )
        ...
        sugarchain_fee = Decimal(funding["fee"])

        # w0 sees fee + 5 sugar decrease, w2 sees + 5 sugar
        assert_approx(
            w0.simulaterawtransaction([tx1])["balance_change"],
            -(Decimal("5") + sugarchain_fee),
        )
        assert_approx(
            w2.simulaterawtransaction([tx1])["balance_change"], Decimal("5")
        )
        ...
        tx3 = node.createrawtransaction(
            [{"txid": tx1hex, "vout": tx1vout}], {w0.getnewaddress(): 4.9999}
        )
        # tx4 spends new w1 UTXO paying to w1
        tx4 = node.createrawtransaction(
            [{"txid": tx1hex, "vout": tx1vout}], {w1.getnewaddress(): 4.9999}
        )
        ...
        #       wallet                  tx3                             tx4
        #       w0                      -5 - sugarchain_fee + 4.9999       -5 - sugarchain_fee
        #       w1                      0                               +4.9999
        assert_approx(
            w0.simulaterawtransaction([tx1, tx3])["balance_change"],
            -Decimal("5") - sugarchain_fee + Decimal("4.9999"),
        )
        assert_approx(
            w1.simulaterawtransaction([tx1, tx3])["balance_change"], 0
        )
        assert_approx(
            w0.simulaterawtransaction([tx1, tx4])["balance_change"],
            -Decimal("5") - sugarchain_fee,
        )
        assert_approx(
            w1.simulaterawtransaction([tx1, tx4])["balance_change"],
            Decimal("4.9999"),
        )
        ...
        tx2 = funding["hex"]
        sugarchain_fee2 = Decimal(funding["fee"])
        assert_approx(
            w0.simulaterawtransaction([tx2])["balance_change"],
            -(Decimal("10") + sugarchain_fee2),
        )
        assert_approx(
            w1.simulaterawtransaction([tx2])["balance_change"], +(Decimal("10"))
        )
