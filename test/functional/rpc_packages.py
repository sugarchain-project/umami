...
class RPCPackagesTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        self.generate(
            self.wallet, COINBASE_MATURITY + 100
        ...
        for _ in range(3):
            tx_hex = self.wallet.create_self_transfer(
    ...
    def test_multiple_parents(self):
        ...
        for num_parents in [2, 10, 24]:
            ...
            child_tx = self.wallet.create_self_transfer_multi(
                utxos_to_spend=parent_coins, fee_per_output=2000
            )
            for _ in range(10):
    ...
    def test_rbf(self):
        ...
        replaceable_tx = self.wallet.create_self_transfer(
            utxo_to_spend=coin, sequence=MAX_BIP125_RBF_SEQUENCE, fee=fee
        )
        ...
        assert_equal(
            testres_replaceable["vsize"], replaceable_tx["tx"].get_vsize()
        )
        assert_equal(testres_replaceable["fees"]["base"], fee)
        assert_fee_amount(
            fee,
            replaceable_tx["tx"].get_vsize(),
            testres_replaceable["fees"]["effective-feerate"],
        )
        assert_equal(
            testres_replaceable["fees"]["effective-includes"],
            [replaceable_tx["wtxid"]],
        )

        # Replacement transaction is identical except has double the fee
        replacement_tx = self.wallet.create_self_transfer(
            utxo_to_spend=coin, sequence=MAX_BIP125_RBF_SEQUENCE, fee=2 * fee
        )
        testres_rbf_conflicting = node.testmempoolaccept(
            [replaceable_tx["hex"], replacement_tx["hex"]]
        )
        assert_equal(
            testres_rbf_conflicting,
            [
                {
                    "txid": replaceable_tx["txid"],
                    "wtxid": replaceable_tx["wtxid"],
                    "package-error": "conflict-in-package",
                },
                {
                    "txid": replacement_tx["txid"],
                    "wtxid": replacement_tx["wtxid"],
                    "package-error": "conflict-in-package",
                },
            ],
        )
        ...
        self.wallet.sendrawtransaction(
            from_node=node, tx_hex=replaceable_tx["hex"]
        )
        ...
        self.assert_testres_equal(
            self.independent_txns_hex + [replacement_tx["hex"]],
            testres_rbf_package,
        )
    ...
    def assert_equal_package_results(
        ...
        for testres_tx in testmempoolaccept_result:
            ...
            submitres_tx = submitpackage_result["tx-results"][
                testres_tx["wtxid"]
            ]
    ...
    def test_submit_child_with_parents(self, num_parents, partial_submit):
        ...
        child_tx = self.wallet.create_self_transfer_multi(
            utxos_to_spend=[tx["new_utxo"] for tx in package_txns],
    ...
    def test_submit_cpfp(self):
        ...
        child_result = submitpackage_result["tx-results"][
            tx_child["tx"].getwtxid()
        ]
        ...
        assert_equal(
            [tx_poor["wtxid"], tx_child["tx"].getwtxid()],
    ...
    def test_submitpackage(self):
        ...
        chain_hex = [
            t["hex"]
            for t in self.wallet.create_self_transfer_chain(chain_length=25)
        ]
        assert_raises_rpc_error(
