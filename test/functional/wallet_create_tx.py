...
class CreateTxWalletTest(SugarchainTestFramework):
    ...
    def test_tx_size_too_large(self):
        ...
        outputs = {
            self.nodes[0].getnewaddress(address_type="bech32"): 0.000025
            for _ in range(400)
        }
    ...
    def test_create_too_long_mempool_chain(self):
        ...
        tx_data = df_wallet.send(
            outputs=[{test_wallet.getnewaddress(): 25}],
        ...
        for i in range(1, 25):
            ...
            tx_data = test_wallet.send(
                outputs=[{test_wallet.getnewaddress(): 25 - i}], options=options
            )
            txid = tx_data["txid"]
        ...
        assert_raises_rpc_error(
            ...
            test_wallet.send,
            outputs=[{test_wallet.getnewaddress(): 0.3}],
            options=options,
        )
