...
def dump_leaf_page(data):
    ...
    (
        ...
        level,
        pg_type,
    ) = struct.unpack("QIIIHHBB", page_header)
    page_info["pgno"] = pgno
    ...
    assert (
        pg_type == BTREE_LEAF
    ), "A non-btree leaf page has been encountered while dumping leaves"
...
def dump_meta_page(page):
    ...
    (
        ...
        flags,
        uid,
    ) = struct.unpack("QIIIIBBBBIIIIII20s", meta_page)
    metadata["pgno"] = pgno
    ...
    (
        ...
        iv,
        chksum,
    ) = struct.unpack("IIIII368sI12s16s20s", btree_meta_page)
