...
This file is modified from python-sugarchainlib.
"""
...
def bn2vch(v):
    """Convert number to sugarchain-specific little endian format."""
...
class CScript(bytes):
    ...
    def IsWitnessProgram(self):
        ...
        return (
            (4 <= len(self) <= 42)
            and (self[0] == OP_0 or (OP_1 <= self[0] <= OP_16))
            and (self[1] + 2 == len(self))
        )
...
class TestFrameworkScript(unittest.TestCase):
    def test_bn2vch(self):
        ...
        self.assertEqual(
            bn2vch(0x80000000), bytes([0x00, 0x00, 0x00, 0x80, 0x00])
        )
        self.assertEqual(
            bn2vch(-0x80000000), bytes([0x00, 0x00, 0x00, 0x80, 0x80])
        )
        self.assertEqual(
            bn2vch(0xFFFFFFFF), bytes([0xFF, 0xFF, 0xFF, 0xFF, 0x00])
        )
...
def TaprootSignatureMsg(
    ...
    assert (
        len(ss)
        == 175
        - (in_type == SIGHASH_ANYONECANPAY) * 49
...
def taproot_tree_helper(scripts):
    ...
    if len(scripts) == 1:
        ...
        left = [
            (name, version, script, control + right_h, leaf)
            for name, version, script, control, leaf in left
        ]
        right = []
    else:
        ...
        right = [
            (name, version, script, control + left_h, leaf)
            for name, version, script, control, leaf in right
        ]
    if right_h < left_h:
...
TaprootInfo = namedtuple(
    "TaprootInfo",
    "scriptPubKey,internal_pubkey,negflag,tweak,leaves,merkle_root,output_pubkey",
)
...
TaprootLeafInfo = namedtuple(
    "TaprootLeafInfo", "script,version,merklebranch,leaf_hash"
)
...
def taproot_construct(pubkey, scripts=None, treat_internal_as_infinity=False):
    ...
    leaves = dict(
        (name, TaprootLeafInfo(script, version, merklebranch, leaf))
        for name, version, script, merklebranch, leaf in ret
    )
    return TaprootInfo(
        CScript([OP_1, tweaked]), pubkey, negated + 0, tweak, leaves, h, tweaked
    )
...
def is_op_success(o):
    return (
        ...
        or o == 0x8D
        or o == 0x8E
        or (o >= 0x7E and o <= 0x81)
        or (o >= 0x83 and o <= 0x86)
        or (o >= 0x95 and o <= 0x99)
        or (o >= 0xBB and o <= 0xFE)
    )
