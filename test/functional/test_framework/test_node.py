...
class TestNode:
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            ...
            try:
                rpc = get_rpc_proxy(
                    ...
                    // 2,  # Shorter timeout to allow for one retry in case of ETIMEDOUT
        ...
        self._raise_assertion_error(
            "Unable to connect to sugarchaind after {}s".format(
                self.rpc_timeout
            )
        )
    ...
    def get_wallet_rpc(self, wallet_name):
        if self.use_cli:
            return RPCOverloadWrapper(
                self.cli("-rpcwallet={}".format(wallet_name)),
                True,
                self.descriptors,
            )
        else:
    ...
    def assert_debug_log(self, expected_msgs, unexpected_msgs=None, timeout=2):
        ...
        while True:
            ...
            for unexpected_msg in unexpected_msgs:
                if re.search(
                    re.escape(unexpected_msg), log, flags=re.MULTILINE
                ):
                    self._raise_assertion_error(
    ...
    def wait_for_debug_log(self, expected_msgs, timeout=60):
        ...
        while True:
            ...
            if time.time() >= time_end:
                print_log = " - " + "\n - ".join(
                    log.decode("utf8", errors="replace").splitlines()
                )
                break
            ...
            try:
                ...
                if expected_msg is not None:
                    ...
                    if match == ErrorMatch.PARTIAL_REGEX:
                        if (
                            re.search(expected_msg, stderr, flags=re.MULTILINE)
                            is None
                        ):
                            self._raise_assertion_error(
                ...
                self.running = False
                self.process = None
                assert_msg = f"sugarchaind should have exited within {self.rpc_timeout}s "
    ...
    def add_p2p_connection(self, p2p_conn, *, wait_for_verack=True, **kwargs):
        ...
        p2p_conn.wait_until(
            lambda: p2p_conn.is_connected, check_connected=False
        )
        if wait_for_verack:
    ...
    def add_outbound_p2p_connection(
        ...
        **kwargs,
        ...
        def addconnection_callback(address, port):
            self.log.debug(
                "Connecting to %s:%d %s" % (address, port, connection_type)
            )
