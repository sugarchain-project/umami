...
TMPDIR_PREFIX = "sugarchain_func_test_"
...
class SugarchainTestMetaClass(type):
    """Metaclass for SugarchainTestFramework.

    Ensures that any attempt to register a subclass of `SugarchainTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == "SugarchainTestFramework":
            if not ("run_test" in dct and "set_test_params" in dct):
                raise TypeError(
                    "SugarchainTestFramework subclasses must override "
                    "'run_test' and 'set_test_params'"
                )
            if "__init__" in dct or "main" in dct:
                raise TypeError(
                    "SugarchainTestFramework subclasses may not override "
                    "'__init__' or 'main'"
                )
...
class SugarchainTestFramework(metaclass=SugarchainTestMetaClass):
    """Base class for a sugarchain test script.
    ...
    def __init__(self):
        ...
        self.parse_args()
        self.disable_syscall_sandbox = (
            self.options.nosandbox or self.options.valgrind
        )
        self.default_wallet_name = (
            "default_wallet" if self.options.descriptors else ""
        )
        ...
        assert (
            self.wallet_names is None
            or len(self.wallet_names) <= self.num_nodes
        )
    ...
    def main(self):
        ...
        try:
            ...
            self.log.exception(
                "Called Process failed with '{}'".format(e.output)
            )
            self.success = TestStatus.FAILED
        except Exception:
    ...
    def parse_args(self):
        ...
        parser.add_argument(
            ...
            and bool(os.listdir(previous_releases_path)),
    ...
    def setup(self):
        ...
        fname_sugarchaind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "sugarchaind" + config["environment"]["EXEEXT"],
        )
        fname_sugarchaincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "sugarchain-cli" + config["environment"]["EXEEXT"],
        )
        fname_sugarchainutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "sugarchain-util" + config["environment"]["EXEEXT"],
        )
        self.options.sugarchaind = os.getenv(
            "BITCOIND", default=fname_sugarchaind
        )
        self.options.sugarchaincli = os.getenv(
            "BITCOINCLI", default=fname_sugarchaincli
        )
        self.options.sugarchainutil = os.getenv(
            "BITCOINUTIL", default=fname_sugarchainutil
        )
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info(
                "Note: sugarchainds were not stopped and may still be running"
            )
        ...
        if self.success == TestStatus.PASSED:
            ...
            self.log.error("")
            self.log.error(
                "If this failure happened unexpectedly or intermittently, please file a bug and provide a link or upload of the combined log."
            )
        ...
        rpc_logger = logging.getLogger("SugarchainRPC")
    ...
    def add_wallet_options(self, parser, *, descriptors=True, legacy=True):
        ...
        group = parser.add_mutually_exclusive_group(
            ...
            required=os.getenv("REQUIRE_WALLET_TYPE_SET") == "1"
        ...
        if legacy:
            group.add_argument(
                ...
                const=False,
                **kwargs,
                help="Run test using legacy wallets",
                dest="descriptors"
            )
    ...
    def add_nodes(
        ...
        *,
        ...
        if (
            self.is_syscall_sandbox_compiled()
            and not self.disable_syscall_sandbox
        ):
        ...
        if binary is None:
            binary = [
                get_bin_from_version(v, "sugarchaind", self.options.sugarchaind)
                for v in versions
            ]
        if binary_cli is None:
            binary_cli = [
                get_bin_from_version(
                    v, "sugarchain-cli", self.options.sugarchaincli
                )
                for v in versions
            ]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                sugarchaind=binary[i],
                sugarchain_cli=binary_cli[i],
                version=versions[i],
    ...
    def stop_node(self, i, expected_stderr="", wait=0):
        """Stop a sugarchaind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple sugarchaind test nodes"""
        for node in self.nodes:
    ...
    def connect_nodes(self, a, b):
        ...
        self.wait_until(
            lambda: sum(
                peer["version"] != 0 for peer in from_connection.getpeerinfo()
            )
            == from_num_peers
        )
        self.wait_until(
            lambda: sum(
                peer["version"] != 0 for peer in to_connection.getpeerinfo()
            )
            == to_num_peers
        )
        self.wait_until(
    ...
    def disconnect_nodes(self, a, b):
        def disconnect_nodes_helper(node_a, node_b):
            ...
            if not peer_ids:
                self.log.warning(
                    "disconnect_nodes: {} and {} were not connected".format(
                        node_a.index,
                        node_b.index,
                    )
                )
            ...
            # wait to disconnect
            self.wait_until(
                lambda: not get_peer_ids(node_a, node_b.index), timeout=5
            )
            self.wait_until(
                lambda: not get_peer_ids(node_b, node_a.index), timeout=5
            )
    ...
    def generatetoaddress(self, generator, *args, sync_fun=None, **kwargs):
        blocks = generator.generatetoaddress(
            *args, invalid_call=False, **kwargs
        )
        sync_fun() if sync_fun else self.sync_all()
        return blocks

    def generatetodescriptor(self, generator, *args, sync_fun=None, **kwargs):
        blocks = generator.generatetodescriptor(
            *args, invalid_call=False, **kwargs
        )
        sync_fun() if sync_fun else self.sync_all()
        return blocks
    ...
    def sync_blocks(self, nodes=None, wait=1, timeout=60):
        ...
        raise AssertionError(
            "Block sync timed out after {}s:{}".format(
                timeout,
    ...
    def sync_mempools(
        self, nodes=None, wait=1, timeout=60, flush_scheduler=True
    ):
        """
        ...
        while time.time() <= stop_time:
            ...
            time.sleep(wait)
        raise AssertionError(
            "Mempool sync timed out after {}s:{}".format(
                timeout,
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as sugarchaind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    sugarchaind=self.options.sugarchaind,
                    sugarchain_cli=self.options.sugarchaincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                )
            ...
            for entry in os.listdir(cache_path()):
                if entry not in [
                    ...
                    "blocks",
                    "indexes",
                ]:  # Only indexes, chainstate and blocks folders
        ...
        for i in range(self.num_nodes):
            ...
            )  # Overwrite port/rpcport in sugarchain.conf
    ...
    def skip_if_no_sugarchaind_tracepoints(self):
        """Skip the running test if sugarchaind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest(
                "sugarchaind has not been built with USDT tracepoints enabled."
            )
    ...
    def skip_if_no_bpf_permissions(self):
        ...
        if os.geteuid() != 0:
            raise SkipTest(
                "no permissions to use BPF (please review the tests carefully before running them with higher privileges)"
            )
    ...
    def skip_if_no_sugarchaind_zmq(self):
        """Skip the running test if sugarchaind has not been compiled with zmq support."""
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if sugarchain-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("sugarchain-wallet has not been compiled")

    def skip_if_no_sugarchain_util(self):
        """Skip the running test if sugarchain-util has not been compiled."""
        if not self.is_sugarchain_util_compiled():
            raise SkipTest("sugarchain-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if sugarchain-cli has not been compiled."""
    ...
    def has_previous_releases(self):
        ...
        if not os.path.isdir(self.options.previous_releases_path):
            if self.options.prev_releases:
                raise AssertionError(
                    "Force test of previous releases but releases missing: {}".format(
                        self.options.previous_releases_path
                    )
                )
    ...
    def is_cli_compiled(self):
        """Checks whether sugarchain-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether sugarchain-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_sugarchain_util_compiled(self):
        """Checks whether sugarchain-util was compiled."""
