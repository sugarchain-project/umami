...
"""HTTP proxy for opening RPC connection to sugarchaind.
...
log = logging.getLogger("SugarchainRPC")
...
class AuthServiceProxy:
    ...
    def __init__(
        ...
        user = (
            None
            if self.__url.username is None
            else self.__url.username.encode("utf8")
        )
        passwd = (
            None
            if self.__url.password is None
            else self.__url.password.encode("utf8")
        )
            ...
            raise JSONRPCException(
                {
                    ...
                    "longer to return."
                    % (self._service_name, self.__conn.timeout),
                }
            )
