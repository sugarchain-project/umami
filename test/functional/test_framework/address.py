...
"""Encode and decode Sugarchain addresses.
...
def create_deterministic_address_bcrt1_p2tr_op_true():
    ...
    address = output_key_to_p2tr(
        taproot_construct(
            internal_key, [(None, CScript([OP_TRUE]))]
        ).output_pubkey
    )
    assert_equal(
        address,
...
def address_to_scriptpubkey(address):
    ...
    if version is not None:
        return program_to_witness_script(
            version, payload
        )  # testnet segwit scriptpubkey
...
class TestFrameworkScript(unittest.TestCase):
    ...
    def test_bech32_decode(self):
        def check_bech32_decode(payload, version):
            hrp = "tb"
            self.assertEqual(
                bech32_to_bytes(encode_segwit_address(hrp, version, payload)),
                (version, payload),
            )

        check_bech32_decode(
            bytes.fromhex("36e3e2a33f328de12e4b43c515a75fba2632ecc3"), 0
        )
        check_bech32_decode(
            bytes.fromhex("823e9790fc1d1782321140d4f4aa61aabd5e045b"), 0
        )
        check_bech32_decode(
            bytes.fromhex(
        ...
        check_bech32_decode(
            bytes.fromhex(
                "39cf8ebd95134f431c39db0220770bd127f5dd3cc103c988b7dcd577ae34e354"
            ),
            1,
        )
        check_bech32_decode(
            bytes.fromhex(
                "708244006d27c757f6f1fc6f853b6ec26268b727866f7ce632886e34eb5839a3"
            ),
            1,
        )
        check_bech32_decode(
            bytes.fromhex("616211ab00dffe0adcb6ce258d6d3fd8cbd901e2"), 0
        )
        check_bech32_decode(
            bytes.fromhex("b6a7c98b482d7fb21c9fa8e65692a0890410ff22"), 0
        )
        check_bech32_decode(
            bytes.fromhex("f0c2109cb1008cfa7b5a09cc56f7267cd8e50929"), 0
        )
