...
class P2PConnection(asyncio.Protocol):
    ...
    def peer_connect(self, dstaddr, dstport, *, net, timeout_factor):
        ...
        logger.debug(
            "Connecting to Sugarchain Node: %s:%d"
    ...
    def connection_made(self, transport):
        ...
        logger.debug(
            "Connected & Listening: %s:%d" % (self.dstaddr, self.dstport)
        )
    ...
    def connection_lost(self, exc):
        ...
        if exc:
            ...
            logger.debug(
                "Closed connection to: %s:%d" % (self.dstaddr, self.dstport)
            )
        self._transport = None
        self.recvbuf = b""
        self.on_close()
    ...
    def _on_data(self):
        ...
        try:
            while True:
                ...
                msglen = struct.unpack("<i", self.recvbuf[4 + 12 : 4 + 12 + 4])[
                    0
                ]
...
class P2PInterface(P2PConnection):
    ...
    def peer_connect_send_version(self, services):
        ...
        vt.addrFrom.port = 0
        self.on_connection_send_msg = (
            vt  # Will be sent in connection_made callback
        )

    def peer_connect(
        self, *args, services=P2P_SERVICES, send_version=True, **kwargs
    ):
    ...
    def wait_for_merkleblock(self, blockhash, timeout=60):
        def test_function():
            ...
            if not last_filtered_block:
                return False
            return last_filtered_block.merkleblock.header.rehash() == int(
                blockhash, 16
            )
...
class NetworkThread(threading.Thread):
    ...
    async def create_listen_server(cls, addr, port, callback, proto):
        ...
        if (addr, port) not in cls.listeners:
            ...
            logger.debug(
                "Listening server on %s:%d should be started" % (addr, port)
            )
    ...
    def send_blocks_and_test(
        ...
        *,
        ...
        with node.assert_debug_log(expected_msgs=reject_reason):
            ...
            if success:
                self.wait_until(
                    lambda: node.getbestblockhash() == blocks[-1].hash,
                    timeout=timeout,
                )
            else:
        ...
        *,
        ...
        with node.assert_debug_log(expected_msgs=reject_reason):
            ...
            if success:
                ...
                for tx in txs:
                    assert (
                        tx.hash in raw_mempool
                    ), "{} not found in mempool".format(tx.hash)
            else:
                # Check that none of the txs are now in the mempool
                for tx in txs:
                    assert (
                        tx.hash not in raw_mempool
                    ), "{} tx found in mempool".format(tx.hash)
