...
class BIP68Test(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info(
            "Note that nVersion=2 transactions are always standard (independent of BIP68 activation status)."
        )
    ...
    def test_disable_flag(self):
        ...
        utxo = self.wallet.send_self_transfer(from_node=self.nodes[0])[
            "new_utxo"
        ]
        ...
        tx2.vout = [
            CTxOut(int(value - self.relayfee * COIN), SCRIPT_W0_SH_OP_TRUE)
        ]
        tx2.rehash()
    ...
    def test_sequence_lock_confirmed_inputs(self):
        ...
        for _ in range(400):
            ...
            for j in range(num_inputs):
                ...
                if random.randint(0, 1):
                    ...
                    if random.randint(0, 1) and can_time_lock:
                        ...
                        time_delta = (
                            sequence_value << SEQUENCE_LOCKTIME_GRANULARITY
                        )
                        if (
                            input_will_pass
            ...
            tx.vout.append(
                CTxOut(
                    int(value - self.relayfee * tx_size * COIN / 1000),
                    SCRIPT_W0_SH_OP_TRUE,
                )
            )
    ...
    def test_sequence_lock_unconfirmed_inputs(self):
        ...
        tx2.vout = [
            CTxOut(
                int(tx1.vout[0].nValue - self.relayfee * COIN),
                SCRIPT_W0_SH_OP_TRUE,
            )
        ]
        ...
        def test_nonzero_locks(orig_tx, node, relayfee, use_height_lock):
            ...
            tx.vout = [
                CTxOut(
                    int(orig_tx.vout[0].nValue - relayfee * COIN),
                    SCRIPT_W0_SH_OP_TRUE,
                )
            ]
            tx.rehash()
        ...
        self.nodes[0].prioritisetransaction(
            txid=tx2.hash, fee_delta=int(-self.relayfee * COIN)
        )
    ...
    def test_bip68_not_consensus(self):
        ...
        tx3.vout = [
            CTxOut(
                int(tx2.vout[0].nValue - self.relayfee * COIN),
                SCRIPT_W0_SH_OP_TRUE,
            )
        ]
        tx3.rehash()
    ...
    def activateCSV(self):
        ...
        self.generate(
            self.wallet, min_activation_height - height - 2, sync_fun=self.no_op
        )
        assert not softfork_active(self.nodes[0], "csv")
        self.generate(self.wallet, 1, sync_fun=self.no_op)
