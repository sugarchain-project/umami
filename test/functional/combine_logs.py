...
"""Combine logs from multiple sugarchain nodes as well as the test_framework log.
...
def main():
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "temporary test directory to combine logs from. "
    ...
    if not args.testdir:
        print(
            "Opening latest test directory: {}".format(testdir), file=sys.stderr
        )
...
def read_logs(tmp_dir):
    ...
    if path:
        assert (
            next(glob, None) is None
        )  #  more than one debug.log, should never happen
        chain = re.search(r"node0/(.+?)/debug\.log$", path.as_posix()).group(
            1
...
def get_log_events(source, logfile):
    ...
    try:
        with open(logfile, "r", encoding="utf-8") as infile:
            ...
            yield LogEvent(
                timestamp=timestamp, source=source, event=event.rstrip()
            )
    except FileNotFoundError:
        print(
            "File %s could not be opened. Continuing without it." % logfile,
            file=sys.stderr,
        )
...
def print_logs_html(log_events):
    ...
    print(
        ...
        .render(
            title="Combined Logs from testcase",
