...
class SendTxRcnclTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        peer = self.nodes[0].add_p2p_connection(
            SendTxrcnclReceiver(), send_version=True, wait_for_verack=True
        )
        ...
        peer = self.nodes[0].add_p2p_connection(
            PeerTrackMsgOrder(), send_version=True, wait_for_verack=True
        )
        peer.wait_for_verack()
        verack_index = [
            i for i, msg in enumerate(peer.messages) if msg.msgtype == b"verack"
        ][0]
        sendtxrcncl_index = [
            i
            for i, msg in enumerate(peer.messages)
            if msg.msgtype == b"sendtxrcncl"
        ][0]
        ...
        peer = self.nodes[0].add_p2p_connection(
            SendTxrcnclReceiver(), send_version=False, wait_for_verack=False
        )
        ...
        self.log.info(
            "SENDTXRCNCL for fRelay=false should not be sent (with NODE_BLOOM offered)"
        )
        ...
        peer = self.nodes[0].add_outbound_p2p_connection(
            P2PFeelerReceiver(), p2p_idx=0, connection_type="feeler"
        )
        assert not peer.sendtxrcncl_msg_received
        self.nodes[0].disconnect_p2ps()
        ...
        self.log.info(
            "SENDTXRCNCL not sent if -txreconciliation flag is not set"
        )
        self.restart_node(0, [])
        peer = self.nodes[0].add_p2p_connection(
            SendTxrcnclReceiver(), send_version=True, wait_for_verack=True
        ...
        peer = self.nodes[0].add_p2p_connection(
            PeerNoVerack(), send_version=True, wait_for_verack=False
        )
        ...
        with self.nodes[0].assert_debug_log(
            [
                "(sendtxrcncl received from already registered peer); disconnecting"
            ]
        ):
            peer.send_message(create_sendtxrcncl_msg())
        ...
        with self.nodes[0].assert_debug_log(
            ["ignored, as our node does not have txreconciliation enabled"]
        ):
            peer.send_message(create_sendtxrcncl_msg())
        self.nodes[0].disconnect_p2ps()
        ...
        with self.nodes[0].assert_debug_log(
            ["txreconciliation protocol violation"]
        ):
        ...
        peer = self.nodes[0].add_p2p_connection(
            PeerNoVerack(), send_version=False, wait_for_verack=False
        )
        ...
        with self.nodes[0].assert_debug_log(
            ["Ignore unexpected txreconciliation signal"]
        ):
            peer.send_message(create_sendtxrcncl_msg())
        self.nodes[0].disconnect_p2ps()

        self.log.info(
            "sending SENDTXRCNCL after sending VERACK triggers a disconnect"
        )
        peer = self.nodes[0].add_p2p_connection(P2PInterface())
        with self.nodes[0].assert_debug_log(
            ["sendtxrcncl received after verack"]
        ):
            peer.send_message(create_sendtxrcncl_msg())
            peer.wait_for_disconnect()

        self.log.info(
            "SENDTXRCNCL without WTXIDRELAY is ignored (recon state is erased after VERACK)"
        )
        peer = self.nodes[0].add_p2p_connection(
            PeerNoVerack(wtxidrelay=False),
            send_version=True,
            wait_for_verack=False,
        )
        with self.nodes[0].assert_debug_log(
            ["Forget txreconciliation state of peer"]
        ):
            peer.send_message(create_sendtxrcncl_msg())
        ...
        with self.nodes[0].assert_debug_log(
            ["we indicated no tx relay; disconnecting"]
        ):
            peer.send_message(create_sendtxrcncl_msg())
            peer.wait_for_disconnect()
