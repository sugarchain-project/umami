...
class InvalidMessagesTest(SugarchainTestFramework):
    ...
    def test_magic_bytes(self):
        ...
        conn = self.nodes[0].add_p2p_connection(P2PDataStore())
        with self.nodes[0].assert_debug_log(
            ["Header error: Wrong MessageStart ffffffff received"]
        ):
    ...
    def test_checksum(self):
        ...
        with self.nodes[0].assert_debug_log(
            [
                "Header error: Wrong checksum (badmsg, 2 bytes), expected 78df0a04 was ffffffff"
            ]
        ):
            msg = conn.build_message(msg_unrecognized(str_data="d"))
    ...
    def test_size(self):
        ...
        conn = self.nodes[0].add_p2p_connection(P2PDataStore())
        with self.nodes[0].assert_debug_log(
            ["Header error: Size too large (badmsg, 4000001 bytes)"]
        ):
    ...
    def test_msgtype(self):
        ...
        conn = self.nodes[0].add_p2p_connection(P2PDataStore())
        with self.nodes[0].assert_debug_log(
            ["Header error: Invalid message type"]
        ):
    ...
    def test_addrv2(self, label, required_log_messages, raw_addrv2):
        ...
        # Make sure sugarchaind signals support for ADDRv2, otherwise this test
    ...
    def test_addrv2_too_long_address(self):
        self.test_addrv2(
            ...
            bytes.fromhex(
                "01"
                + "61bc6649"  # number of entries
                + "00"  # time, Fri Jan  9 02:54:25 UTC 2009
                + "01"  # service flags, COMPACTSIZE(NODE_NONE)
                + "fd0102"  # network type (IPv4)
                + "ab" * 513  # address length (COMPACTSIZE(513))
                + "208d"  # address
            ),
        )  # port
    ...
    def test_addrv2_unrecognized_network(self):
        ...
        self.test_addrv2(
            ...
            bytes.fromhex(
                ...
                now_hex
                + "01"  # time
                + "99"  # service flags, COMPACTSIZE(NODE_NETWORK)
                + "02"  # network type (unrecognized)
                + "ab" * 2  # address length (COMPACTSIZE(2))
                + "208d"  # address
                +  # port
                ...
                + "04"  # network type (IPv4)
                + "09" * 4  # address length (COMPACTSIZE(4))
                + "208d"  # address
            ),
        )  # port
    ...
    def test_oversized_msg(self, msg, size):
        ...
        with self.nodes[0].assert_debug_log(
            ["Misbehaving", "{} message size = {}".format(msg_type, size)]
        ):
    ...
    def test_invalid_pow_headers_msg(self):
        ...
        blockheader_tip = from_hex(
            CBlockHeader(),
            self.nodes[0].getblockheader(blockheader_tip_hash, False),
        )
    ...
    def test_resource_exhaustion(self):
        ...
        self.log.info(
            "(a) Send 80 messages, each of maximum valid data size (4MB)"
        )
        for _ in range(80):
