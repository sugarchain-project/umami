...
def call_with_auth(node, user, password):
    ...
    headers = {
        "Authorization": "Basic "
        + str_to_b64str("{}:{}".format(user, password))
    }
...
class HTTPBasicsTest(SugarchainTestFramework):
    ...
    def setup_chain(self):
        super().setup_chain()
        # Append rpcauth to sugarchain.conf before initialization
        ...
        with open(
            os.path.join(
                get_datadir_path(self.options.tmpdir, 1), "sugarchain.conf"
            ),
            "a",
            encoding="utf8",
        ) as f:
    ...
    def test_auth(self, node, user, password):
        ...
        self.log.info("Wrong...")
        assert_equal(
            401, call_with_auth(node, user + "wrong", password + "wrong").status
        )
