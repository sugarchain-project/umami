...
def default_controlblock(ctx):
    ...
    return (
        bytes([get(ctx, "leafversion") + get(ctx, "negflag")])
        + get(ctx, "pubkey_internal")
        + get(ctx, "merklebranch")
    )
    ...
    if mode == "taproot":
        ...
        if get(ctx, "leaf") is not None:
            ...
            return TaprootSignatureMsg(
                ...
                script=script,
                leaf_ver=leaf_ver,
                codeseparator_pos=codeseppos,
                annex=annex,
            )
        else:
            return TaprootSignatureMsg(
                tx, utxos, hashtype, idx, scriptpath=False, annex=annex
            )
    elif mode == "witv0":
        # BIP143 signature hash
        scriptcode = get(ctx, "scriptcode")
        utxos = get(ctx, "utxos")
        return SegwitV0SignatureMsg(
            scriptcode, tx, idx, hashtype, utxos[idx].nValue
        )
    else:
...
def default_witness_taproot(ctx):
    ...
    if get(ctx, "leaf") is None:
        ...
        return (
            get(ctx, "inputs")
            + [bytes(get(ctx, "script_taproot")), get(ctx, "controlblock")]
            + suffix_annex
        )
...
def spend(tx, idx, utxos, **kwargs):
    ...
    scriptsig = CScript(
        b"".join(bytes(to_script(elem)) for elem in scriptsig_list)
    )
...
Spender = namedtuple(
    "Spender",
    "script,comment,is_standard,sat_function,err_msg,sigops_weight,no_fail,need_vin_vout_mismatch",
)
...
def make_spender(
    ...
    *,
    ...
    pkh=None,
    p2sh=False,
    spk_mutate_pre_p2sh=None,
    failure=None,
    standard=True,
    err_msg=None,
    sigops_weight=0,
    need_vin_vout_mismatch=False,
    **kwargs
):
    ...
    * need_vin_vout_mismatch: whether this test requires being tested in a transaction input that has no corresponding
    ...
    return Spender(
        ...
        sat_function=sat_fn,
        err_msg=err_msg,
        sigops_weight=sigops_weight,
        no_fail=failure is None,
        need_vin_vout_mismatch=need_vin_vout_mismatch,
    )
...
def bitflipper(expr):
    ...
    def fn(ctx):
        ...
        return (
            int.from_bytes(sub, "little")
            ^ (1 << random.randrange(len(sub) * 8))
        ).to_bytes(len(sub), "little")
...
ERR_TAPSCRIPT_CHECKMULTISIG = {
    "err_msg": "OP_CHECKMULTISIG(VERIFY) is not available in tapscript"
}
ERR_MINIMALIF = {"err_msg": "OP_IF/NOTIF argument must be minimal in tapscript"}
ERR_UNKNOWN_PUBKEY = {
    "err_msg": "Public key is neither compressed or uncompressed"
}
...
ERR_SIGOPS_RATIO = {
    "err_msg": "Too much signature validation relative to witness weight"
}
ERR_UNDECODABLE = {"err_msg": "Opcode missing or not understood"}
ERR_NO_SUCCESS = {
    "err_msg": "Script evaluated without error but finished with a false/empty top stack element"
}
...
VALID_SIGHASHES_TAPROOT_NO_SINGLE = [
    h
    for h in VALID_SIGHASHES_TAPROOT
    if h not in VALID_SIGHASHES_TAPROOT_SINGLE
]
...
def spenders_taproot_active():
    ...
    add_spender(
        ...
        tap=tap,
        key=secs[0],
        failure={"key_tweaked": bitflipper(default_key_tweaked)},
        **ERR_SIG_SCHNORR
    )
    ...
    add_spender(
        ...
        tap=tap,
        key=secs[0],
        failure={"signature": bitflipper(default_signature)},
        **ERR_SIG_SCHNORR
    )
    ...
    tap = taproot_construct(
        invalid_pub,
        [("true", CScript([OP_1]))],
        treat_internal_as_infinity=True,
    )
    add_spender(
        spenders,
        "output/invalid_x",
        tap=tap,
        key_tweaked=tap.tweak,
        failure={"leaf": "true", "inputs": []},
    ...
    for annex in [
        None,
        lambda _: bytes([ANNEX_TAG]) + random_bytes(random.randrange(0, 250)),
    ]:
        ...
        for hashtype in VALID_SIGHASHES_TAPROOT:
            ...
            add_spender(
                spenders,
                "sighash/keypath_hashtype_%x" % hashtype,
                tap=tap,
                key=secs[0],
                **common,
                **SIGHASH_BITFLIP,
            ...
            add_spender(
                spenders,
                "sighash/scriptpath_hashtype_%x" % hashtype,
                tap=tap,
                leaf="s0",
                key=secs[1],
                **common,
            ...
            if hashtype in VALID_SIGHASHES_TAPROOT_SINGLE:
                add_spender(
                    spenders,
                    "sighash/keypath_hashtype_mis_%x" % hashtype,
                    tap=tap,
                    key=secs[0],
                    annex=annex,
                    standard=no_annex,
                    hashtype_actual=random.choice(
                        VALID_SIGHASHES_TAPROOT_NO_SINGLE
                    ),
                    failure={"hashtype_actual": hashtype},
                    **ERR_SIG_HASHTYPE,
                    need_vin_vout_mismatch=True
                )
                add_spender(
                    spenders,
                    "sighash/scriptpath_hashtype_mis_%x" % hashtype,
                    tap=tap,
                    leaf="s0",
                    key=secs[1],
                    annex=annex,
        ...
        scripts = [
            (
                "pk_codesep",
                CScript(
                    random_checksig_style(pubs[1]) + bytes([OP_CODESEPARATOR])
                ),
            ),  # codesep after checksig
            (
                "codesep_pk",
                CScript(
                    bytes([OP_CODESEPARATOR]) + random_checksig_style(pubs[1])
                ),
            ...
            (
                ...
                CScript(
                    [
                        random_bytes(random.randrange(511)),
                        OP_DROP,
                        OP_IF,
                        OP_CODESEPARATOR,
                        pubs[0],
        ...
        add_spender(
            spenders,
            "sighash/branched_codesep/left",
            tap=tap,
            leaf="branched_codesep",
            key=secs[0],
            codeseppos=3,
            **common,
            inputs=[getter("sign"), b"\x01"],
            **SIGHASH_BITFLIP,
            **ERR_SIG_SCHNORR
        )
        add_spender(
            spenders,
            "sighash/branched_codesep/right",
            tap=tap,
            leaf="branched_codesep",
            key=secs[1],
            codeseppos=6,
            **common,
            inputs=[getter("sign"), b""],
            **SIGHASH_BITFLIP,
            **ERR_SIG_SCHNORR
        )
    ...
    add_spender(
        ...
        standard=False,
        **SINGLE_SIG,
        annex=bytes([ANNEX_TAG]),
        failure={"sighash": override(default_sighash, annex=None)},
        **ERR_SIG_SCHNORR
    )
    add_spender(
        spenders,
        ...
        **SINGLE_SIG,
        failure={
            "sighash": override(
                default_sighash, script_taproot=tap.leaves["codesep_pk"].script
            )
        },
        **ERR_SIG_SCHNORR
    )
    add_spender(
        ...
        **SINGLE_SIG,
        failure={
            "sighash": override(
                default_sighash,
                leafversion=random.choice(
                    [
                        x & 0xFE
                        for x in range(0x100)
                        if x & 0xFE != LEAF_VERSION_TAPSCRIPT
                    ]
                ),
            )
        },
    ...
    add_spender(
        ...
        **common,
        **SINGLE_SIG,
        failure={"sighash": override(default_sighash, leaf=None)},
        **ERR_SIG_SCHNORR
    )
    add_spender(
        spenders,
        ...
        key=secs[0],
        **common,
        failure={"sighash": override(default_sighash, leaf="pk_codesep")},
        **ERR_SIG_SCHNORR
    )

    # Test that invalid hashtypes don't work, both in key path and script path spends
    hashtype = lambda _: random.choice(VALID_SIGHASHES_TAPROOT)
    for invalid_hashtype in [
        x for x in range(0x100) if x not in VALID_SIGHASHES_TAPROOT
    ]:
        add_spender(
            spenders,
            "sighash/keypath_unk_hashtype_%x" % invalid_hashtype,
            tap=tap,
            key=secs[0],
            hashtype=hashtype,
        ...
        add_spender(
            spenders,
            "sighash/scriptpath_unk_hashtype_%x" % invalid_hashtype,
            tap=tap,
            leaf="pk_codesep",
            key=secs[1],
            **SINGLE_SIG,
    ...
    add_spender(
        spenders,
        "sighash/hashtype0_byte_keypath",
        tap=tap,
        key=secs[0],
        hashtype=SIGHASH_DEFAULT,
        failure={"bytes_hashtype": bytes([SIGHASH_DEFAULT])},
        **ERR_SIG_HASHTYPE
    )
    add_spender(
        spenders,
        "sighash/hashtype0_byte_scriptpath",
        tap=tap,
        leaf="pk_codesep",
        key=secs[1],
        **SINGLE_SIG,
    ...
    add_spender(
        spenders,
        "sighash/hashtype1_byte_keypath",
        tap=tap,
        key=secs[0],
        hashtype=SIGHASH_ALL,
        failure={"bytes_hashtype": b""},
        **ERR_SIG_SCHNORR
    )
    add_spender(
        spenders,
        "sighash/hashtype1_byte_scriptpath",
        tap=tap,
        leaf="pk_codesep",
        key=secs[1],
        **SINGLE_SIG,
    ...
    add_spender(
        spenders,
        "sighash/hashtype0to1_keypath",
        tap=tap,
        key=secs[0],
        hashtype=SIGHASH_DEFAULT,
        failure={"bytes_hashtype": bytes([SIGHASH_ALL])},
        **ERR_SIG_SCHNORR
    )
    add_spender(
        spenders,
        "sighash/hashtype0to1_scriptpath",
        tap=tap,
        leaf="pk_codesep",
        key=secs[1],
        **SINGLE_SIG,
    ...
    add_spender(
        spenders,
        "sighash/hashtype1to0_scriptpath",
        tap=tap,
        leaf="pk_codesep",
        key=secs[1],
        **SINGLE_SIG,
    ...
    for hashtype in [SIGHASH_DEFAULT, random.choice(VALID_SIGHASHES_TAPROOT)]:
        scripts = [
            ...
            (
                "csa_pos",
                CScript([OP_0, pubs[2], OP_CHECKSIGADD, OP_1, OP_EQUAL]),
            ),
            ("cs_neg", CScript([pubs[2], OP_CHECKSIG, OP_NOT])),
            (
                "csa_neg",
                CScript([OP_2, pubs[2], OP_CHECKSIGADD, OP_2, OP_EQUAL]),
            ),
        ]
        random.shuffle(scripts)
        ...
        add_spender(
            ...
            **SINGLE_SIG,
            sign=b"",
            failure={"sign": lambda _: random_bytes(random.randrange(1, 63))},
            **ERR_SIG_SIZE
        )
        add_spender(
            spenders,
            "siglen/empty_csa_neg",
            tap=tap,
            key=secs[2],
            leaf="csa_neg",
            hashtype=hashtype,
            **SINGLE_SIG,
            sign=b"",
            failure={"sign": lambda _: random_bytes(random.randrange(66, 100))},
            **ERR_SIG_SIZE
        )
        # Appending a zero byte to signatures invalidates them
        add_spender(
            spenders,
            "siglen/padzero_keypath",
            tap=tap,
            key=secs[3],
            hashtype=hashtype,
            **SIG_ADD_ZERO,
        ...
        add_spender(
            spenders,
            "siglen/popbyte_keypath",
            tap=tap,
            key=secs[3],
            hashtype=hashtype,
            **SIG_POP_BYTE,
            **(ERR_SIG_SIZE if hashtype == SIGHASH_DEFAULT else ERR_SIG_SCHNORR)
        )
        add_spender(
            spenders,
            "siglen/popbyte_csv",
        ...
        add_spender(
            ...
            **SINGLE_SIG,
            ...
            failure={
                "sign": default_sign,
                "sighash": bitflipper(default_sighash),
            },
            **ERR_SIG_SCHNORR
        )
        add_spender(
            spenders,
            "siglen/invalid_csa_neg",
            tap=tap,
            key=secs[2],
            leaf="csa_neg",
            hashtype=hashtype,
    ...
    for p2sh in [False, True]:
        for witver in range(1, 17):
            for witlen in [20, 31, 32, 33]:
                ...
                scripts = [
                    ("s0", CScript([pubs[0], OP_CHECKSIG])),
                    ("dummy", CScript([OP_RETURN])),
                ]
    ...
    PARTNER_MERKLE_FN = [
        ...
        lambda h: (
            int.from_bytes(h, "little") ^ (1 << random.randrange(256))
        ).to_bytes(32, "little"),
    ]
    # Start with a tree of that has depth 1 for "128deep" and depth 2 for "129deep".
    scripts = [
        ("128deep", CScript([pubs[0], OP_CHECKSIG])),
        [
            ("129deep", CScript([pubs[0], OP_CHECKSIG])),
            random.choice(PARTNER_MERKLE_FN),
        ],
    ]
    ...
    add_spender(
        spenders,
        "spendpath/merklelimit",
        tap=tap,
        leaf="128deep",
        **SINGLE_SIG,
        key=secs[0],
        failure={"leaf": "129deep"},
    ...
    add_spender(
        ...
        **SINGLE_SIG,
        key=secs[0],
        failure={"negflag": lambda ctx: 1 - default_negflag(ctx)},
        **ERR_WITNESS_PROGRAM_MISMATCH
    )
    # Test that bitflips in the Merkle branch invalidate it.
    add_spender(
        spenders,
        "spendpath/bitflipmerkle",
        tap=tap,
        leaf="128deep",
        **SINGLE_SIG,
        key=secs[0],
        failure={"merklebranch": bitflipper(default_merklebranch)},
        **ERR_WITNESS_PROGRAM_MISMATCH
    )
    # Test that bitflips in the internal pubkey invalidate it.
    add_spender(
        spenders,
        "spendpath/bitflippubkey",
        tap=tap,
        leaf="128deep",
        **SINGLE_SIG,
        key=secs[0],
        failure={"pubkey_internal": bitflipper(default_pubkey_internal)},
        **ERR_WITNESS_PROGRAM_MISMATCH
    )
    ...
    add_spender(
        spenders,
        "spendpath/padlongcontrol",
        tap=tap,
        leaf="128deep",
        **SINGLE_SIG,
        key=secs[0],
        failure={
            "controlblock": lambda ctx: default_controlblock(ctx)
            + random_bytes(random.randrange(1, 32))
        },
        **ERR_CONTROLBLOCK_SIZE
    )
    ...
    add_spender(
        spenders,
        "spendpath/trunclongcontrol",
        tap=tap,
        leaf="128deep",
        **SINGLE_SIG,
        key=secs[0],
        failure={
            "controlblock": lambda ctx: default_merklebranch(ctx)[
                0 : random.randrange(1, 32)
            ]
        },
        **ERR_CONTROLBLOCK_SIZE
    )
    ...
    add_spender(
        spenders,
        "spendpath/trunc1shortcontrol",
        tap=tap,
        leaf="s",
        **SINGLE_SIG,
        key=secs[0],
        failure={"controlblock": lambda ctx: default_merklebranch(ctx)[0:1]},
        **ERR_CONTROLBLOCK_SIZE
    )
    ...
    csa_high_val = (
        random.randrange(17, 100)
        if random.getrandbits(1)
        else random.randrange(-100, -1)
    )  # Outside OP_n range
    ...
    def big_spend_inputs(ctx):
        ...
        for ht in VALID_SIGHASHES_TAPROOT:
            for k in range(len(pubs)):
                sigs[(k, ht)] = override(
                    default_sign, hashtype=ht, key=secs[k]
                )(ctx)
        num = get(ctx, "num")
        return [
            sigs[(big_choices[i], random.choice(VALID_SIGHASHES_TAPROOT))]
    ...
    scripts = [
        ...
        (
            "t5",
            CScript(
                [OP_IF, pubs[1], OP_CHECKSIG, OP_ELSE, OP_RETURN, OP_ENDIF]
            ),
        ),
        ...
        (
            "t6",
            CScript(
                [OP_NOTIF, OP_RETURN, OP_ELSE, pubs[1], OP_CHECKSIG, OP_ENDIF]
            ),
        ),
        ...
        (
            "t9",
            CScript(
                [csa_low_val, pubs[1], OP_CHECKSIGADD, csa_low_result, OP_EQUAL]
            ),
        ),
        ...
        (
            "t10",
            CScript(
                [csa_low_val, OP_0, OP_CHECKSIGADD, csa_low_result, OP_EQUAL]
            ),
        ),
        ...
        (
            "t12",
            CScript([pubs[1], OP_CHECKSIGVERIFY, pubs[0], OP_CHECKSIG, OP_NOT]),
        ),
        # 13) OP_CHECKSIG with empty key that needs invalid signature
        (
            "t13",
            CScript([pubs[1], OP_CHECKSIGVERIFY, OP_0, OP_CHECKSIG, OP_NOT]),
        ),
        ...
        (
            "t15",
            CScript(
                [pubs[1], OP_CHECKSIGVERIFY, OP_0, OP_0, OP_CHECKSIGADD, OP_NOT]
            ),
        ),
        ...
        (
            "t19",
            CScript([OP_0, OP_0, OP_2DROP] * 10001 + [pubs[1], OP_CHECKSIG]),
        ),
        ...
        (
            ...
            CScript(
                [
                    random_bytes(MAX_SCRIPT_ELEMENT_SIZE),
                    OP_DROP,
                    pubs[1],
                    OP_CHECKSIG,
                ]
        ...
        (
            ...
            CScript(
                [
                    random_bytes(MAX_SCRIPT_ELEMENT_SIZE + 1),
                    OP_DROP,
                    pubs[1],
                    OP_CHECKSIG,
                ]
        ...
        (
            "t27",
            CScript([CScriptNum(OVERSIZE_NUMBER), pubs[1], OP_CHECKSIGADD]),
        ),
        ...
        (
            "t29",
            CScript([CScriptNum(OVERSIZE_NUMBER - 1), pubs[1], OP_CHECKSIGADD]),
        ),
    ...
    add_spender(
        ...
        **common,
        standard=False,
        inputs=[getter("sign"), random_bytes(MAX_SCRIPT_ELEMENT_SIZE)],
        failure={
            "inputs": [
                getter("sign"),
                random_bytes(MAX_SCRIPT_ELEMENT_SIZE + 1),
    ...
    add_spender(
        ...
        leaf="t0",
        **common,
        inputs=[getter("sign"), random_bytes(80)]
    )
    add_spender(
        spenders,
        ...
        **common,
        standard=False,
        inputs=[getter("sign"), random_bytes(81)]
    )
    ...
    add_spender(
        ...
        **common,
        inputs=[getter("sign"), b"\x01"],
        failure={"inputs": [getter("sign"), b"\x02"]},
        **ERR_MINIMALIF
    )
    add_spender(
        spenders,
        ...
        **common,
        inputs=[getter("sign"), b"\x01"],
        failure={"inputs": [getter("sign"), b"\x03"]},
        **ERR_MINIMALIF
    )
    add_spender(
        spenders,
        ...
        **common,
        inputs=[getter("sign"), b"\x01"],
        failure={"inputs": [getter("sign"), b"\x0001"]},
        **ERR_MINIMALIF
    )
    add_spender(
        spenders,
        ...
        **common,
        inputs=[getter("sign"), b"\x01"],
        failure={"inputs": [getter("sign"), b"\x0100"]},
        **ERR_MINIMALIF
    )
    ...
    add_spender(
        spenders,
        "tapscript/unkpk/checksigverify",
        leaf="t18",
        standard=False,
        **common,
        **SINGLE_SIG,
    ...
    add_spender(
        spenders,
        "tapscript/oldpk/checksigverify",
        leaf="t32",
        standard=False,
        **common,
        **SINGLE_SIG,
    ...
    add_spender(
        spenders,
        "tapscript/emptypk/checksigverify",
        leaf="t2",
        **SINGLE_SIG,
        **common,
        failure={"leaf": "t8"},
    ...
    add_spender(
        spenders,
        "tapscript/emptysigs/checksig",
        leaf="t12",
        **common,
        inputs=[b"", getter("sign")],
        failure={"leaf": "t13"},
        **ERR_UNKNOWN_PUBKEY
    )
    add_spender(
        spenders,
        "tapscript/emptysigs/nochecksigverify",
        leaf="t12",
        **common,
        inputs=[b"", getter("sign")],
        failure={"leaf": "t20"},
    ...
    add_spender(
        spenders,
        "tapscript/emptysigs/checksigadd",
        leaf="t14",
        **common,
        inputs=[b"", getter("sign")],
        failure={"leaf": "t15"},
    ...
    add_spender(
        spenders, "tapscript/no10000limit", leaf="t19", **SINGLE_SIG, **common
    )
    ...
    add_spender(
        ...
        leaf="t23",
        **common,
        inputs=[getter("sign")] + [b"" for _ in range(999)],
        failure={
            "leaf": "t24",
            "inputs": [getter("sign")] + [b"" for _ in range(1000)],
        },
        **ERR_STACK_SIZE
    )
    ...
    add_spender(
        ...
        leaf="t33",
        **common,
        inputs=big_spend_inputs,
        num=999,
        failure={"leaf": "t34", "num": 1000},
        **ERR_STACK_SIZE
    )
    ...
    SIGOPS_RATIO_SCRIPTS = [
        ...
        lambda n, pk: (
            CScript(
                [OP_DROP, pk] + [OP_2DUP, OP_CHECKSIGVERIFY] * n + [OP_CHECKSIG]
            ),
            n + 1,
        ),
        ...
        lambda n, pk: (
            CScript(
                [OP_DROP, pk, OP_0, OP_IF, OP_2DUP, OP_CHECKSIGVERIFY, OP_ENDIF]
                + [OP_2DUP, OP_CHECKSIGVERIFY] * n
                + [OP_2, OP_SWAP, OP_CHECKSIGADD, OP_3, OP_EQUAL]
            ),
            n + 1,
        ),
        ...
        lambda n, pk: (
            CScript(
                ...
                + [OP_2DUP, OP_CHECKSIGVERIFY] * n
                + [OP_4, OP_SWAP, OP_CHECKSIGADD, OP_5, OP_EQUAL]
            ),
            n + 1,
        ),
        ...
        lambda n, pk: (
            CScript(
                ...
                + [OP_2DUP, OP_CHECKSIGVERIFY] * n
                + [OP_8, OP_SWAP, OP_CHECKSIGADD, OP_9, OP_EQUAL]
            ),
            n + 1,
        ),
        ...
        lambda n, pk: (
            CScript(
                [OP_DROP, OP_0, pk, OP_CHECKSIG, OP_NOT, OP_VERIFY, pk]
                + [OP_2DUP, OP_CHECKSIG, OP_VERIFY] * n
                + [OP_CHECKSIG]
            ),
            n + 1,
        ),
    ...
    for annex in [
        None,
        bytes([ANNEX_TAG]) + random_bytes(random.randrange(1000)),
    ]:
        for hashtype in [SIGHASH_DEFAULT, SIGHASH_ALL]:
            for pubkey in [
                pubs[1],
                random_bytes(
                    random.choice([x for x in range(2, 81) if x != 32])
                ),
            ]:
                for fn_num, fn in enumerate(SIGOPS_RATIO_SCRIPTS):
                    ...
                    def predict_sigops_ratio(n, dummy_size):
                        ...
                        annexlen_size = (
                            0 if annex is None else 1 + 2 * (annex_size >= 253)
                        )
                        witsize = (
                            stacklen_size
                            + sig_size
                    ...
                    tap = taproot_construct(pubs[0], scripts)
                    standard = (
                        annex is None and dummylen <= 80 and len(pubkey) == 32
                    )
                    add_spender(
                        spenders,
                        "tapscript/sigopsratio_%i" % fn_num,
                        tap=tap,
                        leaf="s",
                        annex=annex,
                        hashtype=hashtype,
                        key=secs[1],
                        inputs=[getter("sign"), random_bytes(dummylen)],
                        standard=standard,
                        failure={
                            "inputs": [
                                getter("sign"),
                                random_bytes(dummylen - 1),
                            ]
                        },
                        **ERR_SIGOPS_RATIO
                    )
    ...
    for leafver in range(0, 0x100, 2):
        ...
        scripts = [
            ...
            (
                "bigpush_c0",
                CScript([random_bytes(MAX_SCRIPT_ELEMENT_SIZE + 1), OP_DROP]),
            ),
            (
                "bigpush_unkver",
                CScript([random_bytes(MAX_SCRIPT_ELEMENT_SIZE + 1), OP_DROP]),
        ...
        add_spender(
            ...
            standard=False,
            tap=tap,
            leaf="undecodable_unkver",
            failure={"leaf": "undecodable_c0"},
            **ERR_UNDECODABLE
        )
        add_spender(
            spenders,
        ...
        add_spender(
            ...
            standard=False,
            tap=tap,
            leaf="1001push_unkver",
            failure={"leaf": "1001push_c0"},
            **ERR_STACK_SIZE
        )
        add_spender(
            spenders,
    ...
    for opval in range(76, 0x100):
        ...
        scripts = [
            ...
            (
                "undecodable_bypassed_success",
                CScript([OP_PUSHDATA1, OP_2, opcode]),
            ),
            (
                "bigpush_success",
                CScript(
                    [random_bytes(MAX_SCRIPT_ELEMENT_SIZE + 1), OP_DROP, opcode]
                ),
            ),
            (
                "bigpush_nop",
                CScript(
                    [random_bytes(MAX_SCRIPT_ELEMENT_SIZE + 1), OP_DROP, OP_NOP]
                ),
            ),
        ...
        add_spender(
            ...
            standard=False,
            tap=tap,
            leaf="unexecif_success",
            failure={"leaf": "unexecif_nop"},
            **ERR_CLEANSTACK
        )
        add_spender(
            spenders,
            ...
            tap=tap,
            leaf="return_success",
            failure={"leaf": "return_nop"},
            **ERR_OP_RETURN
        )
        add_spender(
            spenders,
            "opsuccess/undecodable",
            standard=False,
            tap=tap,
            leaf="undecodable_success",
            failure={"leaf": "undecodable_nop"},
            **ERR_UNDECODABLE
        )
        add_spender(
            spenders,
            "opsuccess/undecodable_bypass",
            standard=False,
            tap=tap,
            leaf="undecodable_success",
            failure={"leaf": "undecodable_bypassed_success"},
            **ERR_UNDECODABLE
        )
        add_spender(
            spenders,
            ...
            standard=False,
            tap=tap,
            leaf="bigpush_success",
            failure={"leaf": "bigpush_nop"},
            **ERR_PUSH_LIMIT
        )
        add_spender(
            spenders,
            ...
            standard=False,
            tap=tap,
            leaf="1001push_success",
            failure={"leaf": "1001push_nop"},
            **ERR_STACK_SIZE
        )
        add_spender(
            spenders,
            "opsuccess/1001inputs",
            standard=False,
            tap=tap,
            leaf="bare_success",
            inputs=[b""] * 1001,
    ...
    add_spender(
        ...
        tap=tap,
        leaf="leaf",
        inputs=[getter("sign"), getter("sign")],
        key=secs[1],
        no_fail=True,
    )
    ...
    for compressed in [False, True]:
        ...
        for p2sh in [False, True]:
            for witv0 in [False, True]:
                for hashtype in VALID_SIGHASHES_ECDSA + [
                    random.randrange(0x04, 0x80),
                    random.randrange(0x84, 0x100),
                ]:
                    standard = (hashtype in VALID_SIGHASHES_ECDSA) and (
                        compressed or not witv0
                    )
                    add_spender(
                        spenders,
                        "legacy/pk-wrongkey",
                        ...
                        witv0=witv0,
                        standard=standard,
                        script=key_to_p2pk_script(pubkey1),
                        **SINGLE_SIG,
                        key=eckey1,
                        failure={"key": eckey2},
                        sigops_weight=4 - 3 * witv0,
                        **ERR_NO_SUCCESS
                    )
                    add_spender(
                        spenders,
                        "legacy/pkh-sighashflip",
                        hashtype=hashtype,
                        p2sh=p2sh,
                        witv0=witv0,
                        standard=standard,
        ...
        for witv0 in [False, True]:
            for hashtype in VALID_SIGHASHES_ECDSA + [
                ...
                add_spender(
                    ...
                    sigops_weight=4 - 3 * witv0,
                    inputs=[getter("sign"), b""],
                    failure={"inputs": [getter("sign"), b"\x01"]},
                    **ERR_UNDECODABLE
                )
...
def spenders_taproot_nonstandard():
    ...
    scripts = [
        ...
        (
            "op_success",
            CScript([pub, OP_CHECKSIG, OP_0, OP_IF, CScriptOp(0x50), OP_ENDIF]),
        ),
    ]
    ...
    add_spender(
        spenders,
        "inactive/scriptpath_valid_unkleaf",
        key=sec,
        tap=tap,
        leaf="future_leaf",
        standard=False,
    ...
    add_spender(
        spenders,
        "inactive/scriptpath_invalid_unkleaf",
        key=sec,
        tap=tap,
        leaf="future_leaf",
        standard=False,
    ...
    add_spender(
        spenders,
        "inactive/scriptpath_valid_opsuccess",
        key=sec,
        tap=tap,
        leaf="op_success",
        standard=False,
...
def dump_json_test(tx, input_utxos, idx, success, failure):
    ...
    flags = (
        LEGACY_FLAGS
        if spender.comment.startswith("legacy/")
        or spender.comment.startswith("inactive/")
        else TAPROOT_FLAGS
    )
...
class TaprootTest(SugarchainTestFramework):
    ...
    def block_submit(
        ...
        # transactions.
        extra_output_script = CScript(
            bytes(
                [OP_CHECKSIG]
                * (
                    (MAX_BLOCK_SIGOPS_WEIGHT - sigops_weight)
                    // WITNESS_SCALE_FACTOR
                )
            )
        )
        ...
        block = create_block(
            self.tip, coinbase_tx, self.lastblocktime + 1, txlist=txs
        )
        ...
        if err_msg is not None:
            assert (
                block_response is not None and err_msg in block_response
            ), "Missing error message '%s' from block response '%s': %s" % (
                err_msg,
                "(None)" if block_response is None else block_response,
                msg,
            )
        if accept:
            assert (
                node.getbestblockhash() == block.hash
            ), "Failed to accept: %s (response: %s)" % (msg, block_response)
            self.tip = block.sha256
            self.lastblockhash = block.hash
            self.lastblocktime += 1
            self.lastblockheight += 1
        else:
            assert node.getbestblockhash() == self.lastblockhash, (
                "Failed to reject: " + msg
            )
    ...
    def test_spenders(self, node, spenders, input_counts):
        ...
        for i in range(16):
            addr = node.getnewaddress(
                address_type=random.choice(["legacy", "p2sh-segwit", "bech32"])
            )
        ...
        mismatching_utxos = (
            []
        )  # UTXOs with input that requires mismatching output position
        done = 0
        while done < len(spenders):
            # Compute how many UTXOs to create with this transaction
            count_this_tx = min(
                len(spenders) - done, (len(spenders) + 4) // 5, 10000
            )
            ...
            unspents.sort(
                key=lambda x: int(x["amount"] * 100000000), reverse=True
            )
            if len(unspents) > 50:
            ...
            for unspent in unspents:
                ...
                fund_tx.vin.append(
                    CTxIn(COutPoint(txid, int(unspent["vout"])), CScript())
                )
            # Add outputs
            cur_progress = done / len(spenders)
            next_progress = (done + count_this_tx) / len(spenders)
            change_goal = (
                (1.0 - 0.6 * next_progress)
                / (1.0 - 0.6 * cur_progress)
                * balance
            )
            self.log.debug(
                "Create %i UTXOs in a transaction spending %i inputs worth %.8f (sending ~%.8f to change)"
                % (
                    count_this_tx,
                    len(unspents),
                    balance * 0.00000001,
                    change_goal * 0.00000001,
                )
            )
            for i in range(count_this_tx):
                ...
                amount = int(
                    random.randrange(
                        int(avg * 0.85 + 0.5), int(avg * 1.15 + 0.5)
                    )
                    + 0.5
                )
                balance -= amount
            ...
            # Add change
            fund_tx.vout.append(
                CTxOut(balance - 10000, random.choice(host_spks))
            )
            # Ask the wallet to sign
            fund_tx = tx_from_hex(
                node.signrawtransactionwithwallet(fund_tx.serialize().hex())[
                    "hex"
                ]
            )
            ...
            for i in range(count_this_tx):
                utxodata = UTXOData(
                    outpoint=COutPoint(fund_tx.sha256, i),
                    output=fund_tx.vout[i],
                    spender=spenders[done],
                )
        ...
        while left:
            ...
            tx = CTransaction()
            tx.nVersion = random.choice(
                [1, 2, random.randint(-0x80000000, 0x7FFFFFFF)]
            )
            min_sequence = (
                tx.nVersion != 1 and tx.nVersion != 0
            ) * 0x80000000  # The minimum sequence number to disable relative locktime
            if random.choice([True, False]):
                tx.nLockTime = random.randrange(
                    LOCKTIME_THRESHOLD, self.lastblocktime - 7200
                )  # all absolute locktimes in the past
            else:
                tx.nLockTime = random.randrange(
                    self.lastblockheight + 1
                )  # all block heights in the past
            ...
            acceptable = [
                ...
                if n <= left
                and (
                    left - n > max(input_counts)
                    or (left - n) in [0] + input_counts
                )
            ]
            ...
            input_utxos = []
            while len(mismatching_utxos) and (
                len(input_utxos) == 0 or len(normal_utxos) == 1
            ):
            ...
            fee = min(
                random.randrange(MIN_FEE * 2, MIN_FEE * 4), amount - DUST_LIMIT
            )  # 10000-20000 sat fee
            in_value = amount - fee
            tx.vin = [
                CTxIn(
                    outpoint=utxo.outpoint,
                    nSequence=random.randint(min_sequence, 0xFFFFFFFF),
                )
                for utxo in input_utxos
            ]
            tx.wit.vtxinwit = [CTxInWitness() for _ in range(len(input_utxos))]
            sigops_weight = sum(
                utxo.spender.sigops_weight for utxo in input_utxos
            )
            self.log.debug(
                "Test: %s"
                % (", ".join(utxo.spender.comment for utxo in input_utxos))
            )

            # Add 1 to 4 random outputs (but constrained by inputs that require mismatching outputs)
            num_outputs = random.choice(
                range(
                    1,
                    1
                    + min(
                        4,
                        4
                        if first_mismatch_input is None
                        else first_mismatch_input,
                    ),
                )
            )
            ...
            for i in range(num_outputs):
                ...
                sigops_weight += (
                    CScript(tx.vout[-1].scriptPubKey).GetSigOpCount(False)
                    * WITNESS_SCALE_FACTOR
                )
            fee += in_value
            assert fee >= 0
            ...
            for i in range(len(input_utxos)):
                ...
                if not input_utxos[i].spender.no_fail:
                    fail = fn(
                        tx, i, [utxo.output for utxo in input_utxos], False
                    )
            ...
            for fail_input in list(range(len(input_utxos))) + [None]:
                ...
                if (
                    fail_input is not None
                    and input_utxos[fail_input].spender.no_fail
                ):
                    continue
                ...
                expected_fail_msg = (
                    ...
                    else input_utxos[fail_input].spender.err_msg
                )
                # Fill inputs/witnesses
                for i in range(len(input_utxos)):
                    tx.vin[i].scriptSig = input_data[i][i != fail_input][0]
                    tx.wit.vtxinwit[i].scriptWitness.stack = input_data[i][
                        i != fail_input
                    ][1]
                ...
                is_standard_tx = (
                    ...
                    and (
                        all(utxo.spender.is_standard for utxo in input_utxos)
                    )  # All inputs must be standard
                    and tx.nVersion >= 1  # The tx version must be standard
                    and tx.nVersion <= 2
                )
                tx.rehash()
                msg = ",".join(
                    utxo.spender.comment + ("*" if n == fail_input else "")
                    for n, utxo in enumerate(input_utxos)
                )
                if is_standard_tx:
                    node.sendrawtransaction(tx.serialize().hex(), 0)
                    assert node.getmempoolentry(tx.hash) is not None, (
                        "Failed to accept into mempool: " + msg
                    )
                else:
                    assert_raises_rpc_error(
                        -26,
                ...
                self.block_submit(
                    ...
                    msg,
                    witness=True,
                    accept=fail_input is None,
                    cb_pubkey=cb_pubkey,
                    fees=fee,
                    sigops_weight=sigops_weight,
                    err_msg=expected_fail_msg,
                )

            if (len(spenders) - left) // 200 > (
                len(spenders) - left - len(input_utxos)
            ) // 200:
    ...
    def gen_test_vectors(self):
        ...
        coinbase.vin = [
            CTxIn(
                COutPoint(0, 0xFFFFFFFF), CScript([OP_1, OP_1]), SEQUENCE_FINAL
            )
        ]
        ...
        assert (
            coinbase.hash
            == "f60c73405d499a956d3162e3483c395526ef78286458a4cb17b125aa92e49b20"
        )
        # Mine it
        block = create_block(
        ...
        SEED = 317
        VALID_LEAF_VERS = list(range(0xC0, 0x100, 2)) + [
            0x66,
            0x7E,
            0x80,
            0x84,
        ...
        prvs = [
            hashlib.sha256(SEED.to_bytes(2, "big") + bytes([i])).digest()
            for i in range(100)
        ]
        ...
        script_lists = [
            ...
            [
                ("0", CScript([pubs[52], OP_CHECKSIG]), LEAF_VERSION_TAPSCRIPT),
                ("1", CScript([b"BIP341"]), VALID_LEAF_VERS[pubs[99][0] % 41]),
            ],
            [
                ("0", CScript([pubs[53], OP_CHECKSIG]), LEAF_VERSION_TAPSCRIPT),
                ("1", CScript([b"Taproot"]), VALID_LEAF_VERS[pubs[99][1] % 41]),
            ],
            [
                ...
                [
                    (
                        "1",
                        CScript([pubs[55], OP_CHECKSIG]),
                        LEAF_VERSION_TAPSCRIPT,
                    ),
                    (
                        "2",
                        CScript([pubs[56], OP_CHECKSIG]),
                        LEAF_VERSION_TAPSCRIPT,
                    ),
                ],
            ],
            [
                ...
                [
                    (
                        "1",
                        CScript([pubs[58], OP_CHECKSIG]),
                        LEAF_VERSION_TAPSCRIPT,
                    ),
                    (
                        "2",
                        CScript([pubs[59], OP_CHECKSIG]),
                        LEAF_VERSION_TAPSCRIPT,
                    ),
                ],
            ],
        ...
        taps = [
            taproot_construct(inner_keys[i], script_lists[i])
        ...
        cblks = [
            {
                leaf: get(
                    {**DEFAULT_CONTEXT, "tap": taps[i], "leaf": leaf},
                    "controlblock",
                )
                for leaf in taps[i].leaves
            }
            for i in range(7)
        ]
        # Require one swapped and one unswapped in taps 3 and 4.
        assert (cblks[3]["0"][33:65] < cblks[3]["1"][33:65]) != (
            cblks[4]["0"][33:65] < cblks[4]["1"][33:65]
        )
        # Require one swapped and one unswapped in taps 5 and 6, both at the top and child level.
        assert (cblks[5]["0"][33:65] < cblks[5]["1"][65:]) != (
            cblks[6]["0"][33:65] < cblks[6]["1"][65:]
        )
        assert (cblks[5]["1"][33:65] < cblks[5]["2"][33:65]) != (
            cblks[6]["1"][33:65] < cblks[6]["2"][33:65]
        )
        # Require within taps 5 (and thus also 6) that one level is swapped and the other is not.
        assert (cblks[5]["0"][33:65] < cblks[5]["1"][65:]) != (
            cblks[5]["1"][33:65] < cblks[5]["2"][33:65]
        )
        ...
        for i, spk in enumerate(old_spks + tap_spks):
            ...
            tx.nVersion = 1
            tx.vin = [
                CTxIn(COutPoint(lasttxid, i & 1), CScript([]), SEQUENCE_FINAL)
            ]
        ...
        for i, tap in enumerate(taps):
            ...
            if len(tap.leaves):
                leafhashes = intermediary.setdefault(
                    "leafHashes", [None] * len(tap.leaves)
                )
                for leaf in tap.leaves:
                    leafhashes[int(leaf)] = tap.leaves[leaf].leaf_hash.hex()
            intermediary["merkleRoot"] = (
                tap.merkle_root.hex() if tap.merkle_root else None
            )
            ...
            expected["bip350Address"] = program_to_witness(
                1, bytes(tap.output_pubkey), True
            )
            if len(tap.leaves):
                control_blocks = expected.setdefault(
                    "scriptPathControlBlocks", [None] * len(tap.leaves)
                )
                for leaf in tap.leaves:
        ...
        for i, spk in enumerate(input_spks):
            tx.vin.append(
                CTxIn(spend_info[spk]["prevout"], CScript(), sequences[i])
            )
        ...
        for i in range(len(input_spks)):
            utxos_spent.append(
                {
                    "scriptPubKey": inputs[i].scriptPubKey.hex(),
                    "amountSats": inputs[i].nValue,
                }
            )
        ...
        for i in range(len(input_spks)):
            ...
            if ctx["mode"] == "taproot":
                ...
                intermediary = test_case.setdefault("intermediary", {})
                intermediary["internalPubkey"] = get(
                    ctx, "tap"
                ).internal_pubkey.hex()
                ...
                intermediary["precomputedUsed"] = [
                    key
                    for key in sorted(precomputed.keys())
                    if sigmsg.count(precomputed[key])
                ]
            ...
            tx.wit.vtxinwit[i].scriptWitness.stack = flatten(
                get(ctx, "witness")
            )
        ...
        assert_equal(
            hashlib.sha256(tx.serialize()).hexdigest(),
    ...
    def run_test(self):
        ...
        self.test_spenders(
            ...
            input_counts=[1, 2, 2, 2, 2, 3],
        )
        # Run each test twice; once in isolation, and once combined with others. Testing in isolation
        # means that the standardness is verified in every test (as combined transactions are only standard
        # when all their inputs are standard).
        self.test_spenders(
            self.nodes[0], spenders_taproot_nonstandard(), input_counts=[1]
        )
        self.test_spenders(
            self.nodes[0], spenders_taproot_nonstandard(), input_counts=[2, 3]
        )
