...
class ConfArgsTest(SugarchainTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in sugarchain.conf or in an included conf file
        bad_conf_file_path = os.path.join(
            self.options.tmpdir, "node0", "sugarchain_bad.conf"
        )
        util.write_config(
            bad_conf_file_path, n=0, chain="", extra_config=f"conf=some.conf\n"
        )
        ...
        with open(
            os.path.join(self.nodes[0].datadir, "sugarchain.conf"),
            "a",
            encoding="utf-8",
        ) as conf:
        ...
        with open(inc_conf_file_path, "w", encoding="utf-8") as conf:
            conf.write("-dash=1\n")
        self.nodes[0].assert_start_raises_init_error(
            expected_msg="Error: Error reading configuration file: parse error on line 1: -dash=1, options in configuration file must be specified without leading -"
        )
        ...
        main_conf_file_path = os.path.join(
            self.options.tmpdir, "node0", "sugarchain_main.conf"
        )
        util.write_config(
            main_conf_file_path,
            n=0,
        ...
        with open(inc_conf_file_path, "w", encoding="utf-8") as conf:
            conf.write("nono\n")
        self.nodes[0].assert_start_raises_init_error(
            expected_msg="Error: Error reading configuration file: parse error on line 1: nono, if you intended to specify a negated option, use nono=1 instead"
        )

        with open(inc_conf_file_path, "w", encoding="utf-8") as conf:
            conf.write("server=1\nrpcuser=someuser\nrpcpassword=some#pass")
        self.nodes[0].assert_start_raises_init_error(
            expected_msg="Error: Error reading configuration file: parse error on line 3, using # in rpcpassword can be ambiguous and should be avoided"
        )
        ...
        self.nodes[0].assert_start_raises_init_error(
            expected_msg="Error: Error reading configuration file: parse error on line 4, using # in rpcpassword can be ambiguous and should be avoided"
        )
        ...
        self.nodes[0].stop_node(
            expected_stderr=f"Warning: {inc_conf_file_path}:1 Section [testnot] is not recognized.{os.linesep}{inc_conf_file2_path}:1 Section [testnet] is not recognized."
        )
    ...
    def test_seed_peers(self):
        ...
        self.stop_node(0)
        self.nodes[0].assert_start_raises_init_error(
            ["-dnsseed=1", "-onlynet=i2p", "-i2psam=127.0.0.1:7656"],
    ...
    def test_connect_with_seednode(self):
        ...
        with self.nodes[0].assert_debug_log(expected_msgs=seednode_ignored):
            self.start_node(
                0,
                extra_args=["-connect=fakeaddress1", "-seednode=fakeaddress2"],
            )
        ...
        with self.nodes[0].assert_debug_log(
            expected_msgs=addcon_thread_started, unexpected_msgs=dnsseed_ignored
        ):
            self.restart_node(
                0, extra_args=["-connect=fakeaddress1", "-proxy=1.2.3.4"]
            )
        ...
        for connect_arg in ["-connect=0", "-noconnect"]:
            with self.nodes[0].assert_debug_log(
                expected_msgs=addcon_thread_started,
    ...
    def run_test(self):
        ...
        none_existent_conf_file = os.path.join(
            default_data_dir, "none_existent_sugarchain.conf"
        )
        self.nodes[0].assert_start_raises_init_error(
            ["-conf=" + none_existent_conf_file],
            'Error: Error reading configuration file: specified config file "'
            + none_existent_conf_file
            + '" could not be opened.',
        )
        ...
        assert os.path.exists(
            os.path.join(new_data_dir_2, self.chain, "blocks")
        )
