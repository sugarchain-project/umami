...
class BackwardsCompatibilityTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        self.generatetoaddress(
            node_miner, COINBASE_MATURITY + 1, node_miner.getnewaddress()
        )
        ...
        address_18075 = wallet.rpc.addmultisigaddress(
            1,
            [
        ...
        node_master_wallets_dir = os.path.join(
            node_master.datadir, "regtest/wallets"
        )
        ...
        # RPC loadwallet failure causes sugarchaind to exit, in addition to the RPC
        ...
        if self.options.descriptors:
            ...
            node_v17.assert_start_raises_init_error(
                ["-wallet=w3"],
                "Error: Error loading w3: Wallet requires newer version of Sugarchain Core",
            )
        ...
        if self.is_bdb_compiled():
            ...
            # Copy the 0.16 wallet to the last Sugarchain Core version and open it:
            shutil.copyfile(
            ...
            # Copy the 0.17 wallet to the last Sugarchain Core version and open it:
            ...
            descriptor = (
                f"wpkh([{info['hdmasterfingerprint']}{hdkeypath[1:]}]{pubkey})"
            )
            assert_equal(info["desc"], descsum_create(descriptor))
            ...
            # Copy the 0.19 wallet to the last Sugarchain Core version and open it:
            shutil.copytree(
