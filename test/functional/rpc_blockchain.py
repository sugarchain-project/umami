...
class BlockchainTest(SugarchainTestFramework):
    ...
    def _test_prune_disk_space(self):
        self.log.info(
            "Test that a manually pruned node does not run into "
    ...
    def _test_max_future_block_time(self):
        ...
        self.start_node(
            0,
            extra_args=[f"-mocktime={TIME_RANGE_TIP - MAX_FUTURE_BLOCK_TIME}"],
        )
    ...
    def check_signalling_deploymentinfo_result(
        self, gdi_result, height, blockhash, status_next
    ):
    ...
    def _test_getdeploymentinfo(self):
        ...
        self.check_signalling_deploymentinfo_result(
            self.nodes[0].getdeploymentinfo(),
            gbci207["blocks"],
            gbci207["bestblockhash"],
            "started",
        )
        ...
        self.check_signalling_deploymentinfo_result(
            self.nodes[0].getdeploymentinfo(),
            gbci287["blocks"],
            gbci287["bestblockhash"],
            "locked_in",
        )

        # calling with an explicit hash works
        self.check_signalling_deploymentinfo_result(
            self.nodes[0].getdeploymentinfo(gbci207["bestblockhash"]),
            gbci207["blocks"],
            gbci207["bestblockhash"],
            "started",
        )
    ...
    def _test_y2106(self):
        ...
        assert_equal(
            self.nodes[0].getblockheader(last)["mediantime"], time_2106
        )
    ...
    def _test_getchaintxstats(self):
        ...
        assert_equal(
            round(chaintxstats["txrate"] * TIME_RANGE_STEP, 10), Decimal(1)
        )
    ...
    def _test_stopatheight(self):
        ...
        try:
            self.generatetoaddress(
                self.nodes[0], 1, self.wallet.get_address(), sync_fun=self.no_op
            )
    ...
    def _test_waitforblockheight(self):
        ...
        def assert_waitforheight(height, timeout=2):
            assert_equal(
                node.waitforblockheight(height=height, timeout=timeout)[
                    "height"
                ],
                current_height,
            )
    ...
    def _test_getblock(self):
        ...
        def assert_hexblock_hashes(verbosity):
            ...
            assert_equal(
                blockhash, hash256(bytes.fromhex(block[:160]))[::-1].hex()
            )
        ...
        self.log.info(
            "Test that getblock with verbosity 0 hashes to expected value"
        )
