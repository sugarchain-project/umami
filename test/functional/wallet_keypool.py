...
class KeyPoolTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        addr_before_encrypting_data = nodes[0].getaddressinfo(
            addr_before_encrypting
        )
        wallet_info_old = nodes[0].getwalletinfo()
        if not self.options.descriptors:
            assert (
                addr_before_encrypting_data["hdseedid"]
        ...
        assert (
            addr_before_encrypting_data["hdmasterfingerprint"]
            != addr_data["hdmasterfingerprint"]
        )
        if not self.options.descriptors:
        ...
        assert_raises_rpc_error(
            -12, "Keypool ran out", nodes[0].getrawchangeaddress
        )
        ...
        if not self.options.descriptors:
            ...
            start_keypath = nodes[0].getaddressinfo(nodes[0].getnewaddress())[
                "hdkeypath"
            ]
            start_change_keypath = nodes[0].getaddressinfo(
                nodes[0].getrawchangeaddress()
            )["hdkeypath"]
            # flush keypool and get new addresses
            nodes[0].newkeypool()
            end_keypath = nodes[0].getaddressinfo(nodes[0].getnewaddress())[
                "hdkeypath"
            ]
            end_change_keypath = nodes[0].getaddressinfo(
                nodes[0].getrawchangeaddress()
            )["hdkeypath"]
            # The new keypath index should be 100 more than the old one
            new_index = int(start_keypath.rsplit("/", 1)[1][:-1]) + 100
            new_change_index = (
                int(start_change_keypath.rsplit("/", 1)[1][:-1]) + 100
            )
            assert_equal(end_keypath, "m/0'/0'/" + str(new_index) + "'")
            assert_equal(
                end_change_keypath, "m/0'/1'/" + str(new_change_index) + "'"
            )

        # create a blank wallet
        nodes[0].createwallet(
            wallet_name="w2", blank=True, disable_private_keys=True
        )
        ...
        res = w2.walletcreatefundedpsbt(
            inputs=[],
            outputs=[{destination: 0.00008900}],
        ...
        res = w2.walletcreatefundedpsbt(
            inputs=[],
            outputs=[{destination: 0.00008800}],
            options={"feeRate": 0.00010},
        )
        assert_equal("psbt" in res, True)
        ...
        res = w2.walletcreatefundedpsbt(
            inputs=[],
            outputs=[{destination: 0.00010000}],
            options={"subtractFeeFromOutputs": [0], "feeRate": 0.0008823},
        )
        assert_equal("psbt" in res, True)
