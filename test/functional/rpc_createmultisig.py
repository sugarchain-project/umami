...
class RpcCreateMultiSigTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        for keys in itertools.permutations([pk0, pk1, pk2]):
            ...
            for addr_type in ["bech32", "p2sh-segwit"]:
                result = self.nodes[0].createmultisig(
                    nrequired=2, keys=keys, address_type=addr_type
                )
                assert_equal(legacy_addr, result["address"])
                assert_equal(result["warnings"], err_msg)

                if self.is_bdb_compiled():
                    result = wmulti0.addmultisigaddress(
                        nrequired=2, keys=keys, address_type=addr_type
                    )
                    assert_equal(legacy_addr, result["address"])
                    assert_equal(result["warnings"], err_msg)
    ...
    def check_addmultisigaddress_errors(self):
        ...
        assert_raises_rpc_error(
            ...
            lambda: self.nodes[0].addmultisigaddress(
                nrequired=1, keys=addresses
            ),
        )
        for a in addresses:
        ...
        pubs = [
            self.nodes[1].getaddressinfo(addr)["pubkey"] for addr in addresses
        ]
        assert_raises_rpc_error(
            -5,
            "Bech32m multisig addresses cannot be created with legacy wallets",
    ...
    def do_multisig(self):
        ...
        if self.is_bdb_compiled():
            if "wmulti" not in node1.listwallets():
                try:
                    ...
                    if (
                        e.error["code"] == -18
                        and "Wallet file verification failed. Failed to load database path '{}'. Path does not exist.".format(
                            path
                        )
                        in e.error["message"]
                    ):
        ...
        if self.is_bdb_compiled():
            ...
            msigw = wmulti.addmultisigaddress(
                self.nsigs, self.pub, None, self.output_type
            )
            maddw = msigw["address"]
        ...
        txid, _ = self.wallet.send_to(
            from_node=self.nodes[0], scriptPubKey=spk, amount=1300
        )
        ...
        node2.signrawtransactionwithkey(
            rawtx, self.priv[0 : self.nsigs - 1], [prevtx_err]
        )
