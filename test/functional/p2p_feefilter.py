#!/usr/bin/env python3
# Copyright (c) 2016-2021 The Sugarchain Core developers
...
class FeeFilterTest(SugarchainTestFramework):
    ...
    def test_feefilter_forcerelay(self):
        ...
        self.nodes[0].add_p2p_connection(
            FeefilterConn()
        ).assert_feefilter_received(True)
        ...
        self.nodes[0].add_p2p_connection(
            FeefilterConn()
        ).assert_feefilter_received(False)
    ...
    def test_feefilter(self):
        ...
        txids = [
            miniwallet.send_self_transfer(
                fee_rate=Decimal("0.00000200"), from_node=node1
            )["wtxid"]
            for _ in range(3)
        ]
        ...
        txids = [
            miniwallet.send_self_transfer(
                fee_rate=Decimal("0.00000150"), from_node=node1
            )["wtxid"]
            for _ in range(3)
        ]
        conn.wait_for_invs_to_match(txids)
        ...
        txids = [
            miniwallet.send_self_transfer(
                fee_rate=Decimal("0.00000100"), from_node=node1
            )["wtxid"]
            for _ in range(3)
        ]
        ...
        txids = [
            miniwallet.send_self_transfer(
                fee_rate=Decimal("0.00020000"), from_node=node0
            )["wtxid"]
            for _ in range(1)
        ]
        conn.wait_for_invs_to_match(txids)
        ...
        txids = [
            miniwallet.send_self_transfer(
                fee_rate=Decimal("0.00020000"), from_node=node1
            )["wtxid"]
            for _ in range(3)
        ]
        conn.wait_for_invs_to_match(txids)
