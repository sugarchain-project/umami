...
# The block reward of coinbaseoutput.nValue (50) SUGAR/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 SUGAR/block.
...
class TestSugarchainCli(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info(
            "Compare responses from getblockchaininfo RPC and `sugarchain-cli getblockchaininfo`"
        )
        ...
        assert_raises_process_error(
            ...
            self.nodes[0]
            .cli("-rpccookiefile=does-not-exist", "-rpcpassword=")
            .echo,
        )
        ...
        assert (
            "\u001b[0m"
            not in self.nodes[0].cli("-getinfo", "-color=never").send_cli()
        )
        ...
        assert (
            "\u001b[0m"
            in self.nodes[0].cli("-getinfo", "-color=always").send_cli()
        )
        ...
        assert_equal(
            int(cli_get_info["Time offset (s)"]), network_info["timeoffset"]
        )
        ...
        assert_equal(
            cli_get_info["Proxies"], network_info["networks"][0]["proxy"]
        )
        assert_equal(
            Decimal(cli_get_info["Difficulty"]), blockchain_info["difficulty"]
        )
        assert_equal(cli_get_info["Chain"], blockchain_info["chain"])

        self.log.info("Test -getinfo and sugarchain-cli return all proxies")
        self.restart_node(
            0, extra_args=["-proxy=127.0.0.1:9050", "-i2psam=127.0.0.1:7656"]
        )
        network_info = self.nodes[0].getnetworkinfo()
        cli_get_info_string = self.nodes[0].cli("-getinfo").send_cli()
        cli_get_info = cli_get_info_string_to_dict(cli_get_info_string)
        assert_equal(
            ...
            "127.0.0.1:9050 (ipv4, ipv6, onion, cjdns), 127.0.0.1:7656 (i2p)",
        )

        if self.is_specified_wallet_compiled():
            self.log.info(
                "Test -getinfo and sugarchain-cli getwalletinfo return expected wallet info"
            )
            # Explicitly set the output type in order to have consistent tx vsize / fees
            # for both legacy and descriptor wallets (disables the change address type detection algorithm)
            self.restart_node(
                0, extra_args=["-addresstype=bech32", "-changetype=bech32"]
            )
            assert_equal(Decimal(cli_get_info["Balance"]), BALANCE)
            assert "Balances" not in cli_get_info_string
            wallet_info = self.nodes[0].getwalletinfo()
            assert_equal(
                int(cli_get_info["Keypool size"]), wallet_info["keypoolsize"]
            )
            assert_equal(
                int(cli_get_info["Unlocked until"]),
                wallet_info["unlocked_until"],
            )
            assert_equal(
                Decimal(
                    cli_get_info["Transaction fee rate (-paytxfee) (SUGAR/kvB)"]
                ),
                wallet_info["paytxfee"],
            )
            assert_equal(
                Decimal(cli_get_info["Min tx relay fee rate (SUGAR/kvB)"]),
            ...
            # Mine a block to confirm; adds a block reward (50 SUGAR) to the default wallet.
            ...
            cli_get_info_string = (
                self.nodes[0]
                .cli("-getinfo", "-rpcwallet=does-not-exist")
                .send_cli()
            )
            ...
            cli_get_info_string = (
                self.nodes[0].cli("-getinfo", rpcwallet2).send_cli()
            )
            cli_get_info = cli_get_info_string_to_dict(cli_get_info_string)
            assert "Balances" not in cli_get_info_string
            assert_equal(cli_get_info["Wallet"], wallets[1])
            ...
            cli_get_info_string = (
                self.nodes[0].cli("-getinfo", rpcwallet3).send_cli()
            )
            cli_get_info_keys = cli_get_info_string_to_dict(cli_get_info_string)
            assert "Balance" not in cli_get_info_keys
            assert "Balances" not in cli_get_info_string

            # Test sugarchain-cli -generate.
            n1 = 3
            n2 = 4
            ...
            assert_raises_process_error(
                1, BLOCKS_VALUE_OF_ZERO, self.nodes[0].cli("-generate", 0).echo
            )
            assert_raises_process_error(
            ...
            # Test sugarchain-cli -generate with -rpcwallet in multiwallet mode.
            ...
            assert_raises_process_error(
                1,
                BLOCKS_VALUE_OF_ZERO,
                self.nodes[0].cli(rpcwallet2, "-generate", 0).echo,
            )
            assert_raises_process_error(
                1,
                TOO_MANY_ARGS,
                self.nodes[0].cli(rpcwallet2, "-generate", 1, 2, 3).echo,
            )
            ...
            generate = (
                self.nodes[0]
                .cli(rpcwallet2, "-generate", n4, 1000000)
                .send_cli()
            )
            assert_equal(set(generate.keys()), {"address", "blocks"})
            ...
            assert_raises_rpc_error(
                -19, WALLET_NOT_SPECIFIED, self.nodes[0].cli("-generate").echo
            )
            assert_raises_rpc_error(
                -19,
        ...
        self.nodes[
            0
        ].wait_for_cookie_credentials()  # ensure cookie file is available to avoid race condition
