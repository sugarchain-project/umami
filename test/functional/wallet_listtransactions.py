...
class ListTransactionsTest(SugarchainTestFramework):
    ...
    def set_test_params(self):
        ...
        self.extra_args = [
            ["-whitelist=noban@127.0.0.1", "-walletrbf=0"]
        ] * self.num_nodes
    ...
    def run_rbf_opt_in_test(self):
        ...
        tx2 = self.nodes[1].createrawtransaction(
            inputs=inputs, outputs=outputs, replaceable=False
        )
        ...
        tx4 = self.nodes[1].createrawtransaction(
            inputs=inputs, outputs=outputs, replaceable=False
        )
    ...
    def run_externally_generated_address_test(self):
        ...
        self.stop_nodes()
        wallet0 = os.path.join(
            self.nodes[0].datadir,
            self.chain,
            self.default_wallet_name,
            "wallet.dat",
        )
        wallet2 = os.path.join(
        ...
        self.log.info(
            "Verify listtransactions is the same regardless of where the address was generated"
        )
        ...
        self.log.info(
            "Verify labels are persistent on the node that generated the addresses"
        )
    ...
    def run_invalid_parameters_test(self):
        ...
        assert_raises_rpc_error(
            -8, "Negative count", self.nodes[0].listtransactions, count=-1
        )
        assert_raises_rpc_error(
            -8, "Negative from", self.nodes[0].listtransactions, skip=-1
        )
    ...
    def test_op_return(self):
        ...
        op_ret_tx = [
            tx for tx in self.nodes[0].listtransactions() if tx["txid"] == tx_id
        ][0]
