...
class WalletTaprootTest(SugarchainTestFramework):
    ...
    def do_test_addr(self, comment, pattern, privmap, treefn, keys):
        ...
        self.nodes[0].createwallet(
            wallet_name=f"privs_tr_enabled_{wallet_uuid}",
            descriptors=True,
            blank=True,
        )
        self.nodes[0].createwallet(
            wallet_name=f"pubs_tr_enabled_{wallet_uuid}",
            descriptors=True,
            blank=True,
            disable_private_keys=True,
        )
        self.nodes[0].createwallet(
            wallet_name=f"addr_gen_{wallet_uuid}",
            descriptors=True,
            disable_private_keys=True,
            blank=True,
        )
        privs_tr_enabled = self.nodes[0].get_wallet_rpc(
            f"privs_tr_enabled_{wallet_uuid}"
        )
        pubs_tr_enabled = self.nodes[0].get_wallet_rpc(
            f"pubs_tr_enabled_{wallet_uuid}"
        )
        ...
        assert_equal(
            self.nodes[0].getdescriptorinfo(desc)["descriptor"], desc_pub
        )
        result = addr_gen.importdescriptors(
            [{"desc": desc_pub, "active": True, "timestamp": "now"}]
        )
        ...
        result = privs_tr_enabled.importdescriptors(
            [{"desc": desc, "timestamp": "now"}]
        )
        assert result[0]["success"]
        result = pubs_tr_enabled.importdescriptors(
            [{"desc": desc_pub, "timestamp": "now"}]
        )
        assert result[0]["success"]
    ...
    def do_test_sendtoaddress(
        self, comment, pattern, privmap, treefn, keys_pay, keys_change
    ):
        ...
        wallet_uuid = uuid.uuid4().hex
        self.nodes[0].createwallet(
            wallet_name=f"rpc_online_{wallet_uuid}",
            descriptors=True,
            blank=True,
        )
        ...
        assert_equal(
            self.nodes[0].getdescriptorinfo(desc_pay)["descriptor"],
            desc_pay_pub,
        )
        assert_equal(
            self.nodes[0].getdescriptorinfo(desc_change)["descriptor"],
            desc_change_pub,
        )
        result = rpc_online.importdescriptors(
            [{"desc": desc_pay, "active": True, "timestamp": "now"}]
        )
        assert result[0]["success"]
        result = rpc_online.importdescriptors(
            [
        ...
        for i in range(4):
            ...
            self.boring.sendtoaddress(
                address=addr_g,
                amount=Decimal(to_amnt) / 100000000,
                subtractfeefromamount=True,
            )
            self.generatetoaddress(
                self.nodes[0],
                1,
                self.boring.getnewaddress(),
                sync_fun=self.no_op,
            )
            test_balance = int(rpc_online.getbalance() * 100000000)
            ret_amnt = random.randrange(100000, test_balance)
            # Increase fee_rate to compensate for the wallet's inability to estimate fees for script path spends.
            res = rpc_online.sendtoaddress(
                address=self.boring.getnewaddress(),
                amount=Decimal(ret_amnt) / 100000000,
                subtractfeefromamount=True,
                fee_rate=200,
            )
            self.generatetoaddress(
        ...
        # Cleanup
        txid = rpc_online.sendall(recipients=[self.boring.getnewaddress()])[
            "txid"
        ]
        self.generatetoaddress(
            self.nodes[0], 1, self.boring.getnewaddress(), sync_fun=self.no_op
        )
    ...
    def do_test_psbt(
        ...
        wallet_uuid = uuid.uuid4().hex
        self.nodes[0].createwallet(
            wallet_name=f"psbt_online_{wallet_uuid}",
            descriptors=True,
            disable_private_keys=True,
            blank=True,
        )
        self.nodes[1].createwallet(
            wallet_name=f"psbt_offline_{wallet_uuid}",
            descriptors=True,
            blank=True,
        )
        self.nodes[1].createwallet(
            f"key_only_wallet_{wallet_uuid}", descriptors=True, blank=True
        )
        psbt_online = self.nodes[0].get_wallet_rpc(f"psbt_online_{wallet_uuid}")
        psbt_offline = self.nodes[1].get_wallet_rpc(
            f"psbt_offline_{wallet_uuid}"
        )
        key_only_wallet = self.nodes[1].get_wallet_rpc(
            f"key_only_wallet_{wallet_uuid}"
        )
        ...
        result = psbt_online.importdescriptors(
            [{"desc": desc_pay_pub, "active": True, "timestamp": "now"}]
        )
        assert result[0]["success"]
        result = psbt_online.importdescriptors(
            [
                {
                    "desc": desc_change_pub,
                    "active": True,
                    "timestamp": "now",
                    "internal": True,
                }
        ...
        assert result[0]["success"]
        result = psbt_offline.importdescriptors(
            [{"desc": desc_pay, "active": True, "timestamp": "now"}]
        )
        assert result[0]["success"]
        result = psbt_offline.importdescriptors(
        ...
        for key in keys_pay + keys_change:
            result = key_only_wallet.importdescriptors(
                [
                    {
                        "desc": descsum_create(f"wpkh({key['xprv']}/*)"),
                        "timestamp": "now",
                    }
                ]
            )
        ...
        for i in range(4):
            ...
            ret_amnt = random.randrange(100000, test_balance)
            # Increase fee_rate to compensate for the wallet's inability to estimate fees for script path spends.
            psbt = psbt_online.walletcreatefundedpsbt(
                [],
            ...
            for wallet in [psbt_offline, key_only_wallet]:
                ...
                if pattern.startswith("tr("):
                    for psbtin in decoded["inputs"]:
                        ...
                        assert (
                            "taproot_key_path_sig" in psbtin
                            or "taproot_script_path_sigs" in psbtin
                        )
        ...
        psbt = psbt_online.sendall(
            recipients=[self.boring.getnewaddress()], options={"psbt": True}
        )["psbt"]
        res = psbt_offline.walletprocesspsbt(psbt=psbt, finalize=False)
        rawtx = self.nodes[0].finalizepsbt(res["psbt"])["hex"]
        txid = self.nodes[0].sendrawtransaction(rawtx)
    ...
    def do_test(self, comment, pattern, privmap, treefn):
        ...
        self.do_test_psbt(
            ...
            privmap,
            treefn,
            keys[2 * nkeys : 3 * nkeys],
            keys[3 * nkeys : 4 * nkeys],
        )
    ...
    def run_test(self):
        ...
        self.generatetoaddress(
            self.nodes[0], 101, gen_addr, sync_fun=self.no_op
        )

        self.do_test("tr(XPRV)", "tr($1/*)", [True], lambda k1: (key(k1), []))
        self.do_test(
            "tr(H,XPRV)",
            "tr($H,pk($1/*))",
            [True],
        ...
        self.do_test("wpkh(XPRV)", "wpkh($1/*)", [True], None)
        self.do_test(
            "tr(XPRV,{H,{H,XPUB}})",
            "tr($1/*,{pk($H),{pk($H),pk($2/*)}})",
            [True, False],
        ...
        self.do_test(
            ...
            lambda k1, k2: (
                key(k1),
                [[pk(k2), pk(H_POINT)], [pk(H_POINT), pk(k2)]],
            ),
        )
        self.do_test(
            ...
            lambda k1, k2, k3: (
                key(k1),
                [
                    [pk(H_POINT), [pk(H_POINT), pk(k2)]],
                    [pk(H_POINT), [pk(H_POINT), [pk(H_POINT), pk(k3)]]],
                ],
            ),
        )
        self.do_test(
        ...
        self.do_test(
            ...
            lambda k1, k2, k3: (
                key(k2),
                [pk(k2), [pk(k2), multi_a(2, [k1, k2, k3], True)]],
            ),
        )
        ...
        self.do_test(
            ...
            + (",$H" * rnd_pos)
            + ",$1/*"
            + (",$H" * (MAX_PUBKEYS_PER_MULTI_A - 1 - rnd_pos))
            + "))",
            [True, False],
            lambda k1, k2: (
                key(k2),
                [
                    multi_a(
                        ...
                        ([H_POINT] * rnd_pos)
                        + [k1]
                        + ([H_POINT] * (MAX_PUBKEYS_PER_MULTI_A - 1 - rnd_pos)),
                    )
                ],
            ),
        )
        self.do_test("rawtr(XPRV)", "rawtr($1/*)", [True], lambda k1: key(k1))
