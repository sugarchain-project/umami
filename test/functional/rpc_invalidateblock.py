...
class InvalidateTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        blocks = self.generatetodescriptor(
            self.nodes[1],
            10,
        ...
        self.log.info(
            "Verify that invalidating an unknown block throws an error"
        )
        assert_raises_rpc_error(
            -5, "Block not found", self.nodes[1].invalidateblock, "00" * 32
        )
        assert_equal(self.nodes[1].getbestblockhash(), blocks[-1])
