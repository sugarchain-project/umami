...
class BumpFeeTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[1].walletpassphrase(
            WALLET_PASSPHRASE, WALLET_PASSPHRASE_TIMEOUT
        )

        peer_node, rbf_node = self.nodes
        rbf_node_address = rbf_node.getnewaddress()

        # fund rbf node with 10 coins of 0.001 sugar (100,000 satoshis)
    ...
    def test_invalid_parameters(self, rbf_node, peer_node, dest_address):
        ...
        assert (
            ...
            and rbfid in peer_node.getrawmempool()
        )
        ...
        assert_raises_rpc_error(
            -8,
            "Insufficient total fee 0.00000141",
            rbf_node.bumpfee,
            rbfid,
        ...
        assert_raises_rpc_error(
            -4,
            "Specified or calculated fee 0.141 is too high (cannot be higher than -maxtxfee 0.10",
            rbf_node.bumpfee,
            rbfid,
            {"fee_rate": TOO_HIGH},
        )
        ...
        for zero_value in [0, 0.000, 0.00000000, "0", "0.000", "0.00000000"]:
            assert_raises_rpc_error(
                -8, msg, rbf_node.bumpfee, rbfid, {"fee_rate": zero_value}
            )
        msg = "Invalid amount"
        ...
        for invalid_value in [
            ...
            assert_raises_rpc_error(
                -3, msg, rbf_node.bumpfee, rbfid, {"fee_rate": invalid_value}
            )
        ...
        assert_raises_rpc_error(
            -3, "Amount out of range", rbf_node.bumpfee, rbfid, {"fee_rate": -1}
        )
        # Test type error.
        ...
        self.log.info(
            "Test explicit fee rate raises RPC error if both fee_rate and conf_target are passed"
        )
        assert_raises_rpc_error(
            -8,
            "Cannot specify both conf_target and fee_rate. Please provide either a confirmation "
            "target in blocks for automatic fee estimation, or an explicit fee rate.",
            rbf_node.bumpfee,
            rbfid,
            {"conf_target": NORMAL, "fee_rate": NORMAL},
        )

        self.log.info(
            "Test explicit fee rate raises RPC error if both fee_rate and estimate_mode are passed"
        )
        assert_raises_rpc_error(
            -8,
            ...
            rbf_node.bumpfee,
            rbfid,
            {"estimate_mode": "economical", "fee_rate": NORMAL},
        )

        self.log.info("Test invalid conf_target settings")
        assert_raises_rpc_error(
            -8,
            "confTarget and conf_target options should not both be set",
            rbf_node.bumpfee,
            rbfid,
            {"confTarget": 123, "conf_target": 456},
        )
        ...
        for mode in ["foo", Decimal("3.1415"), "sat/B", "SUGAR/kB"]:
            assert_raises_rpc_error(
                -8,
        ...
        assert_raises_rpc_error(
            -8,
            "Invalid parameter, duplicated address: " + dest_address,
            rbf_node.bumpfee,
            rbfid,
            {"outputs": [{dest_address: 0.1}, {dest_address: 0.2}]},
        )
        assert_raises_rpc_error(
            -8,
            ...
            rbf_node.bumpfee,
            rbfid,
            {"outputs": [{"data": "deadbeef"}, {"data": "deadbeef"}]},
        )
...
def test_simple_bumpfee_succeeds(self, mode, rbf_node, peer_node, dest_address):
    ...
    self.sync_mempools((rbf_node, peer_node))
    assert (
        rbfid in rbf_node.getrawmempool() and rbfid in peer_node.getrawmempool()
    )
    if mode == "fee_rate":
        ...
        bumped_psbt = rbf_node.psbtbumpfee(
            rbfid, {"outputs": {new_address: 0.0003}}
        )
...
def test_segwit_bumpfee_succeeds(self, rbf_node, dest_address):
    ...
    segwitid = rbf_node.send(
        {segwit_out: "0.0009"}, options={"change_position": 1}
    )["txid"]

    rbfraw = rbf_node.createrawtransaction(
        [{"txid": segwitid, "vout": 0, "sequence": MAX_BIP125_RBF_SEQUENCE}],
        {
...
def test_notmine_bumpfee(self, rbf_node, peer_node, dest_address):
    self.log.info(
        "Test that it cannot bump fee if non-owned inputs are included"
    )
    ...
    fee = Decimal("0.001")
    utxos = [
        node.listunspent(query_options={"minimumAmount": fee})[-1]
        for node in (rbf_node, peer_node)
    ]
    inputs = [
        {
            "txid": utxo["txid"],
    ...
    psbt = rbf_node.psbtbumpfee(
        txid=rbfid, options={"fee_rate": old_feerate + 10}
    )
    finish_psbtbumpfee(psbt["psbt"])
...
def test_bumpfee_with_descendant_fails(
    self, rbf_node, rbf_node_address, dest_address
):
    self.log.info("Test that fee cannot be bumped when it has descendant")
    # parent is send-to-self, so we don't have to check which output is change when creating the child tx
    parent_id = spend_one_input(rbf_node, rbf_node_address)
    tx = rbf_node.createrawtransaction(
        [{"txid": parent_id, "vout": 0}], {dest_address: 0.00020000}
    )
...
def test_bumpfee_with_abandoned_descendant_succeeds(
    self, rbf_node, rbf_node_address, dest_address
):
    self.log.info(
        "Test that fee can be bumped when it has abandoned descendant"
    )
    # parent is send-to-self, so we don't have to check which output is change when creating the child tx
    parent_id = spend_one_input(rbf_node, rbf_node_address)
    # Submit child transaction with low fee
    child_id = rbf_node.send(
        outputs={dest_address: 0.00020000},
        options={"inputs": [{"txid": parent_id, "vout": 0}], "fee_rate": 2},
    )["txid"]
...
def test_small_output_with_feerate_succeeds(self, rbf_node, dest_address):
    ...
    self.generatetoaddress(
        rbf_node, COINBASE_MATURITY + 1, rbf_node.getnewaddress()
    )
    rbfid = spend_one_input(rbf_node, dest_address)
    ...
    self.log.info(
        "Keep bumping until transaction fee out-spends non-destination value"
    )
    tx_fee = 0
    while True:
        input_list = rbf_node.getrawtransaction(rbfid, 1)["vin"]
...
def test_dust_to_fee(self, rbf_node, dest_address):
    ...
    # The DER formatting used by Sugarchain to serialize ECDSA signatures means that signatures can have a
    ...
    if not 140 <= fulltx["vsize"] <= 141:
        raise AssertionError(
            "Invalid tx vsize of {} (140-141 expected), full tx: {}".format(
                fulltx["vsize"], fulltx
            )
        )
    # Bump with fee_rate of 350.25 sat/vB vbytes to create dust.
    # Expected fee is 141 vbytes * fee_rate 0.00350250 SUGAR / 1000 vbytes = 0.00049385 SUGAR.
    # or occasionally 140 vbytes * fee_rate 0.00350250 SUGAR / 1000 vbytes = 0.00049035 SUGAR.
    # Dust should be dropped to the fee, so actual bump fee is 0.00050000 SUGAR.
...
def test_settxfee(self, rbf_node, dest_address):
    ...
    actual_feerate = (
        bumped_tx["fee"]
        * 1000
        / rbf_node.getrawtransaction(bumped_tx["txid"], True)["vsize"]
    )
    # Assert that the difference between the requested feerate and the actual
    # feerate of the bumped transaction is small.
    assert_greater_than(
        Decimal("0.00001000"), abs(requested_feerate - actual_feerate)
    )
...
def test_maxtxfee_fails(self, rbf_node, dest_address):
    ...
    # expected bump fee of 141 vbytes * 0.00200000 SUGAR / 1000 vbytes = 0.00002820 SUGAR
    ...
    assert_raises_rpc_error(
        -4,
        "Unable to create transaction. Fee exceeds maximum configured by user (e.g. -maxtxfee, maxfeerate)",
        rbf_node.bumpfee,
        rbfid,
    )
    self.restart_node(1, self.extra_args[1])
...
def test_watchonly_psbt(self, peer_node, rbf_node, dest_address):
    ...
    rbf_node.createwallet(
        wallet_name="signer", disable_private_keys=False, blank=True
    )
    ...
    rbf_node.createwallet(
        wallet_name="watcher", disable_private_keys=True, blank=True
    )
    ...
    # Ensure the payment amount + change can be fully funded using one of the 0.001SUGAR inputs.
    psbt = watcher.walletcreatefundedpsbt(
        [watcher.listunspent()[0]],
        {dest_address: 0.0005},
        0,
        {"fee_rate": 1, "add_inputs": False},
        True,
    )["psbt"]
    psbt_signed = signer.walletprocesspsbt(
        psbt=psbt, sign=True, sighashtype="ALL", bip32derivs=True
    )
    ...
    assert_equal(
        bumped_psbt["origfee"], -watcher.gettransaction(original_txid)["fee"]
    )
    assert not watcher.finalizepsbt(bumped_psbt["psbt"])["complete"]

    # Sign bumped transaction
    bumped_psbt_signed = signer.walletprocesspsbt(
        psbt=bumped_psbt["psbt"], sign=True, sighashtype="ALL", bip32derivs=True
    )
...
def test_rebumping(self, rbf_node, dest_address):
    self.log.info(
        "Test that re-bumping the original tx fails, but bumping successor works"
    )
    rbfid = spend_one_input(rbf_node, dest_address)
    bumped = rbf_node.bumpfee(rbfid, {"fee_rate": ECONOMICAL})
    assert_raises_rpc_error(
        -4,
        f"Cannot bump transaction {rbfid} which was already bumped by transaction {bumped['txid']}",
        rbf_node.bumpfee,
        rbfid,
        {"fee_rate": NORMAL},
    )
...
def test_rebumping_not_replaceable(self, rbf_node, dest_address):
    ...
    rbfid = spend_one_input(rbf_node, dest_address)
    bumped = rbf_node.bumpfee(
        rbfid, {"fee_rate": ECONOMICAL, "replaceable": False}
    )
    assert_raises_rpc_error(
        -4,
        "Transaction is not BIP 125 replaceable",
...
def test_bumpfee_already_spent(self, rbf_node, dest_address):
    ...
    spent_input = rbf_node.gettransaction(txid=txid, verbose=True)["decoded"][
        "vin"
    ][0]
    assert_raises_rpc_error(
        -1,
        f"{spent_input['txid']}:{spent_input['vout']} is already spent",
        rbf_node.bumpfee,
        txid,
        {"fee_rate": NORMAL},
    )
...
def test_unconfirmed_not_spendable(self, rbf_node, rbf_node_address):
    self.log.info(
        "Test that unconfirmed outputs from bumped txns are not spendable"
    )
    ...
    assert_equal(
        [
            t
            for t in rbf_node.listunspent(minconf=0, include_unsafe=False)
            if t["txid"] == bumpid
        ],
        [],
    )
    ...
    assert_equal(
        sum(
            ...
            for t in rbf_node.listunspent(minconf=0, include_unsafe=False)
            if t["txid"] == rbfid
            and t["address"] == rbf_node_address
            and t["spendable"]
        ),
        1,
    )
...
def test_bumpfee_metadata(self, rbf_node, dest_address):
    ...
    rbfid = rbf_node.sendtoaddress(
        dest_address, 49, "comment value", "to value"
    )
    bumped_tx = rbf_node.bumpfee(rbfid)
...
def test_change_script_match(self, rbf_node, dest_address):
    ...
    def get_change_address(tx):
        ...
        return [
            address
            for address in txout_addresses
...
def spend_one_input(
    node, dest_address, change_size=Decimal("0.00049000"), data=None
):
    tx_input = dict(
        sequence=MAX_BIP125_RBF_SEQUENCE,
        **next(
            ...
            if u["amount"] == Decimal("0.00100000")
        ),
    )
...
def test_no_more_inputs_fails(self, rbf_node, dest_address):
    self.log.info(
        "Test that bumpfee fails when there are no available confirmed outputs"
    )
...
def test_feerate_checks_replaced_outputs(self, rbf_node, peer_node):
    ...
    inc_fee_rate = max(
        rbf_node.getmempoolinfo()["incrementalrelayfee"], Decimal(0.00005000)
    )  # Wallet has a fixed incremental relay fee of 5 sat/vb
    # RPC gives us fee as negative
    min_fee = (
        -tx_details["fee"] + get_fee(est_bumped_size, inc_fee_rate)
    ) * Decimal(1e8)
    ...
    assert_raises_rpc_error(
        -8,
        "Insufficient total fee",
        rbf_node.bumpfee,
        tx_res["txid"],
        {"fee_rate": min_fee_rate - 1, "outputs": new_outputs},
    )

    # Bumpfee and replace all outputs with a single one using the minimum feerate
    rbf_node.bumpfee(
        tx_res["txid"], {"fee_rate": min_fee_rate, "outputs": new_outputs}
    )
    self.clear_mempool()
