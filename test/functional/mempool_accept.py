...
class MempoolAcceptanceTest(SugarchainTestFramework):
    ...
    def check_mempool_result(self, result_expected, *args, **kwargs):
        ...
        assert_equal(
            self.nodes[0].getmempoolinfo()["size"], self.mempool_size
    ...
    def run_test(self):
        ...
        txid_in_block = self.wallet.sendrawtransaction(
            from_node=node, tx_hex=raw_tx_in_block
        )
        ...
        utxo_to_spend = self.wallet.get_utxo(
            txid=txid_in_block
        )  # use 0.3 SUGAR UTXO
        tx = self.wallet.create_self_transfer(
            utxo_to_spend=utxo_to_spend, sequence=MAX_BIP125_RBF_SEQUENCE
        )["tx"]
        ...
        txid_spend_both = self.wallet.sendrawtransaction(
            from_node=node, tx_hex=raw_tx_spend_both
        )
        self.generate(node, 1)
        self.mempool_size = 0
        ...
        tx.vin = [tx.vin[0]] * math.ceil(
            MAX_BLOCK_WEIGHT // 4 / len(tx.vin[0].serialize())
        )
        self.check_mempool_result(
            result_expected=[
                {
        ...
        raw_tx_coinbase_spent = node.getrawtransaction(
            txid=node.decoderawtransaction(hexstring=raw_tx_in_block)["vin"][0][
                "txid"
            ]
        )
        ...
        tx.vout[0].scriptPubKey = keys_to_multisig_script(
            [pubkey] * 3, k=2
        ...
        output_p2sh_burn = CTxOut(
            nValue=540, scriptPubKey=script_to_p2sh_script(b"burn")
        )
        num_scripts = 100000 // len(
            output_p2sh_burn.serialize()
        ...
        tx.vin[
            0
        ].nSequence -= 1  # Should be non-max, so locktime is not ignored
        ...
        seed_tx = self.wallet.send_to(
            from_node=node,
            scriptPubKey=script_to_p2wsh_script(CScript([OP_TRUE])),
            amount=COIN,
        )
        self.generate(node, 1)
        ...
        tx.vin.append(
            CTxIn(
                COutPoint(int(seed_tx[0], 16), seed_tx[1]), b"", SEQUENCE_FINAL
            )
        )
        ...
        assert_equal(
            len(tx.serialize_without_witness()), MIN_STANDARD_TX_NONWITNESS_SIZE
        )
        self.check_mempool_result(
            result_expected=[
                {
