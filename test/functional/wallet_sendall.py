...
def cleanup(func):
    def wrapper(self):
        try:
            ...
            assert_equal(
                0, self.wallet.getbalances()["mine"]["trusted"]
            )  # wallet is empty

    return wrapper


class SendallTest(SugarchainTestFramework):
    # Setup and helpers
    ...
    def assert_tx_has_output(self, tx, addr, value=None):
        for output in tx["decoded"]["vout"]:
            if (
                ...
                and value is None
                or value == output["value"]
            ):
                return
        raise AssertionError(
            "Output to {} not present or wrong amount".format(addr)
        )
    ...
    def test_sendall_success(self, sendall_args, remaining_balance=0):
        ...
        assert_equal(
            remaining_balance, self.wallet.getbalances()["mine"]["trusted"]
        )

        assert_equal(sendall_tx_receipt["complete"], True)
        return self.wallet.gettransaction(
            txid=sendall_tx_receipt["txid"], verbose=True
        )
    ...
    def sendall_two_utxos(self):
        ...
        tx_from_wallet = self.test_sendall_success(
            sendall_args=[self.remainder_target]
        )

        self.assert_tx_has_outputs(
            tx=tx_from_wallet,
            expected_outputs=[
                {
                    "address": self.remainder_target,
                    "value": pre_sendall_balance + tx_from_wallet["fee"],
                }  # fee is neg
            ],
        )
        self.assert_balance_swept_completely(
            tx_from_wallet, pre_sendall_balance
        )

    @cleanup
    def sendall_split(self):
        self.log.info(
            "Testing sendall where two recipients have unspecified amount"
        )
        pre_sendall_balance = self.add_utxos([1, 2, 3, 15])
        tx_from_wallet = self.test_sendall_success(
            [self.remainder_target, self.split_target]
        )
    ...
    def sendall_and_spend(self):
        self.log.info(
            "Testing sendall in combination with paying specified amount to recipient"
        )
        pre_sendall_balance = self.add_utxos([8, 13])
        tx_from_wallet = self.test_sendall_success(
            [{self.recipient: 5}, self.remainder_target]
        )
        ...
        self.assert_tx_has_outputs(
            ...
            expected_outputs=[
                ...
                {
                    "address": self.remainder_target,
                    "value": pre_sendall_balance - 5 + tx_from_wallet["fee"],
                },
            ],
        )
        self.assert_balance_swept_completely(
    ...
    def sendall_invalid_recipient_addresses(self):
        self.log.info(
            "Test having only recipient with specified amount, missing recipient with unspecified amount"
        )
        self.add_utxos([12, 9])
    ...
    def sendall_duplicate_recipient(self):
        ...
        assert_raises_rpc_error(
            -8,
            "Invalid parameter, duplicated address: {}".format(
    ...
    def sendall_invalid_amounts(self):
        ...
        expected_tx = self.wallet.sendall(
            recipients=[{self.recipient: 5}, self.remainder_target],
        ...
        assert_raises_rpc_error(
            -6,
            "Insufficient funds for fees after creating specified outputs.",
            self.wallet.sendall,
            [{self.recipient: pre_sendall_balance}, self.remainder_target],
        )
        assert_raises_rpc_error(
            -8,
            "Specified output amount to {} is below dust threshold".format(
                self.recipient
            ),
            self.wallet.sendall,
            [{self.recipient: 0.00000001}, self.remainder_target],
        )
        assert_raises_rpc_error(
            -6,
            ...
            self.wallet.sendall,
            [
                {self.recipient: pre_sendall_balance - fee},
                self.remainder_target,
            ],
        )
        assert_raises_rpc_error(
            ...
            [
                {
                    self.recipient: pre_sendall_balance
                    - fee
                    - Decimal(0.00000010)
                },
                self.remainder_target,
    ...
    def sendall_negative_effective_value(self):
        self.log.info(
            "Test that sendall fails if all UTXOs have negative effective value"
        )
    ...
    def sendall_with_send_max(self):
        self.log.info(
            "Check that `send_max` option causes negative value UTXOs to be left behind"
        )
        self.add_utxos([0.00000400, 0.00000300, 1])

        # sendall with send_max
        sendall_tx_receipt = self.wallet.sendall(
            recipients=[self.remainder_target],
            fee_rate=300,
            options={"send_max": True},
        )
        tx_from_wallet = self.wallet.gettransaction(
            txid=sendall_tx_receipt["txid"], verbose=True
        )
        ...
        self.assert_tx_has_outputs(
            ...
            [
                {
                    "address": self.remainder_target,
                    "value": 1 + tx_from_wallet["fee"],
                }
            ],
        )
        assert_equal(
            self.wallet.getbalances()["mine"]["trusted"], Decimal("0.00000700")
        )
    ...
    def sendall_specific_inputs(self):
        ...
        sendall_tx_receipt = self.wallet.sendall(
            recipients=[self.remainder_target], options={"inputs": [utxo]}
        )
        tx_from_wallet = self.wallet.gettransaction(
            txid=sendall_tx_receipt["txid"], verbose=True
        )
    ...
    def sendall_fails_on_missing_input(self):
        ...
        self.log.info(
            "Test sendall fails because specified UTXO is not available"
        )
        self.add_utxos([16, 5])
        ...
        assert_raises_rpc_error(
            ...
            self.wallet.sendall,
            recipients=[self.remainder_target],
            options={"inputs": [{"txid": spent_utxo["txid"], "vout": 1000}]},
        )
        ...
        assert_raises_rpc_error(
            -8,
            "Input not available. UTXO ({}:{}) was already spent.".format(
                spent_utxo["txid"], spent_utxo["vout"]
            ),
            self.wallet.sendall,
            recipients=[self.remainder_target],
            options={"inputs": [spent_utxo]},
        )
        ...
        assert_raises_rpc_error(
            -8,
            "Input not found. UTXO ({}:{}) is not part of wallet.".format(
                foreign_utxo["txid"], foreign_utxo["vout"]
            ),
            self.wallet.sendall,
            recipients=[self.remainder_target],
    ...
    def sendall_watchonly_specific_inputs(self):
        ...
        self.nodes[0].createwallet(
            wallet_name="watching", disable_private_keys=True
        )
    ...
    def sendall_with_minconf(self):
        ...
        self.log.info(
            "Test sendall fails because there are no utxos with enough confirmations specified by minconf"
        )
        ...
        self.log.info(
            "Test sendall only spends utxos with a specified number of confirmations when minconf is used"
        )
        self.wallet.sendall(
            recipients=[self.remainder_target],
            fee_rate=300,
    ...
    def sendall_with_maxconf(self):
        ...
        self.log.info(
            "Test sendall fails because there are no utxos with enough confirmations specified by maxconf"
        )
        assert_raises_rpc_error(
            -6,
            "Total value of UTXO pool too low to pay for transaction. Try using lower feerate or excluding uneconomic UTXOs with 'send_max' option.",
        ...
        self.log.info(
            "Test sendall only spends utxos with a specified number of confirmations when maxconf is used"
        )
        self.wallet.sendall(
            recipients=[self.remainder_target],
            fee_rate=300,
    ...
    def sendall_fails_with_transaction_too_large(self):
        self.log.info(
            "Test that sendall fails if resulting transaction is too large"
        )
