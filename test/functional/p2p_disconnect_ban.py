...
class DisconnectBanTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(
            len(self.nodes[1].getpeerinfo()), 2
        )  # node1 should have 2 connections to node0 at this point
        self.nodes[1].setban(subnet="127.0.0.1", command="add")
        self.wait_until(
            lambda: len(self.nodes[1].getpeerinfo()) == 0, timeout=10
        )
        assert_equal(
        ...
        self.wait_until(
            lambda: len(self.nodes[1].getpeerinfo()) == 1, timeout=10
        )
        assert not [
            node
