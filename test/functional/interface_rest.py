...
class RESTTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        resp = self.test_rest_request(
            uri=f"/tx/{INVALID_PARAM}", ret_type=RetType.OBJ, status=400
        )
        assert_equal(
        ...
        resp = self.test_rest_request(
            uri=f"/tx/{UNKNOWN_PARAM}", ret_type=RetType.OBJ, status=404
        )
        assert_equal(
            resp.read().decode("utf-8").rstrip(), f"{UNKNOWN_PARAM} not found"
        )
        ...
        self.log.info(
            "Test the /block, /blockhashbyheight, /headers, and /blockfilterheaders URIs"
        )
        bb_hash = self.nodes[0].getbestblockhash()
        ...
        assert_equal(
            self.test_rest_request(
                f"/headers/{UNKNOWN_PARAM}", query_params={"count": 1}
            ),
            [],
        )
        self.test_rest_request(
            f"/block/{UNKNOWN_PARAM}", status=404, ret_type=RetType.OBJ
        )
        ...
        assert_equal(
            response_bytes[:BLOCK_HEADER_SIZE].hex().encode(),
        ...
        resp = self.test_rest_request(
            f"/blockhashbyheight/{INVALID_PARAM}",
            ret_type=RetType.OBJ,
            status=400,
        )
        assert_equal(
        ...
        assert_equal(
            resp.read().decode("utf-8").rstrip(), "Block height out of range"
        )
        resp = self.test_rest_request(
        ...
        self.test_rest_request(
            "/blockhashbyheight/", ret_type=RetType.OBJ, status=400
        )
        ...
        for invalid_uri in [
            ...
            resp = self.test_rest_request(
                invalid_uri, ret_type=RetType.OBJ, status=400
            )
            assert_equal(
                resp.read().decode("utf-8").rstrip(),
        ...
        resp = self.test_rest_request(
            f"/blockfilterheaders/{INVALID_PARAM}/{bb_hash}",
            ret_type=RetType.OBJ,
            status=400,
        )
        assert_equal(
            resp.read().decode("utf-8").rstrip(),
            f"Unknown filtertype {INVALID_PARAM}",
        )
        resp = self.test_rest_request(
            f"/blockfilterheaders/basic/{INVALID_PARAM}",
            ret_type=RetType.OBJ,
            status=400,
        )
        assert_equal(
        ...
        json_obj = self.test_rest_request(
            "/mempool/contents",
            query_params={"verbose": "true", "mempool_sequence": "false"},
        )
        assert_equal(json_obj, raw_mempool_verbose)

        # Check the mempool response for not verbose
        json_obj = self.test_rest_request(
            "/mempool/contents", query_params={"verbose": "false"}
        )
        ...
        json_obj = self.test_rest_request(
            "/mempool/contents",
            query_params={"verbose": "false", "mempool_sequence": "true"},
        )
        ...
        resp = self.test_rest_request(
            ...
            ret_type=RetType.OBJ,
            status=400,
            query_params={"verbose": "true", "mempool_sequence": "true"},
        )
        assert_equal(
        ...
        previous_bb_hash = self.nodes[0].getblockhash(
            self.nodes[0].getblockcount() - 1
        )
        deployment_info = self.nodes[0].getdeploymentinfo(previous_bb_hash)
        assert_equal(
            deployment_info,
            self.test_rest_request(f"/deploymentinfo/{previous_bb_hash}"),
        )

        non_existing_blockhash = (
            "42759cde25462784395a337460bde75f58e73d3f08bd31fdc3507cbac856a2c4"
        )
        resp = self.test_rest_request(
            f"/deploymentinfo/{non_existing_blockhash}",
            ret_type=RetType.OBJ,
            status=400,
        )
        ...
        resp = self.test_rest_request(
            f"/deploymentinfo/{INVALID_PARAM}", ret_type=RetType.OBJ, status=400
        )
        assert_equal(
            resp.read().decode("utf-8").rstrip(),
            f"Invalid hash: {INVALID_PARAM}",
