...
class P2PPermissionsTests(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[1].replace_in_config(
            [("bind=127.0.0.1", "whitebind=bloomfilter,forcerelay@" + ip_port)]
        )
        self.checkpermission(
        ...
        self.nodes[1].replace_in_config(
            [("whitebind=bloomfilter,forcerelay@" + ip_port, "bind=127.0.0.1")]
        )
    ...
    def check_tx_relay(self):
        ...
        p2p_rebroadcast_wallet.send_txs_and_test(
            ...
            self.nodes[1],
            success=False,
            reject_reason="{} from peer=0 was not accepted: txn-mempool-conflict".format(
                txid
            ),
        )
        ...
        p2p_rebroadcast_wallet.send_txs_and_test(
            ...
            self.nodes[1],
            success=False,
            reject_reason="Not relaying non-mempool transaction {} from forcerelay peer=0".format(
                txid
            ),
        )
        ...
        for p in expectedPermissions:
            if p not in peerinfo["permissions"]:
                raise AssertionError(
                    "Expected permissions %r is not granted." % p
                )
