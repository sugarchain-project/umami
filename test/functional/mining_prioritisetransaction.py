...
class PrioritiseTransactionTest(SugarchainTestFramework):
    ...
    def test_diamond(self):
        ...
        self.nodes[0].prioritisetransaction(
            txid=txid_b, fee_delta=int(fee_delta_b * COIN)
        )
        self.nodes[0].prioritisetransaction(
            txid=txid_c, fee_delta=int(fee_delta_c_1 * COIN)
        )
        self.nodes[0].prioritisetransaction(
            txid=txid_c, fee_delta=int(fee_delta_c_2 * COIN)
        )
        raw_before[txid_a]["fees"]["descendant"] += (
            fee_delta_b + fee_delta_c_1 + fee_delta_c_2
        )
        ...
        raw_before[txid_c]["fees"]["descendant"] += (
            fee_delta_c_1 + fee_delta_c_2
        )
        raw_before[txid_d]["fees"]["ancestor"] += (
            fee_delta_b + fee_delta_c_1 + fee_delta_c_2
        )
    ...
    def run_test(self):
        ...
        self.nodes[0].prioritisetransaction(
            txid=txids[0][0], fee_delta=int(3 * base_fee * COIN)
        )
