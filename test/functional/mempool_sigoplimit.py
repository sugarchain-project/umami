...
class BytesPerSigOpTest(SugarchainTestFramework):
    ...
    def test_sigops_limit(self, bytes_per_sigop, num_sigops):
        sigop_equivalent_vsize = ceil(
            num_sigops * bytes_per_sigop / WITNESS_SCALE_FACTOR
        )
        self.log.info(
            f"- {num_sigops} sigops (equivalent size of {sigop_equivalent_vsize} vbytes)"
        )
        ...
        tx = self.create_p2wsh_spending_tx(
            witness_script, CScript([OP_RETURN, b"X" * 256])
        )
        ...
        tx.vout[0].scriptPubKey = CScript(
            [OP_RETURN, b"X" * (256 + vsize_to_pad)]
        )
        ...
        tx.vout[0].scriptPubKey = CScript(
            [OP_RETURN, b"X" * (256 + vsize_to_pad + 1)]
        )
        res = self.nodes[0].testmempoolaccept([tx.serialize().hex()])[0]
        assert_equal(res["allowed"], True)
        ...
        self.nodes[0].sendrawtransaction(
            hexstring=tx.serialize().hex(), maxburnamount="1.0"
        )
        ...
        assert_equal(
            entry_child["ancestorsize"],
            sigop_equivalent_vsize + parent_tx.get_vsize(),
        )
        ...
        assert_equal(
            entry_parent["descendantsize"],
            parent_tx.get_vsize() + sigop_equivalent_vsize,
        )

    def run_test(self):
        self.wallet = MiniWallet(self.nodes[0])

        for bytes_per_sigop in (
            ...
            if bytes_per_sigop == DEFAULT_BYTES_PER_SIGOP:
                self.log.info(
                    f"Test default sigops limit setting ({bytes_per_sigop} bytes per sigop)..."
                )
            else:
                bytespersigop_parameter = f"-bytespersigop={bytes_per_sigop}"
                self.log.info(
                    f"Test sigops limit setting {bytespersigop_parameter}..."
                )
                self.restart_node(
                    0, extra_args=[bytespersigop_parameter] + self.extra_args[0]
                )
