...
class ScanblocksTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        wallet.send_to(
            ...
            scriptPubKey=address_to_scriptpubkey(
                "mkS4HXoTYWRTescLGaUTGbtTTYX5EjJyEE"
            ),
            amount=1 * COIN,
        )
        ...
        assert (
            blockhash
            not in node.scanblocks("start", [f"addr({addr_1})"], height_new)[
                "relevant_blocks"
            ]
        )
        ...
        assert (
            blockhash
            in node.scanblocks("start", [f"addr({addr_1})"], height)[
                "relevant_blocks"
            ]
        )
        for v in [False, True]:
            assert (
                ...
                in node.scanblocks(
                    ...
                    start_height=height,
                    options={"filter_false_positives": v},
                )["relevant_blocks"]
            )
        ...
        assert (
            blockhash
            in node.scanblocks("start", [f"addr({addr_1})"], height, height)[
                "relevant_blocks"
            ]
        )
        ...
        assert (
            blockhash
            not in node.scanblocks("start", [f"addr({addr_1})"], 0, height - 1)[
                "relevant_blocks"
            ]
        )
        ...
        assert (
            ...
            in node.scanblocks(
                "start",
                [{"desc": f"pkh({parent_key}/*)", "range": [0, 100]}],
                height,
            )["relevant_blocks"]
        )
        ...
        genesis_coinbase_hash = bip158_basic_element_hash(
            genesis_coinbase_spk, 1, genesis_blockhash
        )
        false_positive_hash = bip158_basic_element_hash(
            false_positive_spk, 1, genesis_blockhash
        )
        ...
        assert (
            genesis_blockhash
            in node.scanblocks(
                "start", [{"desc": f"raw({genesis_coinbase_spk.hex()})"}], 0, 0
            )["relevant_blocks"]
        )
        assert (
            genesis_blockhash
        ...
        assert (
            ...
            in node.scanblocks(
                "start",
                [{"desc": f"raw({genesis_coinbase_spk.hex()})"}],
                0,
                0,
                "basic",
        ...
        assert_raises_rpc_error(
            -8, "Invalid action 'foobar'", node.scanblocks, "foobar"
        )
