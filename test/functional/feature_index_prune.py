...
class FeatureIndexPruneTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info(
            "check if we can access blockfilters and coinstats when pruning is enabled but no blocks are actually pruned"
        )
        ...
        for node in stats_nodes:
            assert node.gettxoutsetinfo(hash_type="muhash", hash_or_height=tip)[
                "muhash"
            ]
        ...
        for node in stats_nodes:
            assert node.gettxoutsetinfo(
                hash_type="muhash", hash_or_height=height_hash
            )["muhash"]
        ...
        for node in stats_nodes:
            ...
            assert_raises_rpc_error(
                -8, msg, node.gettxoutsetinfo, "muhash", height_hash
            )
        ...
        self.log.info(
            "make sure that we can continue with the partially synced indices after having pruned up to the index height"
        )
        ...
        for i, msg in enumerate([filter_msg, stats_msg, filter_msg]):
            self.nodes[i].assert_start_raises_init_error(
                extra_args=self.extra_args[i], expected_msg=msg
            )
