...
class InvalidBlockRequestTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        tx1 = create_tx_with_script(
            block1.vtx[0], 0, script_sig=bytes([OP_TRUE]), amount=50 * COIN
        )
        ...
        block3 = create_block(
            tip, create_coinbase(height, nValue=100), block_time
        )
        block_time += 1
        block3.solve()
        ...
        tx3.rehash()
        block4 = create_block(
            tip, create_coinbase(height), block_time, txlist=[tx3]
        )
        block4.solve()
