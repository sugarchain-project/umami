...
class BIP66Test(SugarchainTestFramework):
    ...
    def create_tx(self, input_txid):
        utxo_to_spend = self.miniwallet.get_utxo(
            txid=input_txid, mark_as_spent=False
        )
    ...
    def run_test(self):
        ...
        block = create_block(
            int(tip, 16),
            create_coinbase(DERSIG_HEIGHT - 1),
            block_time,
            txlist=[spendtx],
        )
        block.solve()
        ...
        block_time += 1
        block = create_block(
            tip, create_coinbase(DERSIG_HEIGHT), block_time, version=2
        )
        block.solve()
