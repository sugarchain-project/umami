...
class NotificationsTest(SugarchainTestFramework):
    ...
    def setup_network(self):
        self.wallet = "".join(
            ...
            if chr(i) not in FILE_CHARS_DISALLOWED
        )
        ...
        self.shutdownnotify_dir = os.path.join(
            self.options.tmpdir, "shutdownnotify"
        )
    ...
    def run_test(self):
        ...
        if self.is_wallet_compiled():
            ...
            tx_details = list(
                map(
                    lambda t: (t["txid"], t["blockheight"], t["blockhash"]),
            ...
            self.generatetoaddress(
                self.nodes[0], 100, ADDRESS_BCRT1_UNSPENDABLE
            )
            ...
            # https://github.com/sugarchain/sugarchain/pull/9371, it might be better
            ...
            assert_equal(
                self.nodes[1].gettransaction(bump1)["confirmations"], 1
            )
            ...
            assert_equal(
                self.nodes[0].gettransaction(bump2)["confirmations"], 1
            )
            ...
            self.expect_wallet_notify(
                [
                    (bump2, blockheight2, blockhash2),
                    (tx2, -1, UNCONFIRMED_HASH_STRING),
                ]
            )
            assert_equal(
                self.nodes[1].gettransaction(bump2)["confirmations"], 1
            )
        ...
        self.stop_nodes()
        self.wait_until(
            lambda: os.path.isfile(self.shutdownnotify_file), timeout=10
        )
    ...
    def expect_wallet_notify(self, tx_details):
        ...
        assert_equal(
            sorted(
                notify_outputname(self.wallet, tx_id)
                for tx_id, _, _ in tx_details
            ),
