...
class UpgradeWalletTest(SugarchainTestFramework):
    ...
    def dumb_sync_blocks(self):
        ...
        Further info: https://github.com/sugarchain/sugarchain/pull/18774#discussion_r416967844
        """
        ...
        for i in range(height, to_height + 1):
            b = node_from.getblock(
                blockhash=node_from.getblockhash(i), verbose=0
            )
            v16_3_node.submitblock(b)
        assert_equal(v16_3_node.getblockcount(), to_height)

    def test_upgradewallet(
        self,
        wallet,
        previous_version,
        ...
        assert_equal(
            wallet.upgradewallet(requested_version),
            {
                ...
                if unchanged
                else "Wallet upgraded successfully from version {} to version {}.".format(
    ...
    def test_upgradewallet_error(
        self, wallet, previous_version, requested_version, msg
    ):
    ...
    def run_test(self):
        self.generatetoaddress(
            ...
            sync_fun=lambda: self.dumb_sync_blocks(),
        )
        ...
        node_master_wallet_dir = os.path.join(
            node_master.datadir, "regtest/wallets", self.default_wallet_name
        )
        node_master_wallet = os.path.join(
            node_master_wallet_dir,
            self.default_wallet_name,
            self.wallet_data_filename,
        )
        v16_3_wallet = os.path.join(
            v16_3_node.datadir, "regtest/wallets/wallet.dat"
        )
        ...
        def copy_v16():
            ...
            # Copy the 0.16.3 wallet to the last Sugarchain Core version and open it:
        ...
        def copy_non_hd():
            node_master.get_wallet_rpc(self.default_wallet_name).unloadwallet()
            # Copy the 0.15.2 non hd wallet to the last Sugarchain Core version and open it:
            shutil.rmtree(node_master_wallet_dir)
            os.mkdir(node_master_wallet_dir)
            shutil.copy(v15_2_wallet, node_master_wallet_dir)
            node_master.loadwallet(self.default_wallet_name)

        def copy_split_hd():
            node_master.get_wallet_rpc(self.default_wallet_name).unloadwallet()
            # Copy the 0.15.2 split hd wallet to the last Sugarchain Core version and open it:
            shutil.rmtree(node_master_wallet_dir)
            os.mkdir(node_master_wallet_dir)
            shutil.copy(
                split_hd_wallet,
        ...
        self.test_upgradewallet(
            wallet, previous_version=159900, expected_version=169900
        )
        ...
        self.test_upgradewallet(
            wallet, previous_version=60000, requested_version=169900
        )
        ...
        self.test_upgradewallet(
            wallet,
            previous_version=60000,
            requested_version=129999,
            expected_version=60000,
        )
        wallet.unloadwallet()
        ...
        copy_non_hd()
        self.test_upgradewallet_error(
            wallet,
            previous_version=60000,
            requested_version=40000,
        ...
        self.test_upgradewallet(
            wallet, previous_version=60000, requested_version=130000
        )
        ...
        hd_chain_version, external_counter, seed_id = struct.unpack(
            "<iI20s", hd_chain
        )
        ...
        for version in [139900, 159900, 169899]:
            self.test_upgradewallet_error(
                ...
                "support pre-split keypool. Please use version 169900 or no version specified.".format(
                    130000, version
                ),
            )
        ...
        self.test_upgradewallet(
            wallet, previous_version=130000, requested_version=169900
        )
        ...
        for k, old_v in old_kvs.items():
            if k.startswith(b"\x07keymeta"):
                (
                    ...
                    new_path,
                    new_has_key_orig,
                ) = deser_keymeta(BytesIO(new_kvs[k]))
                (
                    old_ver,
                    old_create_time,
                    old_kp_str,
                    ...
                    old_path,
                    old_has_key_orig,
                ) = deser_keymeta(BytesIO(old_v))
                assert_equal(10, old_ver)
                if (
                    old_kp_str == b""
                ):  # imported things that don't have keymeta (i.e. imported coinbase privkeys) won't be upgraded
        ...
        self.log.info(
            "Upgrading to NO_DEFAULT_KEY should not remove the defaultkey"
        )
        copy_split_hd()
        ...
        self.test_upgradewallet(
            wallet, previous_version=139900, requested_version=159900
        )
        new_kvs = dump_bdb_kv(node_master_wallet)
        ...
        if self.is_sqlite_compiled():
            self.log.info(
                "Checking that descriptor wallets do nothing, successfully"
            )
            self.nodes[0].createwallet(
                wallet_name="desc_upgrade", descriptors=True
            )
            desc_wallet = self.nodes[0].get_wallet_rpc("desc_upgrade")
            self.test_upgradewallet(
                desc_wallet, previous_version=169900, expected_version=169900
            )

            self.log.info(
                "Checking that descriptor wallets without privkeys do nothing, successfully"
            )
            self.nodes[0].createwallet(
                wallet_name="desc_upgrade_nopriv",
                descriptors=True,
        ...
        if self.is_bdb_compiled():
            ...
            self.nodes[0].createwallet(
                wallet_name="privkeys_disabled_upgrade",
                disable_private_keys=True,
                descriptors=False,
            )
            disabled_wallet = self.nodes[0].get_wallet_rpc(
                "privkeys_disabled_upgrade"
            )
            self.test_upgradewallet(
                disabled_wallet,
                previous_version=169900,
                expected_version=169900,
            )
