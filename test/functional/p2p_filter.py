...
class P2PBloomFilter(P2PInterface):
    ...
    watch_script_pubkey = bytes.fromhex(
        "a914ffffffffffffffffffffffffffffffffffffffff87"
    )
...
class FilterTest(SugarchainTestFramework):
    ...
    def generatetoscriptpubkey(self, scriptpubkey):
        ...
        return self.generatetodescriptor(
            self.nodes[0], 1, f"raw({scriptpubkey.hex()})"
        )[0]
    ...
    def test_size_limits(self, filter_peer):
        ...
        with self.nodes[0].assert_debug_log(["Misbehaving"]):
            filter_peer.send_and_ping(
                msg_filterload(
                    data=b"\xaa", nHashFuncs=MAX_BLOOM_HASH_FUNCS + 1
                )
            )
