...
class TxnMallTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        # All nodes should start with 1,250 SUGAR:
        ...
        self.nodes[0].lockunspent(
            ...
            [
                {
                    "txid": node0_txid1,
                    "vout": find_vout_for_address(
                        self.nodes[0], node0_txid1, node0_address1
                    ),
                }
            ],
        )
        ...
        txid1 = self.spend_txid(
            node0_txid1,
            find_vout_for_address(self.nodes[0], node0_txid1, node0_address1),
            {node1_address: 40},
        )
        txid2 = self.spend_txid(
            node0_txid2,
            find_vout_for_address(self.nodes[0], node0_txid2, node0_address2),
            {node1_address: 20},
        )
        ...
        clone_outputs = {
            rawtx1["vout"][0]["scriptPubKey"]["address"]: rawtx1["vout"][0][
                "value"
            ],
            rawtx1["vout"][1]["scriptPubKey"]["address"]: rawtx1["vout"][1][
                "value"
            ],
        }
        ...
        if (
            ...
            (clone_tx.vout[0], clone_tx.vout[1]) = (
                clone_tx.vout[1],
                clone_tx.vout[0],
            )
        ...
        # Node0's balance should be starting balance, plus 50SUGAR for another
        ...
        # Check node0's total balance; should be same as before the clone, + 100 SUGAR for 2 matured,
