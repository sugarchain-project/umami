...
class MempoolPackagesTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(
            len(self.nodes[0].getrawmempool()), DEFAULT_ANCESTOR_LIMIT + 1
        )
        ...
        replacable_tx = self.wallet.send_self_transfer_multi(
            from_node=self.nodes[0], utxos_to_spend=[chain[0]]
        )["tx"]
        ...
        assert_equal(
            len(self.nodes[0].getrawmempool()), DEFAULT_ANCESTOR_LIMIT + 3
        )
