...
class FullBlockTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        for TxTemplate in invalid_txs.iter_all_templates():
            ...
            self.log.info(
                f"Reject block with invalid tx: {TxTemplate.__name__}"
            )
        ...
        self.send_blocks(
            [b16], success=False, reject_reason="bad-blk-sigops", reconnect=True
        )
        ...
        self.send_blocks(
            [b24], success=False, reject_reason="bad-blk-length", reconnect=True
        )
        ...
        # Extend the b26 chain to make sure sugarchaind isn't accepting b26
        ...
        # Extend the b28 chain to make sure sugarchaind isn't accepting b28
        ...
        b30.vtx[0].vin[0].scriptSig = bytes(
            b30.vtx[0].vin[0].scriptSig
        )  # Convert CScript to raw bytes
        b30.vtx[0].vin[0].scriptSig += b"\x00" * (
            100 - len(b30.vtx[0].vin[0].scriptSig)
        )  # Fill with 0s
        ...
        lots_of_multisigs = CScript(
            [OP_CHECKMULTISIG] * ((MAX_BLOCK_SIGOPS - 1) // 20)
            + [OP_CHECKSIG] * 19
        )
        ...
        too_many_multisigs = CScript(
            [OP_CHECKMULTISIG] * (MAX_BLOCK_SIGOPS // 20)
        )
        b32 = self.next_block(32, spend=out[9], script=too_many_multisigs)
        assert_equal(get_legacy_sigopcount_block(b32), MAX_BLOCK_SIGOPS + 1)
        self.send_blocks(
            [b32], success=False, reject_reason="bad-blk-sigops", reconnect=True
        )
        ...
        self.move_tip(31)
        lots_of_multisigs = CScript(
            [OP_CHECKMULTISIGVERIFY] * ((MAX_BLOCK_SIGOPS - 1) // 20)
            + [OP_CHECKSIG] * 19
        )
        ...
        too_many_multisigs = CScript(
            [OP_CHECKMULTISIGVERIFY] * (MAX_BLOCK_SIGOPS // 20)
        )
        b34 = self.next_block(34, spend=out[10], script=too_many_multisigs)
        self.send_blocks(
            [b34], success=False, reject_reason="bad-blk-sigops", reconnect=True
        )
        ...
        lots_of_checksigs = CScript(
            [OP_CHECKSIGVERIFY] * (MAX_BLOCK_SIGOPS - 1)
        )
        ...
        self.send_blocks(
            [b36], success=False, reject_reason="bad-blk-sigops", reconnect=True
        )
        ...
        for i in range(1, numTxes + 1):
            ...
            (sighash, err) = LegacySignatureHash(
                redeem_script, tx, 1, SIGHASH_ALL
            )
            sig = self.coinbase_key.sign_ecdsa(sighash) + bytes(
        ...
        b40_sigops_to_fill = (
            MAX_BLOCK_SIGOPS - (numTxes * b39_sigops_per_output + sigops) + 1
        )
        tx = CTransaction()
        tx.vin.append(CTxIn(lastOutpoint, b""))
        ...
        self.send_blocks(
            [b40], success=False, reject_reason="bad-blk-sigops", reconnect=True
        )
        ...
        self.blocks[46] = b46
        self.send_blocks(
            [b46], success=False, reject_reason="bad-blk-length", reconnect=True
        )
        ...
        b48.solve()
        self.send_blocks(
            [b48], False, force_send=True, reject_reason="time-too-new"
        )
        ...
        #                                                                        -> b44 (14)\-> b48 () -> b48p ()
        self.log.info(
            "Accept a previously rejected future block at a later time"
        )
        node.setmocktime(int(time.time()) + 2 * 60 * 60)
        self.move_tip(48)
        self.block_heights[b48.sha256] = (
            self.block_heights[b44.sha256] + 1
        )  # b48 is a parent of b44
        ...
        assert_equal(
            self.nodes[0].gettxout(txid=duplicate_tx.hash, n=0)[
                "confirmations"
            ],
            119,
        )
        ...
        tx.vin.append(
            CTxIn(COutPoint(out[18].sha256, 0))
        )  # don't set nSequence
        tx.vout.append(CTxOut(0, CScript([OP_TRUE])))
        ...
        script_length = (
            MAX_BLOCK_WEIGHT - 4 * len(b64a.normal_serialize()) - 276
        ) // 4
        script_output = CScript([b"\x00" * script_length])
        tx.vout.append(CTxOut(0, script_output))
        ...
        # sugarchaind doesn't disconnect us for sending a bloated block, but if we subsequently
        ...
        #                        \-> b66 (20)
        self.log.info(
            "Reject a block with a transaction spending an output created later in the same block"
        )
        self.move_tip(65)
        self.next_block(66)
        ...
        self.block_heights[b71.sha256] = (
            self.block_heights[b69.sha256] + 1
        )  # b71 builds off b69
        self.blocks[71] = b71
        ...
        self.send_blocks(
            [b73], success=False, reject_reason="bad-blk-sigops", reconnect=True
        )
        ...
        self.next_block(74)
        size = (
            MAX_BLOCK_SIGOPS - 1 + MAX_SCRIPT_ELEMENT_SIZE + 42
        )  # total = 20,561
        a = bytearray([OP_CHECKSIG] * size)
        ...
        self.send_blocks(
            [b74], success=False, reject_reason="bad-blk-sigops", reconnect=True
        )
        ...
        #    Low-S and High-S values (whereas the sugarchain code has custom code which does so);
    ...
    def sign_tx(self, tx, spend_tx):
        ...
        if scriptPubKey[0] == OP_TRUE:  # an anyone-can-spend
            ...
            return
        (sighash, err) = LegacySignatureHash(
            spend_tx.vout[0].scriptPubKey, tx, 0, SIGHASH_ALL
        )
        tx.vin[0].scriptSig = CScript(
            [
    ...
    def next_block(
        ...
        *,
        ...
        if spend is None:
            block = create_block(
                base_block_hash, coinbase, block_time, version=version
            )
        else:
