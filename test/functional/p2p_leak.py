...
class LazyPeer(P2PInterface):
    ...
    def on_wtxidrelay(self, message):
        self.got_wtxidrelay = True

    def on_sendaddrv2(self, message):
        self.got_sendaddrv2 = True
...
class P2PLeakTest(SugarchainTestFramework):
    ...
    def run_test(self):
        self.log.info(
            "Check that the node doesn't send unexpected messages before handshake completion"
        )
        ...
        pre_wtxidrelay_peer = self.nodes[0].add_p2p_connection(
            NoVerackIdlePeer(), send_version=False, wait_for_verack=False
        )
        ...
        pre_wtxidrelay_peer.wait_until(
            lambda: pre_wtxidrelay_peer.version_received
        )
        ...
        self.log.info(
            "Connect peer to ensure the net thread runs the disconnect logic at least once"
        )
        ...
        with self.nodes[0].assert_debug_log(
            ["using obsolete version 31799; disconnecting"]
        ):
