...
class VersionBitsWarningTest(SugarchainTestFramework):
    ...
    def send_blocks_with_version(self, peer, numblocks, version):
        ...
        for _ in range(numblocks):
            block = create_block(
                tip, create_coinbase(height + 1), block_time, version=version
            )
            block.solve()
    ...
    def run_test(self):
        ...
        self.send_blocks_with_version(
            peer, VB_THRESHOLD - 1, VB_UNKNOWN_VERSION
        )
        self.generatetoaddress(
            node, VB_PERIOD - VB_THRESHOLD + 1, node_deterministic_address
        )
        ...
        self.generatetoaddress(
            node, VB_PERIOD - VB_THRESHOLD, node_deterministic_address
        )
        ...
        # Stop-start the node. This is required because sugarchaind will only warn once about unknown versions or unknown rules activating.
        self.restart_node(0)
