...
Verify that a sugarchaind node can load multiple wallet files
"""
...
def test_load_unload(node, name):
    ...
    while True:
        ...
        try:
            ...
            if (
                e.error["code"] == -4
                and "Wallet already loading" in e.error["message"]
            ):
                got_loading_error = True
                return


class MultiWalletTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        os.symlink(
            "wallet.dat", wallet_dir("self_walletdat_symlink/wallet.dat")
        )
        ...
        os.rename(
            wallet_dir("created", self.wallet_data_filename),
        ...
        in_wallet_dir = [
            w.replace("/", os.path.sep) for w in to_create
        ]  # Wallets in the wallet dir
        ...
        in_wallet_dir += (
            to_load  # The loaded wallets are also in the wallet dir
        )
        self.start_node(0)
        ...
        try:
            with self.nodes[0].assert_debug_log(
                expected_msgs=["Error scanning"]
            ):
                walletlist = self.nodes[0].listwalletdir()["wallets"]
        finally:
            # Need to ensure access is restored for cleanup
            os.chmod(
                wallet_dir("no_access"),
        ...
        for wallet_name, wallet in zip(wallet_names, wallets):
            ...
            assert_equal(
                info["immature_balance"], 50 if wallet is wallets[0] else 0
            )
        ...
        assert_raises_rpc_error(
            -19, "Wallet file not specified", node.getwalletinfo
        )
        ...
        assert_raises_rpc_error(
            -18,
            "Wallet file verification failed. Failed to load database path '{}'. Path does not exist.".format(
                path
            ),
        ...
        if not self.options.descriptors:
            ...
            path = os.path.join(
                self.options.tmpdir, "node0", "regtest", "wallets", "wallet.dat"
            )
            assert_raises_rpc_error(
                -35,
        ...
        assert_raises_rpc_error(
            -18,
            "Wallet file verification failed. Failed to load database path '{}'. Data is not in recognized format.".format(
                path
            ),
            self.nodes[0].loadwallet,
            "empty_wallet_dir",
        ...
        path = os.path.join(
            self.options.tmpdir, "node0", "regtest", "wallets", "w2"
        )
        assert_raises_rpc_error(
            -4,
