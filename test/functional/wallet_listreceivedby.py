...
class ReceivedByTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        assert_array_result(
            self.nodes[1].listreceivedbyaddress(), {"address": addr}, {}, True
        )
        ...
        for node in self.nodes:
            for addr_obj in node.listreceivedbyaddress():
                assert_equal(
                    node.getaddressinfo(addr_obj["address"])["ischange"], False
                )
        ...
        assert_equal(
            len(res), 2 + num_cb_reward_addresses
        )  # Right now 2 entries
        ...
        # Not on random addr
        other_addr = self.nodes[
            0
        ].getnewaddress()  # note on node[0]! just a random addr
        res = self.nodes[1].listreceivedbyaddress(0, True, True, other_addr)
        ...
        assert_raises_rpc_error(
            ...
            self.nodes[0].getreceivedbyaddress,
            addr,
        )
        ...
        received_by_label_json = [
            r
            for r in self.nodes[1].listreceivedbylabel()
            if r["label"] == label
        ][0]
        ...
        assert_raises_rpc_error(
            -4,
            "Label not found in wallet",
            self.nodes[0].getreceivedbylabel,
            "dummy",
        )
        ...
        assert_array_result(
            self.nodes[0].listreceivedbylabel(), {"label": label}, {}, True
        )
