...
class SignMessagesWithPrivTest(SugarchainTestFramework):
    ...
    def addresses_from_privkey(self, priv_key):
        ...
        return [
            self.nodes[0].deriveaddresses(descsum_create(desc))[0]
    ...
    def run_test(self):
        ...
        for num_params in [0, 1, 2, 4, 5]:
            param_list = ["dummy"] * num_params
            assert_raises_rpc_error(
                -1, "verifymessage", self.nodes[0].verifymessage, *param_list
            )
