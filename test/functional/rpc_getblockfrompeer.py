...
class GetBlockFromPeerTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        x = next(
            filter(
                lambda x: x["hash"] == short_tip, self.nodes[0].getchaintips()
            )
        )
        ...
        self.nodes[0].add_p2p_connection(
            P2PInterface(), services=P2P_SERVICES & ~NODE_WITNESS
        )
        peers = self.nodes[0].getpeerinfo()
        ...
        self.wait_until(
            lambda: self.check_for_block(node=0, hash=short_tip), timeout=1
        )
        assert_equal(result, {})
        ...
        self.log.info(
            "Don't fetch blocks while the node has not synced past it yet"
        )
        ...
        result = pruned_node.getblockfrompeer(
            pruned_block, pruned_node_peer_0_id
        )
        self.wait_until(
            lambda: self.check_for_block(node=2, hash=pruned_block), timeout=1
        )
        assert_equal(result, {})
        ...
        assert_equal(
            pruned_node.getblock(pruned_block)["hash"],
            "36c56c5b5ebbaf90d76b0d1a074dcb32d42abab75b7ec6fa0ffd9b4fbce8f0f7",
        )

        self.log.info(
            "Fetched block can be pruned again when prune height exceeds the height of the tip at the time when the block was fetched"
        )
        self.generate(self.nodes[0], 250, sync_fun=self.no_op)
        self.sync_blocks([self.nodes[0], pruned_node])
        pruneheight += 250
