...
class ImportDescriptorsTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].createwallet(
            wallet_name="w0", disable_private_keys=False, descriptors=True
        )
        ...
        self.generatetoaddress(
            self.nodes[0], COINBASE_MATURITY + 1, w0.getnewaddress()
        )
        ...
        self.test_importdesc(
            {**import_request, "label": "Updated label"}, success=True
        )
        test_address(
            w1,
            key.p2pkh_addr,
        ...
        self.test_importdesc(
            {"desc": "sh(wpkh(" + key.pubkey + "))", "timestamp": "now"},
            success=False,
            error_code=-5,
        ...
        self.test_importdesc(
            {
                "desc": descsum_create(
                    "multi(1," + key1.pubkey + "," + key2.pubkey + ")"
                ),
                "timestamp": "now",
            },
            success=True,
        ...
        self.test_importdesc(
            {"desc": descsum_create(desc), "timestamp": "now"},
            success=True,
        ...
        self.test_importdesc(
            {"desc": descsum_create(desc), "timestamp": "now", "range": 1},
            success=False,
            error_code=-4,
        ...
        self.test_importdesc(
            {"desc": descsum_create(desc), "timestamp": "now", "range": [2, 1]},
            success=False,
            error_code=-8,
        ...
        self.test_importdesc(
            {**range_request, "range": [0, 10]}, wallet=wpriv, success=True
        )
        assert_equal(wpriv.getwalletinfo()["keypoolsize"], 11)
        self.test_importdesc(
            {**range_request, "range": [0, 20]}, wallet=wpriv, success=True
        )
        ...
        self.test_importdesc(
            {**range_request, "range": [5, 10]},
            wallet=wpriv,
            success=False,
            error_code=-8,
            error_message="new range must include current range = [0,20]",
        )
        self.test_importdesc(
            {**range_request, "range": [0, 10]},
            wallet=wpriv,
            success=False,
            error_code=-8,
            error_message="new range must include current range = [0,20]",
        ...
        self.test_importdesc(
            {**range_request, "range": [0, 20], "internal": True},
            wallet=wpriv,
            success=True,
        )
        assert_equal(wpriv.getwalletinfo()["keypoolsize"], 0)
        ...
        self.test_importdesc(
            {**range_request, "range": [0, 20], "internal": False},
            wallet=wpriv,
            success=True,
        )
        assert_equal(wpriv.getwalletinfo()["keypoolsize"], 21)
        ...
        for i, expected_addr in enumerate(addresses):
            ...
            assert_equal(
                pkh_addr_info["desc"][:22], "pkh([12345678/0'/0'/{}]".format(i)
            )
        ...
        self.test_importdesc(
            {
                "desc": descsum_create("pkh([12345678/1h]" + xpub + "/*)"),
                "range": [0, 2],
                "timestamp": "now",
                "internal": True,
            },
        ...
        self.test_importdesc(
            {
                "desc": descsum_create("pkh([12345678]" + xpub + "/*)"),
                "range": [0, 5],
                "active": True,
                "timestamp": "now",
                "internal": True,
        ...
        tx = wpriv.createrawtransaction(
            [{"txid": txid, "vout": 0}], {w0.getnewaddress(): 49.999}
        )
        ...
        assert_equal(
            wmulti_priv.getwalletinfo()["keypoolsize"], 1001
        ...
        )  # Derived at m/84'/1'/0'/0
        assert_equal(wmulti_priv.getwalletinfo()["keypoolsize"], 1000)
        txid = w0.sendtoaddress(addr, 10)
        self.generate(self.nodes[0], 6)
        send_txid = wmulti_priv.sendtoaddress(
            w0.getnewaddress(), 8
        )  # uses change 1
        decoded = wmulti_priv.gettransaction(txid=send_txid, verbose=True)[
            "decoded"
        ]
        ...
        self.test_importdesc(
            {
                "desc": "wsh(multi(2,[7b2d0242/84h/0h/0h]"
                + acc_xpub1
                + "/*,[59b09cd6/84h/0h/0h]"
                + acc_xpub2
                + "/*,[e81a0532/84h/0h/0h]"
        ...
        self.test_importdesc(
            {
                "desc": "wsh(multi(2,[7b2d0242/84h/1h/0h]"
                + chg_xpub1
                + "/*,[59b09cd6/84h/1h/0h]"
                + chg_xpub2
                + "/*,[e81a0532/84h/1h/0h]"
        ...
        assert_equal(
            change_addr,
            "bcrt1qp6j3jw8yetefte7kw6v5pc89rkgakzy98p6gf7ayslaveaxqyjusnw580c",
        )  # Derived at m/84'/1'/0'/2
        ...
        self.nodes[1].createwallet(
            wallet_name="wmulti_priv2", blank=True, descriptors=True
        )
        ...
        res = wmulti_priv2.importdescriptors(
            [
                {
                    "desc": descsum_create(
                        "wsh(multi(2,[7b2d0242/84h/0h/0h]"
                        + acc_xpub1
                        + "/*,"
                        + xprv2
                        + "/84h/0h/0h/*,[e81a0532/84h/0h/0h]"
                        + acc_xpub3
                        + "/*))"
                    ),
                    "active": True,
                ...
                {
                    "desc": descsum_create(
                        "wsh(multi(2,[7b2d0242/84h/1h/0h]"
                        + chg_xpub1
                        + "/*,"
                        + xprv2
                        + "/84h/1h/0h/*,[e81a0532/84h/1h/0h]"
                        + chg_xpub3
                        + "/*))"
                    ),
                    "active": True,
        ...
        self.nodes[1].createwallet(
            wallet_name="wmulti_priv_big", blank=True, descriptors=True
        )
        ...
        res = wmulti_priv_big.importdescriptors(
            [
                {
                    "desc": descsum_create(
                        f"wsh(sortedmulti(20,{(xkey + ',') * 19}{xkey}))"
                    ),
                    "active": True,
                    "range": 1000,
                    "next_index": 0,
        ...
        decoded = wmulti_priv_big.gettransaction(txid=txid, verbose=True)[
            "decoded"
        ]
        ...
        self.log.info(
            "Under P2SH, multisig are standard with up to 15 " "compressed keys"
        )
        self.nodes[1].createwallet(
            wallet_name="multi_priv_big_legacy", blank=True, descriptors=True
        )
        ...
        res = multi_priv_big.importdescriptors(
            [
                ...
                {
                    "desc": descsum_create(
                        f"sh(multi(15,{(xkey_int + ',') * 14}{xkey_int}))"
                    ),
                    "active": True,
                    "internal": True,
                    "range": 1000,
        ...
        self.generate(self.nodes[0], 6)
        # It is standard and would relay.
        txid = multi_priv_big.sendtoaddress(
            w0.getnewaddress(), 10, "", "", True
        )
        decoded = multi_priv_big.gettransaction(txid=txid, verbose=True)[
            "decoded"
        ]
        ...
        rawtx = self.nodes[1].createrawtransaction(
            [{"txid": txid2, "vout": vout2}], {w0.getnewaddress(): 9.999}
        )
        ...
        descriptor = {
            "desc": descsum_create("pkh(" + xpriv + "/1h/*h)"),
            "timestamp": "now",
            "active": True,
            "range": [0, 4000],
            "next_index": 4000,
        ...
        self.generatetoaddress(
            self.nodes[0], COINBASE_MATURITY + 1, temp_wallet.getnewaddress()
        )
        self.generatetoaddress(
            self.nodes[0], COINBASE_MATURITY + 1, temp_wallet.getnewaddress()
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as thread:
            with self.nodes[0].assert_debug_log(
                expected_msgs=[
                    "Rescan started from block 0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206... (slow variant inspecting all blocks)"
                ],
                timeout=5,
            ):
                importing = thread.submit(
                    encrypted_wallet.importdescriptors, requests=[descriptor]
                )

            # Set the passphrase timeout to 1 to test that the wallet remains unlocked during the rescan
            self.nodes[0].cli("-rpcwallet=encrypted_wallet").walletpassphrase(
                "passphrase", 1
            )
            ...
            try:
                ...
                assert (
                    e.error["code"] == -4
                    and "Error: the wallet is currently being used to rescan the blockchain for related transactions. Please call `abortrescan` before locking the wallet."
                    in e.error["message"]
                )

            try:
                self.nodes[0].cli(
                    "-rpcwallet=encrypted_wallet"
                ).walletpassphrasechange("passphrase", "newpassphrase")
            except JSONRPCException as e:
                assert (
                    e.error["code"] == -4
                    and "Error: the wallet is currently being used to rescan the blockchain for related transactions. Please call `abortrescan` before changing the passphrase."
                    in e.error["message"]
                )
