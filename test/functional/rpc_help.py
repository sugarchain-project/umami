...
class HelpRpcTest(SugarchainTestFramework):
    ...
    def test_client_conversion_table(self):
        ...
        mapping_server_conversion = [
            tuple(m[:3]) for m in mapping_server if not m[3]
        ]
        ...
        if self.is_wallet_compiled() and sorted(mapping_client) != sorted(
            ...
            raise AssertionError(
                "RPC client conversion table ({}) and RPC server named arguments mismatch!\n{}".format(
                    file_conversion_table,
                    set(mapping_client).symmetric_difference(
                        mapping_server_conversion
                    ),
                )
            )
