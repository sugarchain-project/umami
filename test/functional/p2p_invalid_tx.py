...
class InvalidTxRequestTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        for BadTxTemplate in invalid_txs.iter_all_templates():
            self.log.info(
                "Testing invalid transaction: %s", BadTxTemplate.__name__
            )
        ...
        tx_withhold.vout = [
            CTxOut(
                nValue=25 * COIN - 12000, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE
            )
        ] * 2
        ...
        self.wait_until(
            lambda: 1 == len(node.getpeerinfo()), timeout=12
        ...
        tx_orphan_include_by_block_A.vout.append(
            CTxOut(
                nValue=12 * COIN - 12000, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE
            )
        )
