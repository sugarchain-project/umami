...
class MempoolSpendCoinbaseTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].invalidateblock(
            self.nodes[0].getblockhash(chain_height + 1)
        )
        ...
        coinbase_txid = lambda h: self.nodes[0].getblock(
            self.nodes[0].getblockhash(h)
        )["tx"][0]
        utxo_mature = wallet.get_utxo(
            txid=coinbase_txid(chain_height - 100 + 1)
        )
        utxo_immature = wallet.get_utxo(
            txid=coinbase_txid(chain_height - 100 + 2)
        )

        spend_mature_id = wallet.send_self_transfer(
            from_node=self.nodes[0], utxo_to_spend=utxo_mature
        )["txid"]
