...
class TransactionTimeRescanTest(SugarchainTestFramework):
    ...
    def set_test_params(self):
        ...
        self.num_nodes = 3
        self.extra_args = [["-keypool=400"], ["-keypool=400"], []]
    ...
    def run_test(self):
        ...
        minernode = self.nodes[
            0
        ]  # node used to mine SUGAR and create transactions
        usernode = self.nodes[1]  # user node with correct time
        restorenode = self.nodes[
            2
        ]  # node used to restore user wallet and check time determination in ComputeSmartTime (wallet.cpp)
        ...
        # generate some sugar to create transactions and check blockcount
        ...
        # send 10 sugar to user's first watch-only address
        self.log.info("Send 10 sugar to user")
        ...
        # send 5 sugar to our second watch-only address
        self.log.info("Send 5 sugar to user")
        ...
        # send 1 sugar to our third watch-only address
        self.log.info("Send 1 sugar to user")
        ...
        for tx in wo_wallet.listtransactions():
            if tx["address"] == wo1:
                ...
                assert_equal(
                    tx["blocktime"], cur_time + ten_days + ten_days + ten_days
                )
                assert_equal(
                    tx["time"], cur_time + ten_days + ten_days + ten_days
                )
        ...
        if self.options.descriptors:
            set_node_times(
                self.nodes, cur_time + ten_days + ten_days + ten_days + ten_days
            )
        ...
        self.log.info(
            "Check user's final balance and transaction count after restoration"
        )
        ...
        self.log.info(
            "Test handling of invalid parameters for rescanblockchain"
        )
        assert_raises_rpc_error(
            -8,
        ...
        assert_raises_rpc_error(
            -8, "Invalid stop_height", restorewo_wallet.rescanblockchain, 1, -1
        )
        assert_raises_rpc_error(
            -8,
        ...
        self.log.info(
            "Test `rescanblockchain` fails when wallet is encrypted and locked"
        )
        ...
        if not self.options.descriptors:
            ...
            usernode.createwallet(
                wallet_name="temp_wallet", blank=True, descriptors=False
            )
            ...
            self.generatetoaddress(
                usernode, COINBASE_MATURITY + 1, temp_wallet.getnewaddress()
            )
            self.generatetoaddress(
                usernode, COINBASE_MATURITY + 1, temp_wallet.getnewaddress()
            )

            minernode.createwallet(
                "encrypted_wallet",
                blank=True,
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as thread:
                with minernode.assert_debug_log(
                    expected_msgs=[
                    ...
                    rescanning = thread.submit(
                        encrypted_wallet.rescanblockchain
                    )

                # set the passphrase timeout to 1 to test that the wallet remains unlocked during the rescan
                minernode.cli("-rpcwallet=encrypted_wallet").walletpassphrase(
                    "passphrase", 1
                )
                ...
                assert_equal(
                    rescanning.result(), {"start_height": 0, "stop_height": 803}
                )

            assert_equal(
                encrypted_wallet.getbalance(), temp_wallet.getbalance()
            )
