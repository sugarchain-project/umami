...
class TestP2PConn(P2PInterface):
    ...
    def on_getdata(self, message):
        for i in message.inv:
            if (
                i.type & MSG_TYPE_MASK == MSG_TX
                or i.type & MSG_TYPE_MASK == MSG_WTX
            ):
...
class TxDownloadTest(SugarchainTestFramework):
    ...
    def test_in_flight_max(self):
        self.log.info(
            "Test that we don't load peers with more than {} transaction requests immediately".format(
        ...
        self.log.info(
            "No more than {} requests should be seen within {} seconds after announcement".format(
                MAX_GETDATA_IN_FLIGHT,
                INBOUND_PEER_TX_DELAY + OVERLOADED_PEER_DELAY - 1,
            )
        )
        self.nodes[0].setmocktime(
            mock_time + INBOUND_PEER_TX_DELAY + OVERLOADED_PEER_DELAY - 1
        )
        p.sync_with_ping()
        with p2p_lock:
        ...
        self.log.info(
            "If we wait {} seconds after announcement, we should eventually get more requests".format(
                INBOUND_PEER_TX_DELAY + OVERLOADED_PEER_DELAY
            )
        )
        self.nodes[0].setmocktime(
            mock_time + INBOUND_PEER_TX_DELAY + OVERLOADED_PEER_DELAY
        )
    ...
    def test_expiry_fallback(self):
        ...
        peer2.wait_until(
            lambda: sum(p.tx_getdata_count for p in [peer1, peer2]) == 1
        )
        with p2p_lock:
            peer_expiry, peer_fallback = (
                (peer1, peer2)
                if peer1.tx_getdata_count == 1
                else (peer2, peer1)
            )
            assert_equal(peer_fallback.tx_getdata_count, 0)
        self.nodes[0].setmocktime(
            int(time.time()) + GETDATA_TX_INTERVAL + 1
    ...
    def test_notfound_fallback(self):
        ...
        peer_notfound.send_and_ping(
            msg_notfound(vec=[CInv(MSG_WTX, WTXID)])
    ...
    def test_preferred_inv(self, preferred=False):
        if preferred:
            self.log.info(
                "Check invs from preferred peers are downloaded immediately"
            )
            self.restart_node(0, extra_args=["-whitelist=noban@127.0.0.1"])
        else:
            self.log.info(
                "Check invs from non-preferred peers are downloaded after {} s".format(
    ...
    def test_txid_inv_delay(self, glob_wtxid=False):
        self.log.info(
            "Check that inv from a txid-relay peers are delayed by {} s, with a wtxid peer {}".format(
                TXID_RELAY_DELAY, glob_wtxid
            )
        )
        self.restart_node(0, extra_args=["-whitelist=noban@127.0.0.1"])
        mock_time = int(time.time() + 1)
    ...
    def test_large_inv_batch(self):
        ...
        peer.send_message(
            msg_inv(
                [
                    CInv(t=MSG_WTX, h=wtxid)
                    for wtxid in range(MAX_PEER_TX_ANNOUNCEMENTS + 1)
                ]
            )
        )
        peer.wait_until(
            lambda: peer.tx_getdata_count == MAX_PEER_TX_ANNOUNCEMENTS + 1
        )
        ...
        peer.wait_until(
            lambda: peer.tx_getdata_count == MAX_PEER_TX_ANNOUNCEMENTS
        )
        peer.sync_with_ping()
    ...
    def run_test(self):
        ...
        # Run each test against new sugarchaind instances, as setting mocktimes has long-term effects on when
        ...
        for test in [
            ...
            self.log.info(
                "Nodes are setup with {} incoming connections each".format(
                    NUM_INBOUND
                )
            )
            test()
