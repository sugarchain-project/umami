...
class AvoidReuseTest(SugarchainTestFramework):
    ...
    def test_persistence(self):
        ...
        self.nodes[1].createwallet(
            wallet_name="avoid_reuse_persist", avoid_reuse=True
        )
    ...
    def test_immutable(self):
        ...
        self.nodes[1].createwallet(
            wallet_name=tempwallet, disable_private_keys=True
        )
    ...
    def test_sending_from_reused_address_without_avoid_reuse(self):
        """
        Test the same as test_sending_from_reused_address_fails, except send the 10 SUGAR with
        the avoid_reuse flag set to false. This means the 10 SUGAR send should succeed,
        ...
        # listunspent should show 1 single, unused 10 sugar output
        assert_unspent(
            self.nodes[1],
            total_count=1,
            total_sum=10,
        ...
        # getbalances should show no used, 10 sugar trusted
        ...
        # listunspent should show 1 single, unused 5 sugar output
        assert_unspent(
            self.nodes[1],
            total_count=1,
            total_sum=5,
        ...
        # getbalances should show no used, 5 sugar trusted
        assert_balances(self.nodes[1], mine={"used": 0, "trusted": 5})

        self.nodes[0].sendtoaddress(fundaddr, 10)
        self.generate(self.nodes[0], 1)

        # listunspent should show 2 total outputs (5, 10 sugar), one unused (5), one reused (10)
        assert_unspent(
            self.nodes[1],
            total_count=2,
            total_sum=15,
        ...
        # getbalances should show 10 used, 5 sugar trusted
        ...
        # listunspent should show 1 total outputs (5 sugar), unused
        assert_unspent(
            self.nodes[1], total_count=1, total_sum=5, reused_count=0
        )
        # getbalances should show no used, 5 sugar trusted
        assert_balances(self.nodes[1], mine={"used": 0, "trusted": 5})

        # node 1 should now have about 5 sugar left (for both cases)
    ...
    def test_sending_from_reused_address_fails(self, second_addr_type):
        ...
        [0] sends 10 SUGAR to A.
        [1] spends 5 SUGAR from A. (leaving roughly 5 SUGAR useable)
        [0] sends 10 SUGAR to A again.
        [1] tries to spend 10 SUGAR (fails; dirty).
        [1] tries to spend 4 SUGAR (succeeds; change address sufficient)
        """
        self.log.info(
        ...
        if not self.options.descriptors:
            ...
            assert_approx(self.nodes[1].getbalance(), 5, 0.001)
            assert_approx(
                self.nodes[1].getbalance(avoid_reuse=False), 15, 0.001
            )
            ...
            # listunspent should show 2 total outputs (1, 10 sugar), one unused (1), one reused (10)
            assert_unspent(
                self.nodes[1],
                total_count=2,
                total_sum=11,
            ...
            # getbalances should show 10 used, 1 sugar trusted
            assert_balances(self.nodes[1], mine={"used": 10, "trusted": 1})

            # node 1 should now have about 1 sugar left (no dirty) and 11 (including dirty)
            assert_approx(self.nodes[1].getbalance(), 1, 0.001)
            assert_approx(
                self.nodes[1].getbalance(avoid_reuse=False), 11, 0.001
            )
    ...
    def test_getbalances_used(self):
        ...
        assert_balances(
            self.nodes[1], mine={"used": 1, "trusted": 95}, margin=0.01
        )

    def test_full_destination_group_is_preferred(self):
        """
        Test the case where [1] only has 101 outputs of 1 SUGAR in the same reused
        address and tries to send a small payment of 0.5 SUGAR. The wallet
        should use 100 outputs from the reused address as inputs and not a
        single 1 SUGAR input, in order to join several outputs from the reused
        address.
        """
        self.log.info(
        ...
        # Send 101 outputs of 1 SUGAR to the same, reused address in the wallet
        for _ in range(101):
            self.nodes[0].sendtoaddress(new_addr, 1)
    ...
    def test_all_destination_groups_are_used(self):
        """
        Test the case where [1] only has 202 outputs of 1 SUGAR in the same reused
        address and tries to send a payment of 200.5 SUGAR. The wallet
        ...
        # Send 202 outputs of 1 SUGAR to the same, reused address in the wallet
        for _ in range(202):
            self.nodes[0].sendtoaddress(new_addr, 1)
