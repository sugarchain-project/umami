...
def displayaddress(args):
    ...
    if args.desc not in expected_desc:
        return sys.stdout.write(
            json.dumps({"error": "Unexpected descriptor", "desc": args.desc})
        )

    return sys.stdout.write(
        json.dumps({"address": "bcrt1qm90ugl4d48jv8n6e5t9ln6t9zlpm5th68x4f8g"})
    )
...
def signtx(args):
    ...
    with open(
        os.path.join(os.getcwd(), "mock_psbt"), "r", encoding="utf8"
    ) as f:
        mock_psbt = f.read()
...
parser = argparse.ArgumentParser(
    prog="./signer.py", description="External signer mock"
)
...
parser_displayaddress = subparsers.add_parser(
    "displayaddress", help="display address on signer"
)
