...
class CreateWalletTest(SugarchainTestFramework):
    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            result = w1.importdescriptors(
                [
                    {
                        "desc": descsum_create("wpkh(" + privkey + ")"),
                        "timestamp": "now",
                    }
                ]
            )
        ...
        assert_equal(
            ...
            EMPTY_PASSPHRASE_MSG
            if self.options.descriptors
            else f"{EMPTY_PASSPHRASE_MSG}\n{LEGACY_WALLET_MSG}",
        )
        assert_equal(
            resp["warnings"],
            [EMPTY_PASSPHRASE_MSG]
            if self.options.descriptors
            else [EMPTY_PASSPHRASE_MSG, LEGACY_WALLET_MSG],
        )
        ...
        if self.is_bdb_compiled():
            ...
            result = self.nodes[0].createwallet(
                wallet_name="legacy_w0", descriptors=False, passphrase=None
            )
            assert_equal(
                result,
                {
            ...
            result = self.nodes[0].createwallet(
                wallet_name="legacy_w1", descriptors=False, passphrase=""
            )
            assert_equal(
                result,
                {
        ...
        self.log.info(
            'Test "warning" field deprecation, i.e. not returned without -deprecatedrpc=walletwarningfield'
        )
        self.restart_node(0, extra_args=[])
        result = self.nodes[0].createwallet(
            wallet_name="w7_again",
            disable_private_keys=False,
            blank=False,
            passphrase="",
        )
