...
class DecodeScriptTest(SugarchainTestFramework):
    ...
    def decodescript_script_pub_key(self):
        ...
        p2wsh_p2pk_script_hash = (
            "d8590cf8ea0674cf3d49fd7ca249b85ef7485dea62c138468bddeb20cd6519f7"
        )
        ...
        # <pubkey> OP_CHECKSIG
        rpc_result = self.nodes[0].decodescript(
            push_uncompressed_public_key + "ac"
        )
        assert_equal("pubkey", rpc_result["type"])
        assert_equal(
            uncompressed_public_key + " OP_CHECKSIG", rpc_result["asm"]
        )
        ...
        rpc_result = self.nodes[0].decodescript(
            "52" + push_public_key + push_uncompressed_public_key + "52ae"
        )
        assert_equal("multisig", rpc_result["type"])
        assert_equal(
            "2 "
    ...
    def decoderawtransaction_asm_sighashtype(self):
        ...
        assert_equal(
            signature_sighash_decoded, rpc_result["vin"][0]["scriptSig"]["asm"]
        )
        ...
        txSave.vin[0].scriptSig = bytes.fromhex(
    ...
    def decodescript_miniscript(self):
        ...
        # Sourced from https://github.com/sugarchain/sugarchain/pull/27037#issuecomment-1416151907.
        # Miniscript-compatible offered HTLC
        res = self.nodes[0].decodescript(
        ...
        assert (
            res["segwit"]["desc"]
            == "wsh(and_v(and_v(v:hash160(ffffffffffffffffffffffffffffffffffffffff),v:pk(0250929b74c1a04954b78b4b6035e97a5e078a5a0f28ec96d547bfee9ace803ac0)),older(1)))#gm8xz4fl"
        )
        # Miniscript-incompatible offered HTLC
        res = self.nodes[0].decodescript(
            "82012088a914ffffffffffffffffffffffffffffffffffffffff882102ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffacb2"
        )
        assert (
            res["segwit"]["desc"]
            == "wsh(raw(82012088a914ffffffffffffffffffffffffffffffffffffffff882102ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffacb2))#ra6w2xa7"
        )
