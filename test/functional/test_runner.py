...
BASE_SCRIPTS = [
    ...
    "interface_sugarchain_cli.py --legacy-wallet",
    "interface_sugarchain_cli.py --descriptors",
...
def main():
    ...
    parser.add_argument(
        "--filter", help="filter scripts to run by regular expression"
    )
    ...
    enable_sugarchaind = config["components"].getboolean("ENABLE_BITCOIND")
    ...
    check_script_list(
        src_dir=config["environment"]["SRCDIR"], fail_on_warn=args.ci
    )
    check_script_prefixes()

    if not args.keepcache:
        shutil.rmtree(
            "%s/test/cache" % config["environment"]["BUILDDIR"],
            ignore_errors=True,
        )
    ...
    *,
    ...
    while i < test_count:
        ...
        for (
            ...
            stderr,
            skip_reason,
        ) in job_queue.get_next():
            test_results.append(test_result)
            i += 1
            done_str = "{}/{} - {}{}{}".format(
                i, test_count, BOLD[1], test_result.name, BOLD[0]
            )
            if test_result.status == "Passed":
                logging.debug(
                    "%s passed, Duration: %s s" % (done_str, test_result.time)
                )
                ...
                if combined_logs_len and os.path.isdir(testdir):
                    ...
                    combined_logs, _ = subprocess.Popen(
                        combined_logs_args, text=True, stdout=subprocess.PIPE
                    ).communicate()
                    print(
                        "\n".join(
            ...
            for job in self.jobs:
                ...
                if proc.poll() is not None:
                    ...
                    ret.append(
                        (
                            TestResult(
                                name, status, int(time.time() - start_time)
                            ),
                            testdir,
                            stdout,
                            stderr,
...
def check_script_prefixes():
    ...
    bad_script_names = [
        ...
        if good_prefixes_re.match(script) is None
    ]
...
def check_script_list(*, src_dir, fail_on_warn):
    ...
    missed_tests = list(
        python_files
        - set(map(lambda x: x.split()[0], ALL_SCRIPTS + NON_SCRIPTS))
    )
...
class RPCCoverage:
    ...
    def _get_uncovered_rpc_commands(self):
        ...
        with open(
            coverage_ref_filename, "r", encoding="utf8"
        ) as coverage_ref_file:
            all_cmds.update(
                [line.strip() for line in coverage_ref_file.readlines()]
            )
