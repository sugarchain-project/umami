...
"""Test sugarchaind aborts if a disallowed syscall is used when compiled with the syscall sandbox."""

from test_framework.test_framework import SugarchainTestFramework, SkipTest


class SyscallSandboxTest(SugarchainTestFramework):
    ...
    def skip_test_if_missing_module(self):
        if not self.is_syscall_sandbox_compiled():
            raise SkipTest(
                "sugarchaind has not been built with syscall sandbox enabled."
            )
        if self.disable_syscall_sandbox:
            raise SkipTest("--nosandbox passed to test runner.")

    def run_test(self):
        disallowed_syscall_terminated_sugarchaind = False
        ...
        with self.nodes[0].assert_debug_log([expected_log_entry]):
            ...
            try:
                ...
                disallowed_syscall_terminated_sugarchaind = True
        assert disallowed_syscall_terminated_sugarchaind
        self.nodes = []
