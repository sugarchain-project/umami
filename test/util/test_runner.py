...
# Copyright 2016-2017 The Sugarchain Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Test framework for sugarchain utils.
...
def bctest(testDir, testObj, buildenv):
    ...
    if "output_cmp" in testObj:
        ...
        if not outputType:
            logging.error(
                "Output file %s does not have a file extension" % outputFn
            )
            raise Exception
    ...
    if outputData:
        ...
        try:
            ...
            logging.error(
                "Error parsing command output as %s: %s" % (outputType, e)
            )
            raise
        try:
    ...
    if "error_txt" in testObj:
        ...
        # That stderr is empty if no errors are expected. However, sugarchain-tx
