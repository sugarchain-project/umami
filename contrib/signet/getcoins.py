...
class PPMImage:
    ...
    def __init__(self, f):
        ...
        self._grid = [
            [
                tuple(data[stride * y + 3 * x : stride * y + 3 * (x + 1)])
...
parser = argparse.ArgumentParser(
    ...
    epilog="You may need to start with double-dash (--) when providing sugarchain-cli arguments.",
)
parser.add_argument(
    "-c",
    "--cmd",
    dest="cmd",
    default="sugarchain-cli",
    help="sugarchain-cli command to use",
)
parser.add_argument(
    "-f",
    "--faucet",
...
parser.add_argument(
    ...
    "--captcha",
    dest="captcha",
    default=DEFAULT_GLOBAL_CAPTCHA,
    help="URL of the faucet captcha, or empty if no captcha is needed",
)
parser.add_argument(
    "-a",
    "--addr",
    dest="addr",
    default="",
    help="Sugarchain address to which the faucet should send",
)
parser.add_argument(
    "-p",
    "--password",
...
parser.add_argument(
    "sugarchain_cli_args",
    nargs="*",
    help="Arguments to pass on to sugarchain-cli (default: -signet)",
)

args = parser.parse_args()

if args.sugarchain_cli_args == []:
    args.sugarchain_cli_args = ["-signet"]


def sugarchain_cli(rpc_command_and_params):
    argv = [args.cmd] + args.sugarchain_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = sugarchain_cli(["getblockhash", "1"])
    if curr_signet_hash != GLOBAL_FIRST_BLOCK_HASH:
        raise SystemExit(
            "The global faucet cannot be used with a custom Signet network. Please use the global signet or setup your custom faucet to use this functionality.\n"
        )
else:
...
if args.addr == "":
    ...
    args.addr = sugarchain_cli(["getnewaddress", "faucet", "bech32"])
...
if args.captcha != "":  # Retrieve a captcha
    ...
    try:
        ...
        raise SystemExit(
            f"The binary {args.imagemagick} could not be found. Please make sure ImageMagick (or a compatible fork) is installed and that the correct path is specified."
        )
