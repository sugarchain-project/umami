...
def main():
    ...
    expect_code(
        run_verify("", "pub", "0.32"), 4, "Nonexistent version should fail"
    )
    expect_code(
        run_verify("", "pub", "0.32.awefa.12f9h"),
        11,
        "Malformed version should fail",
    )
    expect_code(
        run_verify("--min-good-sigs 20", "pub", "22.0"),
        9,
...
def expect_code(
    completed: subprocess.CompletedProcess, expected_code: int, msg: str
):
