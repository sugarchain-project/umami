# Bootstrappable Sugarchain Core Builds

This directory contains the files necessary to perform bootstrappable Sugarchain
Core builds.
...
To build Sugarchain Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Sugarchain Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/sugarchain-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/sugarchain-core/guix.sigs).
