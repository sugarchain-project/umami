...
def tree_sha512sum(commit="HEAD"):
    ...
    This is copied from github-merge.py. See https://github.com/sugarchain-core/sugarchain-maintainer-tools.
    """
    ...
    for line in subprocess.check_output(
        [GIT, "ls-tree", "--full-tree", "-r", commit]
    ).splitlines():
...
def main():
    ...
    with open(
        dirname + "/trusted-sha512-root-commit", "r", encoding="utf8"
    ) as f:
    ...
    with open(
        dirname + "/allow-unclean-merge-commits", "r", encoding="utf8"
    ) as f:
        unclean_merge_allowed = f.read().splitlines()
    with open(
        dirname + "/allow-incorrect-sha512-commits", "r", encoding="utf8"
    ) as f:
    ...
    while True:
        ...
        logging.debug(
            "verify-commits: [in-progress] processing commit {}".format(
                current_commit[:8]
            )
        )
        ...
        if verify_tree:
            if current_commit == verified_sha512_root:
                print(
                    "All Tree-SHA512s matched up to {}".format(
                        verified_sha512_root
                    ),
                    file=sys.stderr,
                )
                verify_tree = False
        ...
        os.environ["BITCOIN_VERIFY_COMMITS_ALLOW_SHA1"] = (
            "0" if no_sha1 else "1"
        )
        ...
        verify_res = subprocess.run(
            [
                GIT,
                "-c",
                "gpg.program={}/gpg.sh".format(dirname),
                "verify-commit",
                "--raw",
                current_commit,
            ],
        ...
        for line in verify_res.stderr.decode().splitlines():
            if line.startswith("[GNUPG:] VALIDSIG "):
                ...
                line.startswith("[GNUPG:] REVKEYSIG ")
                or line.startswith("[GNUPG:] EXPKEYSIG ")
        ...
        if not valid_sig:
            if prev_commit != "":
                print(
                    "No parent of {} was signed with a trusted key!".format(
                        prev_commit
                    ),
                    file=sys.stderr,
                )
                ...
                parents = (
                    subprocess.check_output(
                        [GIT, "show", "-s", "--format=format:%P", prev_commit]
                    )
                    .decode("utf8")
                    .splitlines()[0]
                    .split(" ")
                ...
                print(
                    "{} was not signed with a trusted key!".format(
                        current_commit
                    ),
                    file=sys.stderr,
                )
        ...
        if (
            ...
            if (
                ...
                print(
                    "Tree-SHA512 did not match for commit " + current_commit,
                    file=sys.stderr,
                )
                sys.exit(1)
        ...
        parents = (
            subprocess.check_output(
                [GIT, "show", "-s", "--format=format:%P", current_commit]
            )
            .decode("utf8")
            .splitlines()[0]
            .split(" ")
        ...
        if len(parents) > 2:
            print(
                "Commit {} is an octopus merge".format(current_commit),
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        commit_time = int(
            subprocess.check_output(
                [GIT, "show", "-s", "--format=format:%ct", current_commit]
            )
            .decode("utf8")
            .splitlines()[0]
        )
        ...
        if len(parents) == 2 and check_merge and not allow_unclean:
            ...
            if current_tree != recreated_tree:
                print(
                    "Merge commit {} is not clean".format(current_commit),
                    file=sys.stderr,
                )
