...
def call_security_check(cc, source, executable, options):
    ...
    subprocess.run(
        [*cc, source, "-o", executable] + env_flags + options, check=True
    )
    p = subprocess.run(
        [
            os.path.join(os.path.dirname(__file__), "security-check.py"),
            executable,
        ],
    ...
    def test_PE(self):
        ...
        self.assertEqual(
            ...
            (
                1,
                executable
                + ": failed PIE DYNAMIC_BASE HIGH_ENTROPY_VA NX RELOC_SECTION CONTROL_FLOW Canary",
            ),
        )
        self.assertEqual(
            call_security_check(
        ...
        if arch == lief.ARCHITECTURES.X86:
            ...
            self.assertEqual(
                call_security_check(
                    ...
                    source,
                    executable,
                    ["-Wl,-bind_at_load", "-fstack-protector-all"],
                ),
                (0, ""),
            )
