#!/usr/bin/env python3
# Copyright (c) 2013-2022 The Sugarchain Core developers
...
def filterbyasn(
    asmap: ASMap, ips: List[Dict], max_per_asn: Dict, max_per_net: int
) -> List[Dict]:
    """Prunes `ips` by
...
def parse_args():
    argparser = argparse.ArgumentParser(
        description="Generate a list of sugarchain node seed ip addresses."
    )
    argparser.add_argument(
        "-a",
        "--asmap",
        help="the location of the asmap asn database file (required)",
        required=True,
    )
    argparser.add_argument(
        "-s",
...
def main():
    ...
    ips.sort(
        key=lambda x: (x["uptime"], x["lastsuccess"], x["ip"]), reverse=True
    )
    # Filter out hosts with multiple sugarchain ports, these are likely abusive
    ips = filtermultiport(ips)
    print(
        f"{ip_stats(ips):s} Filter out hosts with multiple sugarchain ports",
        file=sys.stderr,
    )
