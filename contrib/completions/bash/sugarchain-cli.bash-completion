...
_sugarchain_rpc() {
    ...
    $sugarchain_cli "${rpcargs[@]}" "$@"
}

_sugarchain_cli() {
    local cur prev words=() cword
    local sugarchain_cli
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($sugarchain_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_sugarchain_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _sugarchain_cli sugarchain-cli
